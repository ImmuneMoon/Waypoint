...
```jsonc
{
  ...
  "campaigns": {
    "camp_...": {
      ...
      "system": { "v": 1, "name": "Basic d20", "fields": [], "rolls": [], "sheet": { "sections": [] } },   // OPTIONAL, app-managed (1.5.0): the campaign's rules system for character sheets (System button in the Campaign pill; docs in Help ▸ Character sheets). Travels to players with GM-only fields removed, and with any pool (resource) whose max is worked out from one, since its value would show the max. Carry through untouched; author only through the editor or an exported .wpsystem.json. Its "combat" may carry "turn" (turn-based combat, all optional): "move" (a formula, no dice: how far a character moves in one turn) with "unit" ("cells" | "ft" | "yd" | "m"), "diag" ("one" | "alt": how a square grid counts a diagonal; absent: a straight line), "secs" (how long a round lasts, in seconds), "units" (up to six [{ "key", "label", "secs" }]: the system's own time units) and "acts" (up to six [{ "key", "label", "n" 1-9 }]: what one turn allows). Since 1.5.0 it may hold "effects" (the status-effect library, each with the numbers it changes), and a Status effects field's value on a character is a list of rows referring to it or made on the spot. An Item list field may carry list options ("list": categories, a level and a switch per row, no quantity, the same item more than once), and its rows on a character carry their own facts ("lvl", "on", "note"); an item may carry a "key", a default "lvl" and a GM-only equip lock ("eq", "eqMsg") that players never receive. Since the F4c folds a list may also carry up to ten "stats" (numbers its items hold: key, label, default, value names, shown on the row), a "price" (one of those stats), "custom" (players may add rows of their own) and up to six "cols" (a formula worked out per row: key, label, formula, unit, value names, hide, foot; formulas elsewhere read a list's totals and a row by its item's key); a stat may be a choice ("kind": "pick" with "opts": [{ "label", "name" }], a formula reading the named value; an item holds the option's label); a list may carry up to four "rolls" (a label and a formula, dice allowed: a button on each row, the formula reading that row as Row.*; any of them may instead be an apply action, "apply" as below, its amounts reading the row); a list may carry up to four "counters" (key, label, a start "def", a most "max" formula reading the row) and a row then its own "ct" ({ key: a whole number }); a list's roll may carry "then" (as below; a change may name a counter, { "c": key }, in place of a field) and "needs" (a formula it must meet, "needsText" when it does not) and "malf" (as below, reading the row); an item may carry "stats" (its values under the lists' stat keys); the system may carry "listRules" ({"ownerStats": true}: players may change the stats of their own copies). A row may carry "paid" (what one cost, recorded when added; the GM's), "ov" (that copy's own values over the library: name, icon, category, notes, stats, a blast, and GM-only damage/cost formulas and locks that never reach players; "held" lists the stats the GM set) and, on a row the character made, its own "def" with "own": 1 (the player made it). A roll may instead be an apply action: "apply" (up to four changes, each { "f": a pool or number field's id, "formula": the amount, no dice, "add": true to raise the value instead, or "set": true to make it the amount }) in place of its "formula", never the initiative roll — a button that moves those values (any roll or apply action, a system's or a list's, may carry "cost": the key of an action a turn allows, below); with "round": true (Each round) the host also runs it on every character with a token in a combat each time the combat's round changes. A roll may also carry "then" (up to four such changes, each with "when": "hit", "miss" or "malf" for On success / On failure of its test / On malfunction, absent for always): what the roll changes after it lands; and "malf" (a formula, no dice): a natural total (the dice alone) at or past it is a malfunction, a failure that takes the On malfunction changes (a roll's record then carries "malf", the number it was worked out to). All app-managed: carry through untouched
      "chars": { "c_x": { "id": "c_x", "name": "Pat", "ownerId": "u_abc123", "portrait": "", "npc": false, "values": { "f_str": 12 } } },   // OPTIONAL, app-managed (1.5.0): the campaign's characters, values keyed by the system's field ids; tokens point at one with `charId`; players receive their own in full (minus GM-only fields and such pools), other PCs' hover fields, NPCs never. Carry through untouched, never author
...
- `elevation` + `posture` — (1.4.6, character tokens) the token's height above the ground and its handbook ch. 9 posture. `elevation` is a free number in **yards** (a ledge at `3`, a catwalk at `5`, a pit at `-2`); `posture` is one of `"standing"` | `"crouching"` | `"sitting"` | `"kneeling"` | `"crawling"` | `"lying-prone"` | `"lying-face-up"` — the same ids shadow-base.com uses in `details.posture` (the 1.4.6 pre-release ids `prone` / `supine` and the handbook's long names are still accepted on read, but author the canonical ones). **Omit both** for ground level / standing — the app deletes them when they are at the default, and a drop that omits them means exactly that. App-managed like `charStats`: the GM edits them in Properties or the token's right-click menu, a player may set them on their own token in a session (same permission line as moving it), and an attached ShadowBase sheet's `details.posture` / `details.elevation` seed the token's and Export JSON writes them back. Whether they *show* is a VTT feature setting (1.4.9: ⚙ Settings ▸ VTT features — **Token elevation** and **Token posture**, both on from the first launch, set **per campaign** in the campaign's app-managed `vtt` block and never authored in a drop; the default for new campaigns lives in `saves/preferences.json`); the values are kept either way. With Elevation on, the 💥 Blast tool measures each token's distance to a blast straight-line, height included (`d = √(h² + v²)`, h = hex distance in yards, one hex = one yard), and rulers between two tokens at different heights print the 3D figure too. Both fields are sent to players. Character-sheet formulas read them as the built-in names `Posture` (0 standing, 1 crouching, 2 sitting, 3 kneeling, 4 crawling, 5 lying-prone, 6 lying-face-up) and `Elevation` (yards) (1.5.0).
- `threats` — (1.5.0, character tokens) threat marks from a character sheet's **Facing dial**: an array of at most 6 world bearings in whole degrees, in (-180, 180], 0 = up, clockwise (the same frame as `rot` + `front`); the first is the active threat, the rest are queued. **Omit** it for none. Sheet formulas read it through the built-in names `Arc`, `Arc.front` / `.side` / `.rear`, `Threats`, `Threats.front` / `.side` / `.rear` and `Facing`. Sent to players; a player may set it on their own token while Token facing is on. A copy of the token made for another map (travel, summon, duplicate) leaves it behind.
...
- On `text` items, `color` is the **text color** (not a fill) — `"transparent"`/unset renders the default light ink. Any CSS color works. (1.4.6) `textOpacity` (0.1–1) and `bgOpacity` (0–1) fade the words and the box background separately; both optional, both independent of the whole-item `opacity`.
...
A page is a **player-facing** document: what the GM previews is exactly what a joined player reads. Its block roster is the player-safe subset of the planner's plus three of its own — `h1`, `h2`, `h3` (`{ "title" }`), the prose blocks `text` / `lede` / `oneline` / `callout` / `flare` (`content` = HTML, reduced on render to p, br, b/strong, i/em, u, s, ul/ol/li, code, pre and `a href="http(s)…"`; anything else becomes plain text), `image` (`src` must be a `/saves/images/...` path; `caption`, `alt`), `table` (`{ "title", "cols": [...], "rows": [...] }` — rows as `{ "col1": ... }` objects like a planner node or as arrays; at most 8 columns, cells are text), `rule` (a horizontal line), `diagram` and `flowchart` (as in planners; lines beginning `click`, `callback`, `href` or `linkStyle` are removed for players). **Not allowed on a page:** `node` (scene nodes), `raw`, GM notes of any kind — a block of another type is dropped when the page is sent. Caps: 300-character titles, 2,000-character cells, 64 KB per prose block, 500 blocks and 2 MB per page (a longer page reaches players truncated with a notice). An optional `layout` object on `image`, `callout`, `flare`, `table`, `diagram` and `flowchart` (`{ "width": 25|33|50|66|75|100, "float": "none"|"left"|"right", "dx": -200..200, "dy": -200..200, "span": false }`) and `cols` (1–3) on `h2` describe the flowing layout; the editor's Layout row sets them (and a picture can be dragged in the preview to nudge or resize it), the renderer honours them.

**Markdown (1.5.0):** a planner or a page can be authored as a `.md` file and imported through the app (header Import button, the planner toolbar's Import…, or the file button beside + in the Planners / Handbook headers), with its pictures picked alongside or zipped with it; and exported back (Export ▸ Save Markdown). Dialect: front matter `title` / `subtitle` / `status` (planners) / `players` (pages); the first `#` is the title, later `#` and `##` sections (`## Name {cols=2}` = page columns), `###` a page sub-heading — in a planner `### Scene: Name` opens a scene node with `**Tag:**`, `**Must resolve:**`, `Map: <map title> / <room name>` lines and a table under it; `> quote` = callout, `> [!lede|oneline|flare|callout]` or `::: type … :::` = that prose block; pipe tables (≤ 8 columns, a bold line above = title); ```` ```mermaid ```` = diagram, ```` ```flowchart ```` in the simple subset (`a["Label"]:::gold --> b{"Choice?"}`, `-->|"text"|`, `-.->`) = a native flowchart; `![caption](path){width=50 float=left dx=0 dy=0 span}` = picture; `---` = rule (page) / text-block boundary (planner). Raw HTML from a file is reduced to the page allow-list, never a `raw` block. The full sheet is in Help ▸ Handbook ▸ Import and export.
