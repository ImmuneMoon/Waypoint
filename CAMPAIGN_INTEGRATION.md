...
```jsonc
{
  ...
  "campaigns": {
    "camp_...": {
      ...
      "system": { "v": 1, "name": "Basic d20", "fields": [], "rolls": [], "sheet": { "sections": [] } },   // OPTIONAL, app-managed (1.5.0): the campaign's rules system for character sheets (System button in the Campaign pill; docs in Help ▸ Character sheets). Travels to players with GM-only fields removed. Carry through untouched; author only through the editor or an exported .wpsystem.json. Since 1.5.0 it may hold "effects" (the status-effect library, each with the numbers it changes), and a Status effects field's value on a character is a list of rows referring to it or made on the spot
      "chars": { "c_x": { "id": "c_x", "name": "Pat", "ownerId": "u_abc123", "portrait": "", "npc": false, "values": { "f_str": 12 } } },   // OPTIONAL, app-managed (1.5.0): the campaign's characters, values keyed by the system's field ids; tokens point at one with `charId`; players receive their own in full (minus GM-only fields), other PCs' hover fields, NPCs never. Carry through untouched, never author
