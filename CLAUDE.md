...
| `tools/netcheck.js` | host-side wire gates: admission, identity, rate limits, chat, the patch/pos/threat gates (a locked token), stage and summons (admitted peers only), travel, Forget, the GM's own roll privacy (slices `net.js` by `[netcheck:*]` markers) |
