...
function inspectCampaign(camp) {
    ...
    if (isObj(camp.chars) && Object.values(camp.chars).some(function(ch) { return isObj(ch) && isObj(ch.values) && Object.values(ch.values).some(function(v) { return Array.isArray(v) && v.some(function(r) { return isObj(r) && (r.hid === 1 || r.keptOn === 1 || isObj(r.snap) || (isObj(r.def) && r.lnk !== 1 && (r.def.damage || r.def.cost || r.def.rm || r.def.rmMsg || r.def.eq || r.def.eqMsg || r.def.throwSkill || (r.def.vis === 'gm' && (r.def.area || r.def.key))))); }); }); })) lm.gm++;   // Stage 6: a copy of a deleted item (players get it inline), a custom item's GM texts or removal rule, a curse kept out of its owner's sight — none travels as stored
