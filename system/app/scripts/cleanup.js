...
function inspectCampaign(camp) {
    ...
    if (isObj(camp.system)) {   // character sheets (1.5.0): the system TRAVELS as the players' view, so only a GM-only field, roll or item is a local mark; an all-visible system proves nothing
        var sf = Array.isArray(camp.system.fields) ? camp.system.fields : [], sr = Array.isArray(camp.system.rolls) ? camp.system.rolls : [], si = Array.isArray(camp.system.items) ? camp.system.items : [];
        if (sf.some(function(f) { return isObj(f) && f.vis === 'gm'; }) || sr.some(function(r) { return isObj(r) && r.vis === 'gm'; }) || si.some(function(it) { return isObj(it) && it.vis === 'gm'; })) lm.gm++;   // a GM-only item never travels
    }
