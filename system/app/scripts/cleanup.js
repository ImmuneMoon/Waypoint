...
function inspectCampaign(camp) {
    ...
    if (isObj(camp.system)) {   // character sheets (1.5.0): the system TRAVELS as the players' view, so only a GM-only field, roll or item is a local mark; an all-visible system proves nothing
        ...
        if (sf.some(function(f) { return isObj(f) && f.vis === 'gm'; }) || sr.some(function(r) { return isObj(r) && r.vis === 'gm'; }) || si.some(function(it) { return isObj(it) && it.vis === 'gm'; }) || se.some(function(d) { return isObj(d) && d.vis === 'gm'; })) lm.gm++;   // a GM-only item never travels
    }
