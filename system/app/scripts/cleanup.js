...
function inspectCampaign(camp) {
    ...
    if (isObj(camp.system)) {   // character sheets (1.5.0): the system TRAVELS as the players' view, so only a GM-only field, roll or item is a local mark; an all-visible system proves nothing
        ...
        if (sf.some(function(f) { return isObj(f) && f.vis === 'gm'; }) || sr.some(function(r) { return isObj(r) && r.vis === 'gm'; }) || si.some(function(it) { return isObj(it) && (it.vis === 'gm' || it.rm === 'bound' || it.rm === 'curse' || it.eq === 'bound' || it.eq === 'curse'); }) || se.some(function(d) { return isObj(d) && d.vis === 'gm'; })) lm.gm++;   // a GM-only item never travels
    }
    ...
    if (isObj(camp.chars) && Object.values(camp.chars).some(function(ch) { return isObj(ch) && isObj(ch.values) && Object.values(ch.values).some(function(v) { return Array.isArray(v) && v.some(function(r) { return isObj(r) && (r.hid === 1 || r.keptOn === 1 || isObj(r.snap) || (isObj(r.ov) && (r.ov.damage || r.ov.cost || r.ov.rm || r.ov.rmMsg || r.ov.eq || r.ov.eqMsg)) || (isObj(r.def) && r.lnk !== 1 && (r.def.damage || r.def.cost || r.def.rm || r.def.rmMsg || r.def.eq || r.def.eqMsg || r.def.throwSkill || (r.def.vis === 'gm' && (r.def.area || r.def.key))))); }); }); })) lm.gm++;   // Stage 6: a copy of a deleted item (players get it inline), a custom item's GM texts or removal rule, a curse kept out of its owner's sight, a copy's own formulas or locks (F4c2) — none travels as stored
