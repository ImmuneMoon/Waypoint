...
(function () {
    ...
    var DESC = {
        ...
        wpFog: 'Fog of war (fog.js): the GM tools and per-recipient host enforcement. GM side - openMenu / closeMenu, paintAt(x, y, hide), setPreview(id) / preview(), brush(), active(), sync() / redraw(), stats(). Host side - fogDropIds(recipientId, camp, map), canSeePoint(pid, camp, map, x, y), tokenSightCells, invalidateVision: the ids and points net.js uses to drop from a recipient what their tokens cannot see. The pure half is fogcore.js (window.wpFogCore).',
        wpFogCore: 'The fog geometry and validators, pure (scripts/fogcore.js): gridFor, cellOf, cellCenter, cellKey, hexDist, rangeToCells, visibleCells, revealedKeys, pointRevealed, cleanFog, cleanCampFog, plus RULESETS / MODES / LIMITS. No state, no DOM; tools/fogcheck.js runs it under Node.',
        wpFogSync: 'Re-check the Fog of war feature for the campaign or table on screen: shows or hides the fog tool and repaints (called by the VTT toggle fan-out). wpFogRedraw() just repaints the current fog overlay.',
