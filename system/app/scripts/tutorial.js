...
function tutorialSystem() {
    return { v: 1, name: 'Tutorial rules', preset: '', updated: 1,
        ...
        sheet: { sections: [], identity: [{ id: 'f_tut_class' }, { id: 'f_tut_str' }, { id: 'f_tut_dex' }, { id: 'f_tut_con' }], ledger: [{ id: 'f_tut_strmod' }, { id: 'f_tut_sight' }], band: [{ id: 'f_tut_hp' }, { id: 'f_tut_ac' }, { id: 'f_tut_prone' }, { roll: 'r_tut_init' }] } };   // the automatic layout, with a header block (Stage 5d) and a pinned band (Stage 5c) so the tour shows them
}
...
function ensureTutorialSheet(camp) {
    ...
    if (camp.system && typeof camp.system === 'object') {
        ...
        if (tsh && typeof tsh === 'object' && !Array.isArray(tsh.identity) && !Array.isArray(tsh.ledger) && !(Array.isArray(tsh.sections) && tsh.sections.length) && !(Array.isArray(tsh.tabs) && tsh.tabs.length)) { tsh.identity = [{ id: 'f_tut_class' }, { id: 'f_tut_str' }, { id: 'f_tut_dex' }, { id: 'f_tut_con' }]; tsh.ledger = [{ id: 'f_tut_strmod' }, { id: 'f_tut_sight' }]; changed = true; }
    }
...
var STEPS = [
    ...
    { target: '#sheetPanel', title: 'A character sheet',
      html: 'Bren&rsquo;s sheet, over the play map. Characters live in the editor&rsquo;s <b>Characters</b> tab and any token can point at one (right-click a token &#9656; <b>Sheet&hellip;</b>): numbers and skills with their totals, HP as a bar with &minus; and +, conditions, notes &mdash; the <b>header block</b> under the name says who Bren is (Fighter, his abilities, STR modifier, Sight) and the <b>band</b> keeps HP, AC, Prone and Initiative in reach on every tab &mdash; and the roll buttons roll at the table with the sheet&rsquo;s values. Drag its head to move it and its bottom-right corner to resize it; both are remembered. Bren is <b>Blessed</b>: the &#9650; beside his Sword total is the effect &mdash; hover it for the source. A player opens their own the same way and edits what you left editable; fields marked <b>Hover</b> show on the hover card and in the party strip. Bren&rsquo;s <b>Kit</b> holds a <b>Firepot</b> with a &#128165; <b>Throw</b> &mdash; press it, then click the map.',
