...
var STEPS = [
    ...
    { target: '#wbFloatingToolbar', title: 'Play map tools',
      html: 'Now inside the cave, on its Play Map. Left to right: centre, undo, grid and snap, then the tools — move, pan, draw, erase, <b>measure</b> (rulers) and <b>blast</b> (a grenade radius with per-token distances), then text, shapes, images and the picture library, and <b>Import Character</b> for a shadow-base.com sheet. <i>The blast button is a stopgap: blasts will be thrown from the VTT character sheets once those are in, and the blast presets are not permanent &mdash; they will follow each campaign\'s own weapons.</i>',
    ...
    { target: '#whiteboardWrap', title: 'Tokens',
      html: 'Any shape or image with <b>Is Character</b> set is a token: hover it for its name and stats, drag it (it seats itself in a hex), <b>right-click</b> it for conditions, posture and elevation. The <b>Cave Wyrm</b> is hidden from players — you see it dimmed — until you tick <b>Visible to players</b>. The hex trigger zone fires its message when a token is dropped on it.',
