...
(async () => {
    ...
    /* ---- Stage 6 F4c1: item stats, the price and what was paid ---- */
    {
        ...
        check('F4c1 tour and Help: the Items step (a box per stat), the Lists step (ten stats, On the row, the price and Paid, the key is the identity, formulas later) and the sheet step (stats and what was paid); Help\'s Items and Lists lines; the Lists tab\'s title and both toolbar notes',
            /Once a list has <b>stats<\/b>, an item it can be on gets a box for each \(Acc, Wt, Cost&hellip;\); formulas will read them in a later update\./.test(tut4) && /A list can carry up to ten <b>stats<\/b>[^']*a <b>price<\/b> stat: a row records what one cost when it is added \(<b>Paid<\/b>, yours to correct on the row\)\. A stat&rsquo;s key is its identity[^']*Formulas will read stats in a later update\./.test(tut4) && /Rows show their stats and what was paid\./.test(tut4)
            && /an item gets a box for each stat of the lists it can be on/.test(html4) && /A list can also carry up to ten <b>stats<\/b>/.test(html4) && /One stat can be the list&rsquo;s <b>price<\/b>: a row records it as <b>Paid<\/b> when it is added/.test(html4) && /Formulas will read stats and Paid in a later update\./.test(html4)
