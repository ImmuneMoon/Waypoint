...
(async () => {
    ...
    /* ---- Stage 6 F4b: row facts (level, switch, note), list options, the equip lock, the Lists tab ---- */
    {
        ...
        const deps = ['el', 'iconNode', 'rowDef', 'rowIdOf', 'rowLvl', 'rowOn', 'commitItem', 'opt', 'fmtNum', 'iconText', 'LIMITS', '_fxLive', 'document', 'renderViews', 'closeHud', 'closeSheet', 'window', 'systemOf', 'getActiveCampaign', 'facingTarget', 'uid', 'F', 'fxText', 'canRoll', 'sheetRoll', 'commit', 'valueTone', 'TONE_CLASS', 'rowStat', 'rowPaid', '_fxView'];
        const depV = [fe, (v, cls) => fe('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, (c, f, q) => commits.push([f.id, q]), optF, S.fmtNum, v => v || '', S.LIMITS, true, docF, () => {}, () => {}, () => {}, {}, () => sysB, () => null, () => null, p => p + 'new', () => F, S.fxText, () => true, () => {}, () => {}, S.valueTone, {}, S.rowStat, S.rowPaid, 'sheet'];
    ...
    /* ---- Stage 6 F4b review: the lock covers dropping while it is on; a stored "on"; the level clamp's fixed point; the UI findings ---- */
    {
        ...
        const WR = new Function('el', 'iconNode', 'rowDef', 'rowIdOf', 'rowLvl', 'rowOn', 'commitItem', 'opt', 'fmtNum', 'iconText', 'LIMITS', '_fxLive', 'document', 'renderViews', 'closeHud', 'closeSheet', 'window', 'rowStat', 'rowPaid', '_fxView', cutR('// ---- item-list widgets', '// 5h: an effect\'s changes as short text') + '\nreturn { itemListInto: itemListInto };')(
            feR, (v, cls) => feR('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, () => {}, optR, S.fmtNum, v => v || '', S.LIMITS, true, docR, () => {}, () => {}, () => {}, {}, S.rowStat, S.rowPaid, 'sheet');
    ...
    /* ---- Stage 6 F4c1: item stats, the price and what was paid ---- */
    {
        const LS = S.cleanListSpec, SK = k => S.statKey(k, F), C7 = String.fromCharCode(7);
        const rawS = { v: 1, name: 'S', rolls: [], fields: [
            { id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', edit: 'owner', vis: 'all', list: { multi: true, on: { label: 'Readied' }, cats: ['Gun', 'Blade'], price: 'cost', stats: [{ key: 'Acc', show: true }, { key: 'Dmg', def: 1, show: true }, { key: 'Cost' }, { key: 'Wt' }, { key: 'diff', labels: ['E', 'A', 'H', 'VH'] }] } },
            { id: 'f_sk', key: 'Skills', label: 'Skills', kind: 'item-list', edit: 'owner', vis: 'all', list: { noQty: true, cats: ['Skill'], stats: [{ key: 'Rel' }] } },
            { id: 'f_sc', key: 'Secret', label: 'Secret', kind: 'item-list', edit: 'owner', vis: 'gm', list: { cats: ['Gun'], stats: [{ key: 'Heat' }] } },
            { id: 'f_pl', key: 'Plain', label: 'Plain', kind: 'item-list', edit: 'owner', vis: 'all' }],
            items: [
            { id: 'i_blaster', name: 'Blaster', category: 'Gun', stats: JSON.parse('{"Acc":2,"Dmg":3,"Cost":500,"Wt":0.0001,"Heat":7,"Rel":1,"Zzz":4,"toString":5,"constructor":6}') },
            { id: 'i_knife', name: 'Knife', category: 'Blade', stats: { Cost: 99999999, Wt: 2e9 } },
            { id: 'i_rune', name: 'Rune', category: 'Gun', vis: 'gm', key: 'Rune', area: { ft: 5 }, damage: '2d6', cost: '10', rm: 'bound', stats: { Acc: 5, Cost: 10, Heat: 9 } },
            { id: 'i_sneak', name: 'Sneak', category: 'Skill', stats: { Rel: -2 } },
            { id: 'i_bare', name: 'Bare', category: 'Gun' }] };
        const sysS = cleanSystem(rawS, { F, gmView: true }), pvS = cleanSystem(sysS, { F, gmView: false }), fS = id => sysS.fields.find(f => f.id === id), pS = id => pvS.fields.find(f => f.id === id), itS = id => sysS.items.find(i => i.id === id), libS = {}; sysS.items.forEach(i => { libS[i.id] = i; });
        const wpL = fS('f_wp').list, blG = itS('i_blaster'), blP = pvS.items.find(i => i.id === 'i_blaster');

        const refused = ['__proto__', 'constructor', 'Constructor', 'toString', 'valueOf', 'hasOwnProperty', 'isPrototypeOf', 'max', 'qty', 'Paid', 'Row', 'lvl', 'base', 'Base', 'ranks', 'true', 'floor', 'if', 'and', '1a', '_a', 'a.b', 'a b', ' Acc', 'Acc ', 'x'.repeat(25), '', 7, null];
        const eleven = LS({ stats: Array.from({ length: 11 }, (_, i) => ({ key: 'S' + i })) }, true, F), dup = LS({ stats: [{ key: 'Acc', label: 'First' }, { key: 'acc', label: 'Second' }, { key: 'Dmg', kind: 'pick' }, { key: 'Wt', kind: 'num' }] }, true, F);
        const odd = LS({ stats: [{ key: 'A', def: 0 }, { key: 'B', def: '3' }, { key: 'C', def: 2e9 }, { key: 'D', def: -0.5, label: 'Dee' + C7 + 'x', labels: Array.from({ length: 12 }, (_, i) => 'N' + i), show: 1 }, { key: 'E', show: true }, 'x', null] }, true, F);
        const order = LS({ cats: ['A'], multi: true, noQty: true, lvl: {}, on: {}, stats: [{ key: 'A' }], price: 'a' }, true, F), f4b = [{ lvl: { min: 1, labels: ['I', 'II'] }, on: { label: 'Active', def: true }, cats: ['Force'], multi: true }, { noQty: true, cats: [] }, { on: {} }, {}];
        check('F4c1 the stats schema: at most 10 a list (11 become 10), 16 an item, values within 1e9; a key is a letter, then letters, digits and _ (24 at most, never trimmed) — never a prototype name in either case, a row word, a reserved suffix (base: cpBase), a function name or one the engine cannot address; once ignoring case (the first wins); kind num, or pick (F5a2: a choice with its options); a default of 0 or not a number left out; names cut to 10; On the row only when true; the price stored as its stat\'s key (cost: Cost) or dropped; the keys in order; a list without stats cleans as before',
            S.LIMITS.listStats === 10 && S.LIMITS.entryStats === 16 && S.LIMITS.statAbs === 1e9 && eleven.stats.length === 10 && eleven.stats[9].key === 'S9'
            && refused.every(k => SK(k) === '') && SK('Acc') === 'Acc' && SK('cpBase') === 'cpBase' && SK('a'.repeat(24)) === 'a'.repeat(24) && S.statKey('base') === '' && S.statKey('constructor') === '' && S.statKey('Acc') === 'Acc'
            && j(dup.stats) === j([{ key: 'Acc', label: 'First' }, { key: 'Dmg', label: 'Dmg', kind: 'pick', opts: [] }, { key: 'Wt', label: 'Wt' }])
            && j(odd.stats) === j([{ key: 'A', label: 'A' }, { key: 'B', label: 'B' }, { key: 'C', label: 'C' }, { key: 'D', label: 'Dee x', def: -0.5, labels: Array.from({ length: 10 }, (_, i) => 'N' + i) }, { key: 'E', label: 'E', show: true }])
            && wpL.price === 'Cost' && !('price' in LS({ stats: [{ key: 'A' }], price: 'nope' }, true, F)) && LS({ price: 'A' }, true, F) === null && LS({ stats: [{ key: 'max' }] }, true, F) === null
            && j(Object.keys(order)) === j(['cats', 'multi', 'noQty', 'lvl', 'on', 'stats', 'price']) && order.price === 'A' && f4b.every(x => j(LS(x, true, F)) === j(LS(x, true)) && !/stats|price/.test(j(LS(x, true, F)))), j([refused.filter(k => SK(k) !== ''), dup, odd, order]));

        const gen = S.cleanItemDef({ id: 'i_x', name: 'X', stats: JSON.parse('{"Zzz":1,"max":2,"constructor":3,"__proto__":4,"a":"3","b":null,"c":1e10}') }, F, true), many = {}; for (let i = 0; i < 17; i++) many['K' + i] = i + 1;
        const bad3 = S.cleanItemDef({ id: 'i_y', name: 'Y', stats: { A: '3', B: NaN, C: Infinity, D: -Infinity } }, F, true), cs17 = S.cleanItemDef({ id: 'i_z', name: 'Z', stats: many }, F, true);
        const caseSys = cleanSystem(Object.assign({}, rawS, { items: [{ id: 'i_p', name: 'P', category: 'Gun', stats: { acc: 1, ACC: 2, cost: 3 } }] }), { F, gmView: true });
        check('F4c1 entry stats: an item keeps a stat any item list of the view defines (the GM\'s: Blaster keeps Heat and Rel — never scoped by category) under the list\'s spelling, and drops one no list has (Zzz) and a prototype name; a value is a number within 1e9 (99,999,999 kept, 2e9, a string, NaN and Infinity dropped), 16 at most, a plain object; the players\' view keeps visible lists\' stats only (no Heat anywhere; the GM-only Rune gone); both views are fixed points; an item with none has no key; without the lists, the key rule alone; a blast\'s name cleans every control character',
            j(blG.stats) === j({ Acc: 2, Dmg: 3, Cost: 500, Wt: 0.0001, Heat: 7, Rel: 1 }) && j(itS('i_knife').stats) === j({ Cost: 99999999 }) && j(itS('i_sneak').stats) === j({ Rel: -2 })
            && j(blP.stats) === j({ Acc: 2, Dmg: 3, Cost: 500, Wt: 0.0001, Rel: 1 }) && !pvS.items.some(i => i.id === 'i_rune') && !/Heat/.test(j(pvS))
            && j(cleanSystem(sysS, { F, gmView: true })) === j(sysS) && j(cleanSystem(pvS, { F, gmView: false })) === j(pvS)
            && !('stats' in itS('i_bare')) && j(gen.stats) === j({ Zzz: 1 }) && j(bad3) === j(S.cleanItemDef({ id: 'i_y', name: 'Y' }, F, true)) && Object.keys(cs17.stats).length === 16 && !('K16' in cs17.stats)
            && j(caseSys.items[0].stats) === j({ Acc: 1, Cost: 3 }) && Object.getPrototypeOf(blG.stats) === Object.prototype
            && S.cleanItemDef({ id: 'i_a', name: 'A', area: { ft: 5, name: 'a' + C7 + 'b' + C7 + 'c' } }, F, true).area.name === 'a b c', j([blG.stats, blP.stats, gen.stats, cs17.stats, caseSys.items[0].stats]));

        const vo = S.valueOpts(sysS), tF = { id: 'f_t', kind: 'item-list', list: LS({ lvl: { min: 0, max: 5 }, on: { label: 'On' }, stats: [{ key: 'Acc' }, { key: 'Cost' }], price: 'Cost' }, true, F) };
        const ord = S.cleanValue(tF, [{ paid: 12.5, note: 'n', on: true, lvl: 2, qty: 1, defId: 'i_blaster', id: 'w_1' }], vo)[0];
        const paidOf = p => S.cleanValue(fS('f_wp'), [{ id: 'w_a', defId: 'i_blaster', qty: 1, paid: p }, { defId: 'i_knife', qty: 1, paid: p }, { id: 'w_c', qty: 1, paid: p, def: { name: 'C' } }, { id: 'w_i', qty: 1, paid: p, def: { name: 'I' }, lnk: 1 }, { id: 'w_l', defId: 'i_gone', qty: 1, paid: p, snap: { name: 'L' } }], vo).map(r => 'paid' in r ? r.paid : 'none');
        const kinds = S.cleanValue(fS('f_wp'), [{ id: 'w_c', qty: 1, def: { name: 'Mine', stats: { Acc: 1, Rel: 2, Zzz: 3, cost: 4 } } }, { id: 'w_i', qty: 1, def: { name: 'In', stats: { Acc: 1, Rel: 2 } }, lnk: 1 }, { id: 'w_l', defId: 'i_gone', qty: 1, snap: { name: 'Old', stats: { Acc: 1, Rel: 2, Zzz: 3 } } }], vo);
        const plainC = S.cleanValue(fS('f_pl'), [{ id: 'w_c', qty: 1, def: { name: 'Mine', stats: { Acc: 1 } } }], vo), f4bRows = [{ id: 'w_1', defId: 'i_blaster', qty: 2, on: true, note: 'x' }, { defId: 'i_knife', qty: 1 }, { id: 'w_c', qty: 1, on: false, def: S.cleanRowDef({ name: 'Mine' }, true) }];
        check('F4c1 row facts: what one cost (paid) comes after the note — {id, defId, qty, lvl, on, note, paid}; a number 0..1e9 on every row kind (0 and 12.5 kept; -1, 1e10, a string, NaN, null dropped); a custom or inline copy keeps its list\'s stats only (under its spelling; none on a list without stats), a deleted item\'s copy all of its own; a row without them cleans as before',
            j(Object.keys(ord)) === j(['id', 'defId', 'qty', 'lvl', 'on', 'note', 'paid']) && j(paidOf(0)) === j([0, 0, 0, 0, 0]) && j(paidOf(12.5)) === j([12.5, 12.5, 12.5, 12.5, 12.5]) && [-1, 1e10, 'x', NaN, null].every(p => j(paidOf(p)) === j(['none', 'none', 'none', 'none', 'none']))
            && j(kinds[0].def.stats) === j({ Acc: 1, Cost: 4 }) && j(kinds[1].def.stats) === j({ Acc: 1 }) && j(kinds[2].snap.stats) === j({ Acc: 1, Rel: 2, Zzz: 3 }) && !('stats' in plainC[0].def)
            && j(S.cleanValue(fS('f_wp'), f4bRows, vo)) === j(f4bRows), j([ord, kinds, plainC]));

        const bare = itS('i_bare');
        check('F4c1 rowStat and rowPaid: a stat as its definition holds it (the list\'s spelling: acc reads Acc), else the stat\'s default (Dmg 1), else 0; what one cost as recorded (0 counts), else the list price (never below 0), else 0',
            S.rowStat(wpL, blG, 'Acc') === 2 && S.rowStat(wpL, blG, 'acc') === 2 && S.rowStat(wpL, bare, 'Dmg') === 1 && S.rowStat(wpL, bare, 'Acc') === 0 && S.rowStat(wpL, { stats: { Acc: 'x' } }, 'Acc') === 0 && S.rowStat(null, { stats: { Q: 4 } }, 'Q') === 4 && S.rowStat(wpL, blG, '') === 0 && S.rowStat(wpL, blG, 'Nope') === 0
            && S.rowPaid(wpL, { paid: 7 }, blG) === 7 && S.rowPaid(wpL, { paid: 0 }, blG) === 0 && S.rowPaid(wpL, {}, blG) === 500 && S.rowPaid(wpL, {}, { stats: { Cost: -5 } }) === 0 && S.rowPaid(fS('f_sk').list, {}, blG) === 0 && S.rowPaid(null, { paid: 3 }, blG) === 3);

        const chP = { id: 'c_p', name: 'P', ownerId: 'u_p', npc: false, values: { f_wp: [], f_sk: [] } }, PS = { player: true, view: pvS };
        const opS = (fid, q, o) => { const r = S.applyRowOp(sysS, chP, fid, q, F, o || {}); if (r.ok) chP.values[fid] = r.value; return r; }, rowP = id => chP.values.f_wp.concat(chP.values.f_sk).find(r => r.id === id);
        const ad1 = opS('f_wp', { op: 'add', defId: 'i_blaster', rowId: 'w_b1' }, PS), ad2 = opS('f_wp', { op: 'add', defId: 'i_blaster', rowId: 'w_b2' }), ad3 = opS('f_wp', { op: 'add', defId: 'i_knife', rowId: 'w_k1' }, PS), ad4 = opS('f_wp', { op: 'add', defId: 'i_bare', rowId: 'w_x1' }, PS), ad5 = opS('f_sk', { op: 'add', defId: 'i_sneak', rowId: 'w_s1' }, PS);
        const addRows = j([rowP('w_b1'), rowP('w_b2'), rowP('w_k1'), rowP('w_x1'), rowP('w_s1')]);
        rowP('w_b1').paid = 450; const mg = opS('f_wp', { op: 'add', defId: 'i_blaster' }, {});
        const negS = cleanSystem({ v: 1, name: 'N', rolls: [], fields: [{ id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', edit: 'owner', vis: 'all', list: { price: 'Cost', stats: [{ key: 'Cost', def: 7 }] } }], items: [{ id: 'i_n', name: 'Refund', stats: { Cost: -5 } }, { id: 'i_d', name: 'Default' }] }, { F, gmView: true });
        const negA = S.applyRowOp(negS, { id: 'c_n', values: {} }, 'f_wp', { op: 'add', defId: 'i_n', rowId: 'w_q1' }, F, {}), defA = S.applyRowOp(negS, { id: 'c_n', values: {} }, 'f_wp', { op: 'add', defId: 'i_d', rowId: 'w_q2' }, F, {});
        check('F4c1 add: a new row records what one cost from the host\'s own entry — its price stat (Blaster 500, Knife 99,999,999), else the stat\'s default, else 0 — for a player and the GM alike; a list without a price records none; a merged add keeps the first (a recorded fact)',
            [ad1, ad2, ad3, ad4, ad5, mg].every(r => r.ok) && addRows === j([{ id: 'w_b1', defId: 'i_blaster', qty: 1, on: false, paid: 500 }, { id: 'w_b2', defId: 'i_blaster', qty: 1, on: false, paid: 500 }, { id: 'w_k1', defId: 'i_knife', qty: 1, on: false, paid: 99999999 }, { id: 'w_x1', defId: 'i_bare', qty: 1, on: false, paid: 0 }, { id: 'w_s1', defId: 'i_sneak', qty: 1 }])
            && mg.row === 'w_b1' && j(rowP('w_b1')) === j({ id: 'w_b1', defId: 'i_blaster', qty: 2, on: false, paid: 450 }) && negA.ok && negA.value[0].paid === 0 && defA.ok && defA.value[0].paid === 7, j([addRows, rowP('w_b1'), negA.value, defA.value]));
        const before1 = j(rowP('w_b1')), own1 = opS('f_wp', { op: 'set', rowId: 'w_b1', facts: { paid: 1, note: 'mine' } }, PS), after1 = j(rowP('w_b1'));
        const gm1 = opS('f_wp', { op: 'set', rowId: 'w_b2', facts: { paid: 12 } }), after12 = rowP('w_b2').paid, gmBad = [-1, 2e9, '5', NaN, Infinity].map(p => opS('f_wp', { op: 'set', rowId: 'w_b2', facts: { paid: p, note: 'x' } }).reason), still12 = j(rowP('w_b2'));
        const gm2 = opS('f_wp', { op: 'set', rowId: 'w_b2', facts: { paid: null } }), gmNo = opS('f_sk', { op: 'set', rowId: 'w_s1', facts: { paid: 3 } });
        chP.values.f_wp.push({ id: 'w_l', defId: 'i_gone', qty: 1, paid: 3, note: 'old', snap: { name: 'Old', stats: { Acc: 4, Rel: 1 } } }); const kp = opS('f_wp', { op: 'keep', rowId: 'w_l' });
        const ciP = facts => S.cleanCharItem({ rid: 'r1', charId: 'c_p', fieldId: 'f_wp', op: 'set', rowId: 'w_b1', facts });
        check('F4c1 set paid: a player\'s any paid is "field" and nothing of the change applies (the note sent with it neither); the GM\'s 12 is stored, null takes it back to the list price, a value outside 0..1e9 or not a number is "value" (the whole change), a list without a price is "value"; Make custom keeps what was paid (and the copy\'s stats of its list); the message keeps paid (a number 0..1e9 or null), refuses anything else, and an add carries none',
            own1.reason === 'field' && after1 === before1 && gm1.ok && after12 === 12 && gmBad.every(r => r === 'value') && still12 === j({ id: 'w_b2', defId: 'i_blaster', qty: 1, on: false, paid: 12 }) && gm2.ok && !('paid' in rowP('w_b2')) && gmNo.reason === 'value'
            && kp.ok && j(chP.values.f_wp.find(r => r.id === 'w_l')) === j({ id: 'w_l', qty: 1, note: 'old', paid: 3, def: Object.assign(S.cleanRowDef({ name: 'Old' }, true), { stats: { Acc: 4 } }) })
            && ciP({ paid: 12.5 }).facts.paid === 12.5 && ciP({ paid: null }).facts.paid === null && ciP({ paid: 0 }).facts.paid === 0 && [-1, 2e9, '5', true, Infinity, {}].every(p => ciP({ paid: p }) === null) && ciP({ note: 'x', paid: '5' }) === null
            && j(S.cleanCharItem({ rid: 'r1', charId: 'c_1', fieldId: 'f_gear', op: 'add', defId: 'i_rope', rowId: 'w_x1', qty: 2, paid: 5, facts: { paid: 1 }, junk: 1 })) === j({ rid: 'r1', charId: 'c_1', fieldId: 'f_gear', op: 'add', defId: 'i_rope', rowId: 'w_x1', qty: 2 }), j([own1, after1, gmBad, kp, chP.values.f_wp]));

        // critic 15: the host (the full system, judged on the view) and the player's client (the view, over its projected copy) agree on every owner op
        const agree = (sys, fid, q, rows) => {
            const pv = cleanSystem(sys, { F, gmView: false }), lib = {}; sys.items.forEach(i => { lib[i.id] = i; }); const pf = pv.fields.find(f => f.id === fid);
            const hc = { id: 'c_h', values: {} }, cc = { id: 'c_h', values: {} }; hc.values[fid] = JSON.parse(JSON.stringify(rows)); cc.values[fid] = S.projectRows(hc.values[fid], pv, lib, pf.list || null);
            const h = S.applyRowOp(sys, hc, fid, q, F, { player: true, view: pv }), c = S.applyRowOp(pv, cc, fid, q, F, { player: true, view: pv });
            return h.ok === c.ok && h.reason === c.reason && (!h.ok || j(S.projectRows(h.value, pv, lib, pf.list || null)) === j(c.value)) ? (h.ok ? 'ok' : h.reason) : 'DIFFER ' + j([h, c]);
        };
        const seedA = [{ id: 'w_r', defId: 'i_rune', qty: 1, paid: 10, note: 'rune' }, { id: 'w_b', defId: 'i_blaster', qty: 1, paid: 450 }, { id: 'w_c', qty: 1, paid: 2, def: { name: 'Mine', stats: { Acc: 3 } } }];
        const rawH = { v: 1, name: 'H', rolls: [], fields: [{ id: 'f_sc', key: 'Secret', label: 'Secret', kind: 'item-list', vis: 'gm', list: { stats: [{ key: 'heat' }] } }, { id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', edit: 'owner', vis: 'all', list: { multi: true, price: 'Cost', stats: [{ key: 'Heat', show: true }, { key: 'Cost' }] } }, { id: 'f_gr', key: 'Gear', label: 'Gear', kind: 'item-list', edit: 'owner', vis: 'all', list: { multi: true, price: 'cost', stats: [{ key: 'cost' }] } }],
            items: [{ id: 'i_h', name: 'Hot', stats: { Heat: 7, Cost: 20 } }, { id: 'i_g', name: 'Cheap', stats: { cost: 3 } }] };
        const sysH = cleanSystem(rawH, { F, gmView: true }), pvH = cleanSystem(sysH, { F, gmView: false }), fH = (s, id) => s.fields.find(f => f.id === id), iH = (s, id) => s.items.find(i => i.id === id);
        const rawV = cleanSystem(rawH, { F, gmView: false }), hGr = fH(sysH, 'f_gr').list;
        check('F4c1 (critic 2): one spelling per stat key per system — a later list\'s Heat where a GM-only list says heat becomes heat, Gear\'s cost (and its price) becomes Weapons\' Cost, entries follow; the GM\'s and the owner\'s rowStat, rowPaid and a player\'s add agree; both views are fixed points (and a players\' view made from the raw file reads the same numbers)',
            fH(sysH, 'f_wp').list.stats[0].key === 'heat' && j(hGr.stats) === j([{ key: 'Cost', label: 'cost' }]) && hGr.price === 'Cost' && j(iH(sysH, 'i_h').stats) === j({ heat: 7, Cost: 20 }) && j(iH(sysH, 'i_g').stats) === j({ Cost: 3 })
            && S.rowStat(fH(sysH, 'f_wp').list, iH(sysH, 'i_h'), 'heat') === 7 && S.rowStat(fH(pvH, 'f_wp').list, iH(pvH, 'i_h'), fH(pvH, 'f_wp').list.stats[0].key) === 7 && S.rowStat(fH(rawV, 'f_wp').list, iH(rawV, 'i_h'), fH(rawV, 'f_wp').list.stats[0].key) === 7
            && S.rowPaid(hGr, {}, iH(sysH, 'i_g')) === 3 && S.rowPaid(fH(pvH, 'f_gr').list, {}, iH(pvH, 'i_g')) === 3 && S.rowPaid(fH(rawV, 'f_gr').list, {}, iH(rawV, 'i_g')) === 3
            && agree(sysH, 'f_gr', { op: 'add', defId: 'i_g', rowId: 'w_n' }, []) === 'ok' && agree(sysH, 'f_wp', { op: 'add', defId: 'i_h', rowId: 'w_n' }, []) === 'ok'
            && j(cleanSystem(sysH, { F, gmView: true })) === j(sysH) && j(cleanSystem(pvH, { F, gmView: false })) === j(pvH) && j(cleanSystem(rawV, { F, gmView: false })) === j(rawV), j([fH(sysH, 'f_wp').list, hGr, sysH.items, fH(rawV, 'f_wp').list]));
        const ag = [agree(sysS, 'f_wp', { op: 'add', defId: 'i_blaster', rowId: 'w_n' }, seedA), agree(sysS, 'f_wp', { op: 'add', defId: 'i_knife', rowId: 'w_n' }, seedA), agree(sysS, 'f_wp', { op: 'add', defId: 'i_bare', rowId: 'w_n' }, []), agree(sysS, 'f_wp', { op: 'add', defId: 'i_blaster' }, seedA),
            agree(sysS, 'f_wp', { op: 'set', rowId: 'w_b', facts: { paid: 1 } }, seedA), agree(sysS, 'f_wp', { op: 'set', rowId: 'w_b', facts: { paid: null, note: 'n' } }, seedA), agree(sysS, 'f_wp', { op: 'set', rowId: 'w_b', facts: { note: 'n' } }, seedA), agree(sysS, 'f_wp', { op: 'add', defId: 'i_rune', rowId: 'w_n' }, seedA)];
        check('F4c1 (critic 15): the host and the player\'s client agree on each owner op — an add records the same paid (Blaster, Knife, an entry with no price stat, a merged add), a player\'s paid is "field" on both, a note alone goes through, a GM-only item is "missing" on both; the host\'s answer, projected, is the client\'s own',
            j(ag) === j(['ok', 'ok', 'ok', 'ok', 'field', 'field', 'ok', 'missing']), j(ag));

        const chO = { id: 'c_o', name: 'O', ownerId: 'u_o', npc: false, values: { f_wp: [{ id: 'w_b', defId: 'i_blaster', qty: 1, on: true, paid: 450 }, { id: 'w_r', defId: 'i_rune', qty: 1 }, { id: 'w_c', qty: 1, def: { name: 'Mine', damage: '1d6', stats: { Acc: 3, Cost: 1 } } }, { id: 'w_h', defId: 'i_rune', qty: 1, hid: 1 }], f_pl: [{ id: 'w_p', defId: 'i_rune', qty: 1 }] } };
        const cvS = S.charFor(chO, pvS, 'u_o', { items: libS }).values, pWp = cvS.f_wp, prb = S.charFor(chO, pvS, 'u_o', { probe: true }).values;
        check('F4c1 the owner\'s copy: a pointer carries what was paid; a GM-only item inline carries its list\'s stats only (Rune: Acc and Cost — no Heat, no damage, cost, key, area or removal rule); a custom copy its list\'s stats, no damage; a kept curse nothing; on a list without stats an inline copy carries no stats (charFor passes f.list || null: critic 1); the owner\'s re-clean is a fixed point; the probe passes values through as given',
            j(pWp[0]) === j({ id: 'w_b', defId: 'i_blaster', qty: 1, on: true, paid: 450 }) && j(pWp[1].def.stats) === j({ Acc: 5, Cost: 10 }) && !/Heat|2d6|"key"|"area"|bound|"rm"/.test(j(pWp[1])) && pWp[1].lnk === 1
            && j(pWp[2].def.stats) === j({ Acc: 3, Cost: 1 }) && !('damage' in pWp[2].def) && pWp.length === 3 && cvS.f_pl.length === 1 && cvS.f_pl[0].lnk === 1 && !('stats' in cvS.f_pl[0].def)
            && j(S.cleanValue(pS('f_wp'), pWp, S.valueOpts(pvS))) === j(pWp) && j(S.cleanValue(pS('f_pl'), cvS.f_pl, S.valueOpts(pvS))) === j(cvS.f_pl) && j(prb.f_wp) === j(chO.values.f_wp), j(cvS));
        ...
        check('F4c1 ownerSeesSame (the real one, sliced; critic 1): a GM change of a custom copy\'s stat is sent (compared with every stat), its damage is not; a change of what was paid is sent, one only to a kept curse is not',
            ...
            && OSS({}, sysS, hRow(1, 1), hRow(2, 1)) === false && OSS({}, sysS, hRow(1, 1), hRow(1, 9)) === true);
        const gmF = new Function('cleanRowDef', cutS('// [systemcheck:gmfacing-start]', '// [systemcheck:gmfacing-end]') + '\nreturn gmFacing;')(S.cleanRowDef), gf0 = gmF(sysS), runeOf = s => s.items.find(i => i.id === 'i_rune');
        const gfE = fn => { const s = JSON.parse(JSON.stringify(sysS)); fn(s); return gmF(s); }, upper = JSON.parse(JSON.stringify(sysS)); upper.fields[0].list.stats[0].key = 'ACC';
        check('F4c1 gmFacing: a GM-only item\'s stat of a visible list changes what its owner holds (a resend), matched ignoring case (critic 2); its stat of a GM-only list (Heat), its damage, cost or removal rule, and a visible item\'s stats do not',
            gfE(s => { runeOf(s).stats.Acc = 6; }) !== gf0 && gfE(s => { runeOf(s).stats.Cost = 11; }) !== gf0 && gfE(s => { runeOf(s).stats.Heat = 99; }) === gf0 && gfE(s => { runeOf(s).damage = '9d6'; runeOf(s).cost = '1'; runeOf(s).rm = 'curse'; }) === gf0
            && gfE(s => { s.items[0].stats.Acc = 9; }) === gf0 && (() => { const u2 = JSON.parse(JSON.stringify(upper)); runeOf(u2).stats.Acc = 6; return gmF(u2) !== gmF(upper); })());

        // the sheet: the real item widgets (sliced from sheets.js) on a fake DOM, live and inert (the Layout preview, a pop-out: _fxLive false)
        const fe4 = (tag, cls, text) => ({ tag, className: cls || '', textContent: text === undefined || text === null ? '' : String(text), children: [], title: '', type: '', value: '', checked: false, disabled: false, selected: false, dataset: {}, style: {}, classList: { add() {} }, on: {}, get childNodes() { return this.children; }, appendChild(x) { this.children.push(x); return x; }, insertBefore(x) { this.children.unshift(x); return x; }, addEventListener(k, f) { this.on[k] = f; } });
        const kids4 = n => (n.children || []), cl4 = n => kids4(n).map(k => String(k.className).split(' ').pop()), walk4 = (n, p, acc) => { if (p(n)) acc.push(n); kids4(n).forEach(k => walk4(k, p, acc)); return acc; }, find4 = (n, c) => walk4(n, x => (' ' + x.className + ' ').indexOf(' ' + c + ' ') >= 0, []);
        const commits4 = [], opt4 = (v, t, s) => { const o = fe4('option', null, t); o.value = v; if (s) o.selected = true; return o; }, doc4 = { createTextNode: t => ({ tag: '#text', className: '', textContent: t, children: [] }) };
        const itSrc4 = cutS('// ---- item-list widgets', '// 5h: an effect\'s changes as short text'), deps4 = ['el', 'iconNode', 'rowDef', 'rowIdOf', 'rowLvl', 'rowOn', 'commitItem', 'opt', 'fmtNum', 'iconText', 'LIMITS', '_fxLive', 'document', 'renderViews', 'closeHud', 'closeSheet', 'window', 'rowStat', 'rowPaid', '_fxView'];
        const mkW = live => new Function(...deps4, itSrc4 + '\nreturn { itemListInto: itemListInto, itemTableInto: itemTableInto, statFmt: statFmt, statText: statText };')(fe4, (v, cls) => fe4('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, (c, f, q) => commits4.push([f.id, q]), opt4, S.fmtNum, v => v || '', S.LIMITS, live, doc4, () => {}, () => {}, () => {}, {}, S.rowStat, S.rowPaid, 'sheet');
        const W4 = mkW(true), W4off = mkW(false), chW = { id: 'c_w', name: 'W', ownerId: 'u_w', values: {} }, rowsW = [{ id: 'w_b', defId: 'i_blaster', qty: 1, paid: 500 }];
        const gW = fe4('div'); W4.itemListInto(gW, fS('f_wp'), chW, rowsW, sysS, false, true, true);
        const gWK = fe4('div'); W4.itemListInto(gWK, fS('f_wp'), chW, rowsW.concat([{ id: 'w_k', defId: 'i_knife', qty: 1 }]), sysS, false, true, true);
        const r0 = gW.children[0], chipsW = find4(r0, 'sheet-item-stat'), stl = find4(gW.children[1], 'sheet-item-statline')[0], box = find4(r0, 'sheet-item-paid')[0], stlK = find4(gWK.children[3], 'sheet-item-statline')[0];
        check('F4c1 the sheet (the GM, a list with stats): the row reads name, a chip per stat shown On the row (a small label, then its value), the switch, the Paid box, 📝, ✎ (F4c2: the list is shaped), the quantity and ×; a category chip too while its rows span two categories; the 📝 line reads every stat (a value name for a named one) and what one cost (the list price where none was recorded); a plain list and an F4b list draw as before',
            j(cl4(r0)) === j(['sheet-item-name', 'sheet-item-stat', 'sheet-item-stat', 'sheet-item-onl', 'sheet-item-paidl', 'sheet-item-notes-t', 'sheet-item-edit', 'sheet-item-qty', 'sheet-item-rm']) && j(cl4(gWK.children[0])) === j(['sheet-item-name', 'sheet-chip', 'sheet-item-stat', 'sheet-item-stat', 'sheet-item-onl', 'sheet-item-paidl', 'sheet-item-notes-t', 'sheet-item-edit', 'sheet-item-qty', 'sheet-item-rm'])
            && j(chipsW.map(c => kids4(c).map(k => [k.tag, k.textContent]))) === j([[['small', 'Acc'], ['#text', ' 2']], [['small', 'Dmg'], ['#text', ' 3']]])
            && stl.textContent === 'Acc 2 · Dmg 3 · Cost 500 · Wt 0.0001 · diff E · Paid 500' && stlK.textContent === 'Acc 0 · Dmg 1 · Cost 99999999 · Wt 0 · diff E · Paid 99999999' && gW.children[1].children[0] === stl, j([cl4(r0), cl4(gWK.children[0]), stl && stl.textContent, stlK && stlK.textContent]));
        commits4.length = 0; const boxAttrs = j([box.tag, box.type, box.value, box.placeholder, box.min, box.step, box.dataset.fid, box.dataset.part]);
        box.value = '450'; box.on.change(); box.value = ''; box.on.change(); box.value = '-3'; box.on.change(); const back1 = box.value; box.value = '2e9'; box.on.change(); const back2 = box.value; box.value = '12.5'; box.on.change();
        check('F4c1 the GM\'s Paid box: a number box (min 0, any step) holding what was recorded, the list price shown faintly, the field and a per-row part (focus comes back after the redraw); a change sends one set op, a blank sends null (the list price), anything below 0 or past 1e9 puts the box back and sends nothing',
            boxAttrs === j(['input', 'number', '500', '500', '0', 'any', 'f_wp', 'paid-w_b']) && back1 === '500' && back2 === '500'
            && j(commits4) === j([['f_wp', { op: 'set', rowId: 'w_b', facts: { paid: 450 } }], ['f_wp', { op: 'set', rowId: 'w_b', facts: { paid: null } }], ['f_wp', { op: 'set', rowId: 'w_b', facts: { paid: 12.5 } }]]), j([boxAttrs, commits4]));
        commits4.length = 0; box.validity = { badInput: true }; box.value = ''; box.on.change(); const back3 = box.value; delete box.validity;
        check('F4c1 review: the GM\'s Paid box with text that is not a number (a number box reads it as blank, badInput) puts the box back and sends nothing — never the deliberate clear',
            back3 === '500' && commits4.length === 0, j([back3, commits4]));
        const oW = fe4('div'); W4.itemListInto(oW, fS('f_wp'), chW, [{ id: 'w_b', defId: 'i_blaster', qty: 1, paid: 450 }, { id: 'w_n', defId: 'i_blaster', qty: 1 }], sysS, false, true, false);
        const oc1 = find4(oW.children[0], 'sheet-item-paid')[0], oc2 = find4(oW.children[2], 'sheet-item-paid')[0], offW = fe4('div'); W4off.itemListInto(offW, fS('f_wp'), chW, rowsW, sysS, false, true, true);
        const offT = fe4('div'); W4off.itemTableInto(offT, Object.assign({}, fS('f_wp'), { table: { footer: true } }), chW, rowsW, sysS, false, true, true);
        check('F4c1 Paid for the owner, and wherever nothing can be changed: a chip — "Paid 450", titled with the list price, or the list price "Not recorded"; the GM\'s own inert copy (the Layout preview, a pop-out: critic 4) draws the chip, never a box, in a list or a table',
            oc1.tag === 'span' && oc1.textContent === 'Paid 450' && oc1.title === 'Price paid for one (list price 500)' && oc2.textContent === 'Paid 500' && oc2.title === 'Not recorded: the list price' && walk4(oW, x => x.tag === 'input' && /paid/.test(x.className), []).length === 0
            && find4(offW, 'sheet-item-paid').length === 1 && find4(offW, 'sheet-item-paid')[0].tag === 'span' && find4(offW, 'sheet-item-paid')[0].textContent === 'Paid 500' && walk4(offW, x => x.tag === 'input', []).length === 0 && walk4(offT, x => x.tag === 'input', []).length === 0 && find4(offT, 'sheet-item-paid')[0].textContent === '500',
            j([oc1, oc2].map(x => x && [x.tag, x.textContent, x.title])));
        const tFw = Object.assign({}, fS('f_wp'), { table: { footer: true } }), tW4 = fe4('div'); W4.itemTableInto(tW4, tFw, chW, rowsW, sysS, false, true, true);
        const tE4 = fe4('div'); W4.itemTableInto(tE4, tFw, chW, [], sysS, false, true, true);
        const th4 = kids4(kids4(kids4(tW4.children[0])[0])[0]).map(x => x.textContent), tr4 = kids4(kids4(tW4.children[0])[1]), row4 = kids4(tr4[0]), ft4 = kids4(kids4(kids4(tW4.children[0])[2])[0]);
        check('F4c1 the sheet\'s table: a column per stat shown On the row (after the item columns) and Paid after Qty (the GM\'s box, no caption), the 📝 row and an empty table spanning every column, the footer\'s first cell spanning the stats and an empty cell under Paid; a table without stats is as before',
            j(th4) === j(['Item', 'Acc', 'Dmg', 'Readied', 'Qty', 'Paid', '']) && j(row4.map(x => x.className)) === j(['sheet-itcol-name', 'sheet-itcol-stat', 'sheet-itcol-stat', 'sheet-itcol-on', 'sheet-itcol-qty', 'sheet-itcol-paid', 'sheet-itcol-act']) && row4[1].textContent === '2' && row4[2].textContent === '3'
            && kids4(row4[5]).length === 1 && kids4(row4[5])[0].tag === 'input' && kids4(row4[5])[0].dataset.part === 'paid-w_b' && tr4[1].className === 'sheet-itemtable-notes' && kids4(tr4[1])[0].colSpan === 7
            && j(ft4.map(x => [x.className, x.colSpan || 0])) === j([['sheet-itft', 4], ['sheet-itft-qty', 0], ['', 0], ['', 0]]) && kids4(kids4(kids4(tE4.children[0])[1])[0])[0].colSpan === 7, j([th4, row4.map(x => x.className), ft4.map(x => [x.className, x.colSpan])]));
        check('F4c1 the stat formatter: whole numbers as they are, two decimals from 1 up, three significant digits below (0.0001 lb stays 0.0001 where fmtNum reads 0); a value name for a whole value inside the names (a blank name, or a value past them, shows the number); the widgets never use innerHTML',
            j([0.0001, 1234567.891, -0.5, 12.5, 99999999, 1 / 3, 2 / 3, 0, -2, 1e9].map(W4.statFmt)) === j(['0.0001', '1234567.89', '-0.5', '12.5', '99999999', '0.333', '0.667', '0', '-2', '1000000000']) && W4.statFmt(NaN) === '' && W4.statFmt('3') === '' && S.fmtNum(0.0001) === '0'
            && W4.statText({ labels: ['E', 'A', 'H', 'VH'] }, 2) === 'H' && W4.statText({ labels: ['E', 'A'] }, 7) === '7' && W4.statText({ labels: ['E', 'A'] }, 0.5) === '0.5' && W4.statText({ labels: ['', 'A'] }, 0) === '0' && W4.statText({}, 3) === '3'
            && !/innerHTML/.test(itSrc4));

        // the System editor: the Lists card's stats (real), an item's stat boxes (real), the handlers (pinned), the live messages (real)
        const inp4 = (cls, v, t, ph) => { const i = fe4('input', cls); i.type = 'text'; i.value = v == null ? '' : String(v); i.title = t || ''; i.placeholder = ph || ''; return i; };
        const realN4 = new Function('el', cutS('function numInput(', 'function select(') + '\nreturn { numInput: numInput, numField: numField };')(fe4), ni4 = realN4.numInput, nf4 = realN4.numField;   // the real ones (review: a step of their own, 'any')
        const LC4 = new Function('el', 'input', 'numField', 'document', 'LIMITS', cutS('function listCard(', 'function renderCombat(') + '\nreturn listCard;')(fe4, inp4, nf4, doc4, S.LIMITS);
        const cardS = LC4(JSON.parse(JSON.stringify(rawS.fields[0])), ['Gun', 'Blade', 'Skill']), statRows = find4(cardS, 'sys-list-stat'), keys4 = find4(cardS, 'sys-list-statkey'), priceSel = find4(cardS, 'sys-list-price')[0];
        const ten = LC4({ id: 'f_t', key: 'T', label: 'T', kind: 'item-list', list: { stats: Array.from({ length: 10 }, (_, i) => ({ key: 'S' + i })) } }, []), none4 = LC4({ id: 'f_n', key: 'N', label: 'N', kind: 'item-list' }, []);
        check('F4c1 the Lists card (run for real): a row per stat (key up to 24, label, a default taking any number, value names, On the row, ▲ ▼ ×), + Stat (greyed at ten), and once there are stats a Price of No price or each stat (the list\'s price chosen, ignoring case); a list without stats: + Stat alone',
            statRows.length === 5 && j(statRows.map(r => r.dataset.si)) === j(['0', '1', '2', '3', '4']) && j(keys4.map(k => k.value)) === j(['Acc', 'Dmg', 'Cost', 'Wt', 'diff']) && keys4.every(k => k.maxLength === 24)
            && find4(cardS, 'sys-list-statdef')[1].value === '1' && find4(cardS, 'sys-list-statdef').every(x => x.step === 'any') && find4(cardS, 'sys-list-statnames')[4].value === 'E, A, H, VH' && j(find4(cardS, 'sys-list-statshow').map(x => x.checked)) === j([true, true, false, false, false])
            && statRows.every(r => j(kids4(r).filter(x => x.dataset && x.dataset.act).map(x => x.dataset.act)) === j(['statup', 'statdown', 'statdel'])) && find4(cardS, 'sys-list-statadd')[0].disabled === false && find4(ten, 'sys-list-statadd')[0].disabled === true
        ...
        check('F4c1 the Items tab (run for real): an item gets a box for each stat of the lists it can be on (its category, or a list naming none), once a key ignoring case (the first list\'s), in list order; a dropdown for a stat with names (a stored value past them kept, greyed); boxes take any number; each names its key; a value no such list has is drawn after, marked, to clear (never a prototype name); no stat line without stats; the category carries its scope (a change redraws the row)',
            !!sbB && j(capsB) === j(['Acc', 'Dmg', 'Cost', 'Wt', 'diff', 'Heat', 'Extra', 'Rel (not on its lists)', 'Zzz (not on its lists)']) && j(ctlB.map(x => x.dataset.sk)) === j(['Acc', 'Dmg', 'Cost', 'Wt', 'diff', 'Heat', 'Extra', 'Rel', 'Zzz'])
            && j(ctlB.map(x => x.value)) === j(['2', '3', '500', '0.0001', '', '7', '', '1', '4']) && ctlB.filter(x => x.tag === 'input').every(x => x.step === 'any' && x.type === 'number') && ctlB[4].tag === 'select' && j(kids4(ctlB[4]).map(o => o.value)) === j(['', '0', '1', '2', '3'])
            && !!selD && j(kids4(selD).map(o => [o.value, !!o.selected, !!o.disabled])) === j([['', false, false], ['0', false, false], ['1', false, false], ['2', false, false], ['3', false, false], ['7', true, true]])
            && find4(rowB, 'sys-item-cat')[0].dataset.scope === 'acc,dmg,cost,wt,diff,heat,extra' && find4(mkIR(drR).itemRow(drR.items[5]), 'sys-item-stats').length === 0 && j(IR.itemStatDefs(drI.items[3]).map(s => s.key)) === j(['Rel', 'acc', 'Extra']), j([capsB, ctlB.map(x => [x.dataset.sk, x.value])]));
        check('F4c1 the editor\'s handlers: a stat\'s key, label, default and names write the draft (the price follows its key; the Price choices follow as it is typed); On the row and Price; + Stat (at most ten), ▲ ▼ and × (a removed stat takes the price with it, the last one takes the stats key); an item\'s stat box writes a number under its key, checked first (never a prototype name), blank takes it away; a category that changes the item\'s lists redraws its row; each list\'s messages under its card',
            /if \(lcc\.indexOf\('sys-list-statkey'\) >= 0\) statKeyInput\(lsp, stL, sti, t\.value\);/.test(shS) && /numOr = function\(o, k\) \{ if \(t\.validity && t\.validity\.badInput\) return;/.test(shS) && /\(sk\.toLowerCase\(\) in Object\.prototype\)\) return; if \(t\.validity && t\.validity\.badInput\) return;/.test(shS) && /        else return;\n        _priceAt\.delete\(lsb\);\n/.test(shS)
            && /else if \(lcc\.indexOf\('sys-list-statdef'\) >= 0\) numOr\(stD, 'def'\);/.test(shS) && /if \(pSel\) priceOptions\(pSel, lsp\);/.test(shS)
            && /else if \(c\.indexOf\('sys-list-statshow'\) >= 0\) \{[^\n]*if \(t\.checked\) ssD\.show = true; else delete ssD\.show; \}/.test(shS) && /else if \(c\.indexOf\('sys-list-price'\) >= 0\) \{ _priceAt\.delete\(lsc\); if \(t\.value\) lsc\.price = t\.value; else delete lsc\.price; \}/.test(shS)
            && /if \(sact === 'statadd'\) \{ if \(sArr\.length >= LIMITS\.listStats\) \{ toast\('At most ' \+ LIMITS\.listStats \+ ' stats a list\.'\); return; \}/.test(shS)
            && /else if \(sact === 'statdel' && sI >= 0 && sI < sArr\.length\) \{ var gone = sArr\.splice\(sI, 1\)\[0\]; if \(gone && typeof gone\.key === 'string' && typeof lsb\.price === 'string' && lsb\.price\.toLowerCase\(\) === gone\.key\.toLowerCase\(\)\) delete lsb\.price; \}/.test(shS) && /if \(!sArr\.length\) \{ delete lsb\.stats; delete lsb\.price; \}/.test(shS)
            && /else if \(ic\.indexOf\('sys-item-stat'\) >= 0\) \{ var sk = t\.dataset\.sk \|\| ''; if \(!STAT_KEY\.test\(sk\) \|\| \(sk in Object\.prototype\) \|\| \(sk\.toLowerCase\(\) in Object\.prototype\)\) return;/.test(shS) && /if \(sv === ''\) delete ist\[sk\]; else if \(t\.dataset\.pick === '1'\) ist\[sk\] = sv; else \{ var sn = Number\(sv\); if \(!isFinite\(sn\)\) return; ist\[sk\] = sn; \}/.test(shS)
            && /if \(c\.indexOf\('sys-item-cat'\) >= 0\) \{ var nsc = itemStatDefs\(iit\)\.map\(function\(s\) \{ return s\.key\.toLowerCase\(\); \}\)\.join\(','\); if \(nsc !== \(t\.dataset\.scope \|\| ''\)\) \{ markDirty\(\); renderAll\(\); \} return; \}/.test(shS)
            && /var le = errorCell\('list:' \+ f\.id\); le\.dataset\.errFor = 'list:' \+ f\.id; box\.appendChild\(le\);/.test(shS));
        const reSrc = cutS('function refreshErrors(', 'function errorCell('), reDeps = ['draft', 'F', 'validKey', 'KINDS', 'cleanItemKey', 'cleanSystem', 'validateSystem', 'statKey', 'LIMITS', 'getActiveCampaign', 'systemOf'];
        const runRE = (dr, cp) => new Function(...reDeps, 'var errorsById = {}, warningsById = {};\n' + reSrc + '\nrefreshErrors(); return { errorsById: errorsById, warningsById: warningsById };')(dr, () => F, S.validKey, S.KINDS, S.cleanItemKey, cleanSystem, validateSystem, S.statKey, S.LIMITS, () => cp, c => c && c.system && typeof c.system === 'object' ? c.system : null);
        const drE = { v: 1, name: 'E', rolls: [], fields: [
            { id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', edit: 'owner', vis: 'all', list: { price: 'nope', stats: [{ key: 'Acc' }, { key: '' }, { key: 'max' }, { key: 'acc' }, { key: 'Wt' }] } },
            { id: 'f_gr', key: 'Gear', label: 'Gear', kind: 'item-list', edit: 'owner', vis: 'all', list: { stats: [{ key: 'wt' }, { key: 'Heat' }] } },
            { id: 'f_bg', key: 'Big', label: 'Big', kind: 'item-list', edit: 'owner', vis: 'all', list: { stats: Array.from({ length: 11 }, (_, i) => ({ key: 'B' + i })) } },
            { id: 'f_sc', key: 'Secret', label: 'Secret', kind: 'item-list', edit: 'owner', vis: 'gm', list: { stats: [{ key: 'Heat' }] } }],
            items: [{ id: 'i_a', name: 'A', stats: { Dmg: 3 } }, { id: 'i_b', name: 'B', stats: { dmg: 1, Cost: 5 } }, { id: 'i_c', name: 'C' }] };
        const savedE = cleanSystem({ v: 1, name: 'E', rolls: [], items: [], fields: [
            { id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', edit: 'owner', vis: 'all', list: { stats: [{ key: 'Acc' }, { key: 'Dmg' }, { key: 'Wt' }, { key: 'Cost' }] } },
            { id: 'f_gr', key: 'Gear', label: 'Gear', kind: 'item-list', edit: 'owner', vis: 'all', list: { stats: [{ key: 'Cost' }] } }] }, { F, gmView: true });
        const campE = { system: savedE, chars: { c_1: { id: 'c_1', values: { f_wp: [{ id: 'w_c', qty: 1, def: { name: 'M', stats: { Dmg: 4 } } }] } }, c_2: { id: 'c_2', values: { f_wp: [{ id: 'w_d', qty: 1, def: { name: 'N', stats: { dmg: 2 } } }, { id: 'w_e', qty: 1, def: { name: 'O', stats: { Cost: 1 } } }], f_gr: [{ id: 'w_f', qty: 1, def: { name: 'P', stats: { Cost: 2 } } }] } }, c_3: { id: 'c_3', values: { f_wp: [{ id: 'w_g', defId: 'i_a', qty: 1 }, { id: 'w_i', qty: 1, def: { name: 'I', stats: { Dmg: 1 } }, lnk: 1 }] } } } };
        const RE = runRE(drE, campE), msgOf = id => (RE.errorsById['list:' + id] || []).map(e => e.message), NOKEY = 'Stat key "max" is not usable: a letter, then letters, digits and _ (up to 24, no spaces); not a word formulas already use (count, qty, on, has, lvl, paid, row; max, cur, ranks, base; a function or reserved word such as floor, and, true; constructor, toString). Save drops it.';
        check('F4c1 the live messages (refreshErrors, run for real): under each list\'s card — a stat with no key, a key Save drops (every reason named), a repeat ignoring case, past ten, a price naming no stat, another list\'s spelling of the same key; and, before a Save that drops them, the values a renamed or removed stat takes with it — the items\' (when no list keeps the key) and the characters\' own copies\' (critic 8; an inline copy is not theirs); the validator\'s shared-key warning under the GM-only list\'s card',
            j(msgOf('f_wp')) === j(['Stat 2 needs a key: Save drops it.', NOKEY, 'Stat key "acc" repeats "Acc" (keys ignore case): Save keeps the first.', 'The price names no stat of this list: Save drops it.', 'Save drops the “Dmg” values of 2 items and the own values of 2 characters’ copies (a stat’s key is its identity).', 'Save drops the “Cost” values of 1 item and the own values of 1 character’s copy (a stat’s key is its identity).'])
            && j(msgOf('f_gr')) === j(['Stat "wt" is spelled "Wt" on Weapons: Save uses that spelling here too (one key, one spelling).', 'Save drops the “Cost” values of 1 character’s copy (a stat’s key is its identity).']) && j(msgOf('f_bg')) === j(['At most 10 stats a list: Save keeps the first 10.'])
            && j((RE.warningsById['list:f_sc'] || []).map(w => w.message)) === j(['Stat “Heat” is also on Gear: an item’s Heat reaches players.']) && !RE.warningsById.f_sc && msgOf('f_sc').length === 0, j([msgOf('f_wp'), msgOf('f_gr'), msgOf('f_bg'), RE.warningsById]));
        const vW = validateSystem(cleanSystem({ v: 1, name: 'V', rolls: [], items: [], fields: [{ id: 'f_sc', key: 'Secret', label: 'Secret', kind: 'item-list', vis: 'gm', list: { stats: [{ key: 'Heat' }, { key: 'Glow' }] } }, { id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', vis: 'all', list: { stats: [{ key: 'Heat' }] } }, { id: 'f_s2', key: 'Secret2', label: 'Secret2', kind: 'item-list', vis: 'gm', list: { stats: [{ key: 'Glow' }] } }] }, { F, gmView: true }), F);
        check('F4c1 the validator: a stat key a GM-only item list shares with a visible one warns on the GM-only list (an item\'s value under it reaches players); two GM-only lists sharing one do not',
            j(vW.warnings.filter(w => w.prop === 'list')) === j([{ id: 'f_sc', prop: 'list', message: 'Stat “Heat” is also on Weapons: an item’s Heat reaches players.' }]) && vW.ok, j(vW));

        // F4c1 review: the price follows its own stat's key only (statKeyInput, run for real, keystroke by keystroke)
        const PK = new Function(cutS('var _priceAt = new WeakMap();', 'function priceOptions(') + '\nreturn statKeyInput;')();
        const typeK = (lsp, sti, word) => { for (let i = 1; i <= word.length; i++) PK(lsp, lsp.stats, sti, word.slice(0, i)); return lsp; };
        const pk1 = typeK({ price: 'Cost', stats: [{ key: 'Acc' }, { key: 'Cost' }, { key: '' }] }, 2, 'Costume'), pk2 = typeK({ price: 'cr', stats: [{ key: 'wt' }, { key: 'cr' }, { key: 'wl' }, { key: '' }] }, 3, 'crit');
        const pk3 = { price: 'Cost', stats: [{ key: 'Acc' }, { key: 'Cost' }] }; PK(pk3, pk3.stats, 1, 'Cos'); const pk3a = pk3.price; PK(pk3, pk3.stats, 1, ''); const pk3b = pk3.price; typeK(pk3, 1, 'Price');
        const pk4 = typeK({ stats: [{ key: '' }, { key: '' }] }, 0, 'xp'), pk5 = typeK({ price: '', stats: [{ key: '' }, { key: 'Acc' }] }, 0, 'xp'), pk6 = { price: 'cost', stats: [{ key: 'wt' }, { key: 'cost' }, { key: 'costMult' }] };
        PK(pk6, pk6.stats, 2, 'cost'); PK(pk6, pk6.stats, 2, 'cos');
        check('F4c1 review: a stat key typed through the price\'s spelling (Costume past Cost, crit past cr, costMult backspaced past cost) leaves the price on its own stat; renaming the price stat itself carries it (cleared for a moment to retype: blank, then the new key); with no price, or a blank one nobody cleared, typing sets none',
            pk1.price === 'Cost' && pk1.stats[2].key === 'Costume' && pk2.price === 'cr' && pk3a === 'Cos' && pk3b === '' && pk3.price === 'Price' && !('price' in pk4) && pk5.price === '' && pk6.price === 'cost' && pk6.stats[2].key === 'cos', j([pk1, pk2, pk3, pk4, pk5, pk6]));
        // F4c1 review: what Save drops with no word otherwise — a list deleted or no longer a list (its items told on their rows), an item's value past
        // ±1e9 and past 16 an item, a stat default past ±1e9
        const savedX = cleanSystem({ v: 1, name: 'X', rolls: [], items: [], fields: [{ id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', vis: 'all', list: { stats: [{ key: 'Acc' }] } }, { id: 'f_sc', key: 'Secret', label: 'Secret', kind: 'item-list', vis: 'gm', list: { stats: [{ key: 'Heat' }] } }, { id: 'f_kd', key: 'Kind', label: 'Kind', kind: 'item-list', vis: 'all', list: { stats: [{ key: 'Glow' }, { key: 'Acc' }] } }] }, { F, gmView: true });
        const statsO = { Acc: 3e9 }; for (let i = 0; i < 8; i++) statsO['S' + i] = 1; for (let i = 0; i < 10; i++) statsO['T' + i] = 1;
        const drX = { v: 1, name: 'X', rolls: [], fields: [{ id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', vis: 'all', list: { stats: [{ key: 'Acc' }, { key: 'Big', def: 2e9 }].concat(Array.from({ length: 8 }, (_, i) => ({ key: 'S' + i }))) } }, { id: 'f_w2', key: 'W2', label: 'W2', kind: 'item-list', vis: 'all', list: { stats: Array.from({ length: 10 }, (_, i) => ({ key: 'T' + i })) } }, { id: 'f_kd', key: 'Kind', label: 'Kind', kind: 'text' }],
            items: [{ id: 'i_b', name: 'B', stats: { Acc: 2, Heat: 7, Glow: 1 } }, { id: 'i_o', name: 'O', stats: statsO }, { id: 'i_k', name: 'K', stats: { Acc: 1 } }] };
        const REX = runRE(drX, { system: savedX, chars: {} }), eX = id => (REX.errorsById[id] || []).map(e => e.prop + ': ' + e.message), keptX = cleanSystem(drX, { F, gmView: true }).items.map(it => Object.keys(it.stats || {}));
        check('F4c1 review: Save drops nothing of an item\'s stats with no word — a list deleted (Secret) or turned into another kind (Kind) tells each item holding its stat, on the item\'s row, unless another list still has the key (Acc); a value past ±1,000,000,000 and the values past 16 an item (the first come are kept, as Save keeps them) are named; a stat default past ±1e9 on its list\'s card; an item Save keeps whole says nothing',
            j(eX('i_b')) === j(['stats: Save drops its “Heat” value: no list has that stat now (a stat’s key is its identity).', 'stats: Save drops its “Glow” value: no list has that stat now (a stat’s key is its identity).'])
            && j(eX('i_o')) === j(['stats: “Acc” is past ±1,000,000,000: Save drops it.', 'stats: At most 16 stats an item: Save drops “T8”, “T9” (the first 16 are kept; clear the ones it does not need).']) && j(keptX[1]) === j(['S0', 'S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'T0', 'T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'])
            && j(eX('list:f_wp')) === j(['undefined: Stat "Big" has a default past ±1,000,000,000: Save drops it.']) && eX('i_k').length === 0 && eX('list:f_w2').length === 0, j([REX.errorsById, keptX]));
        // F4c1 review: a stat key the packer takes for a typed array, and one spelling a key in a copy's own stats
        const sAB = S.cleanRowDef({ name: 'X', stats: { cost: 7, Cost: 5, acc: 1 } }, true), specAB = { stats: [{ key: 'Cost' }, { key: 'Acc' }], price: 'Cost' };
        check('F4c1 review: BYTES_PER_ELEMENT is never a stat key, in any case (an object holding it packs as an empty typed array: every stat of the item gone for players); a copy\'s own stats keep one spelling a key (the first: cost 7, not Cost 5), so the GM\'s read and its owner\'s projection agree',
            SK('BYTES_PER_ELEMENT') === '' && SK('bytes_per_element') === '' && S.statKey('Bytes_Per_Element') === '' && SK('Bytes') === 'Bytes' && LS({ stats: [{ key: 'BYTES_PER_ELEMENT' }, { key: 'Acc' }] }, true, F).stats.length === 1
            && j(sAB.stats) === j({ cost: 7, acc: 1 }) && S.rowStat(specAB, sAB, 'Cost') === 7 && j(S.rowStats(sAB.stats, specAB)) === j({ Cost: 7, Acc: 1 }) && S.rowPaid(specAB, {}, sAB) === 7, j([sAB, S.rowStats(sAB.stats, specAB)]));
        const fxR4 = n => JSON.parse(fs.readFileSync(path.join(__dirname, 'fixtures', n + '.json'), 'utf8'));
        const fxOk4 = ['rows-d20', 'rows-3d6'].map(n => { const raw = fxR4(n), g1 = cleanSystem(raw, { F, gmView: true }), g2 = cleanSystem(g1, { F, gmView: true }), p1 = cleanSystem(g1, { F, gmView: false }), p2 = cleanSystem(p1, { F, gmView: false }), v = validateSystem(g1, F); return [n, j(g1) === j(raw) && j(g2) === j(g1) && j(p2) === j(p1) && v.errors.length === 0 && !/Heat|Vorpal|Hidden/.test(j(p1)) && g1.fields.some(f => f.list && f.list.stats && f.list.price)]; });
        const adv4 = fxR4('rows-3d6').fields.find(f => f.key === 'Advantages');
        check('F4c1 the rows test systems (tools/fixtures, a generic d20 and a generic 3d6): each is its own GM-clean form, both views are fixed points, they validate with no errors, and the players\' view carries no GM-only list, item or key (Heat, Vorpal, Hidden); an Advantages stat keyed base is refused, so it is cpBase',
            fxOk4.every(x => x[1]) && adv4.list.stats[0].key === 'cpBase' && S.cleanListSpec({ stats: [{ key: 'base' }] }, true, F) === null, j(fxOk4));

        const tut4 = fs.readFileSync(path.join(app, 'scripts', 'tutorial.js'), 'utf8'), html4 = fs.readFileSync(path.join(app, 'index.html'), 'utf8');
        check('F4c1 tour and Help: the Items step (a box per stat), the Lists step (ten stats, On the row, the price and Paid, the key is the identity; F5a1: formulas read them, Columns) and the sheet step (stats and what was paid); Help\'s Items and Lists lines; the Lists tab\'s title and both toolbar notes',
            /Once a list has <b>stats<\/b>, an item it can be on gets a box for each \(Acc, Wt, Cost&hellip;\); formulas read them \(Weapons\.Wt totals a list, Weapons\.Blaster\.Acc reads one row\)\./.test(tut4) && /A list can carry up to ten <b>stats<\/b>[^']*a <b>price<\/b> stat: a row records what one cost when it is added \(<b>Paid<\/b>, yours to correct on the row\)\. A stat&rsquo;s key is its identity[^']*<b>Columns<\/b> work a formula out for each row/.test(tut4) && /Rows show their stats and what was paid\./.test(tut4)
            && /an item gets a box for each stat of the lists it can be on/.test(html4) && /A list can also carry up to ten <b>stats<\/b>/.test(html4) && /One stat can be the list&rsquo;s <b>price<\/b>: a row records it as <b>Paid<\/b> when it is added/.test(html4) && /Formulas read them: a list&rsquo;s totals/.test(html4) && /A list&rsquo;s <b>Columns<\/b> work a formula out for each row/.test(html4)
            && /no quantity, its stats and a price">Lists/.test(html4) && /An item in a list with <b>stats<\/b> gets a box for each\./.test(html4) && /its <b>stats<\/b> \(numbers each of its items carries\) and a <b>price<\/b>\./.test(html4));

        // critic 3: a slice test injects what it needs, so it cannot see a name sheets.js never imported — every injected name must be declared in
        ...
        check('F4c1 (critic 3): every name a sheets.js slice runs with is declared in sheets.js or imported by it, every name it imports from systemcore is exported there, and the new reads are in both the export list and the window API',
            missing4.length === 0 && scImp.length > 60 && notExp.length === 0 && notApi.length === 0 && ['cleanListSpec', 'STAT_KEY', 'statKey', 'rowStat', 'rowPaid', 'itemReach'].every(n => scImp.indexOf(n) >= 0), j({ missing4, notExp, notApi }));   // F4c2: itemReach (saveDraft) too
    }

    /* ---- Stage 6 F4c2: a copy's own values, Setting A, the reach ---- */
    {
        const C7 = String.fromCharCode(7), rawT = { v: 1, name: 'T', rolls: [], listRules: { ownerStats: true }, fields: [
            { id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', edit: 'owner', vis: 'all', list: { multi: true, on: { label: 'Readied' }, cats: ['Gun', 'Blade'], price: 'Cost', stats: [{ key: 'Acc', show: true }, { key: 'Dmg', def: 1, show: true }, { key: 'Cost' }, { key: 'Wt' }, { key: 'diff', labels: ['E', 'A', 'H', 'VH'] }] } },
            { id: 'f_sk', key: 'Skills', label: 'Skills', kind: 'item-list', edit: 'owner', vis: 'all', list: { noQty: true, cats: ['Skill'], stats: [{ key: 'Rel' }] } },
            { id: 'f_gr', key: 'Gear', label: 'Gear', kind: 'item-list', edit: 'owner', vis: 'all', list: { on: { label: 'Worn' } } },
            { id: 'f_sc', key: 'Secret', label: 'Secret', kind: 'item-list', edit: 'owner', vis: 'gm', list: { cats: ['Gun'], stats: [{ key: 'Heat' }] } },
            { id: 'f_pl', key: 'Plain', label: 'Plain', kind: 'item-list', edit: 'owner', vis: 'all' }],
            items: [
            { id: 'i_blaster', name: 'Blaster', category: 'Gun', damage: '2d6', cost: '5', area: { ft: 10, name: 'Burst' }, stats: { Acc: 2, Dmg: 3, Cost: 500, Wt: 0.0001 } },
            { id: 'i_chain', name: 'Chain', category: 'Gun', rm: 'bound', rmMsg: 'It holds', eq: 'bound', eqMsg: 'Still on' },
            { id: 'i_rune', name: 'Rune', category: 'Gun', vis: 'gm', damage: '2d6', rm: 'bound', stats: { Acc: 5, Cost: 10, Heat: 9 } },
            { id: 'i_knife', name: 'Knife', category: 'Blade', stats: { Cost: 20 } },
            { id: 'i_sneak', name: 'Sneak', category: 'Skill', stats: { Rel: -2 } }] };
        const mk = rules => { const r = JSON.parse(j(rawT)); if (rules === undefined) delete r.listRules; else r.listRules = rules; const g = cleanSystem(r, { F, gmView: true }); return { g, p: cleanSystem(g, { F, gmView: false }) }; };
        const T1 = mk({ ownerStats: true }), T0 = mk(undefined), sysT = T1.g, pvT = T1.p, sysO = T0.g, pvO = T0.p;
        const fT = id => sysT.fields.find(f => f.id === id), pT = id => pvT.fields.find(f => f.id === id), iT = id => sysT.items.find(i => i.id === id), wpT = fT('f_wp').list, voT = S.valueOpts(sysT), libT = {}; sysT.items.forEach(i => { libT[i.id] = i; });
        const clT = rows => cleanValue(fT('f_wp'), JSON.parse(j(rows)), voT), rowOf = (v, id) => (v || []).find(r => r && r.id === id);
        const seedT = () => [{ id: 'w_b1', defId: 'i_blaster', qty: 1 }, { id: 'w_b2', defId: 'i_blaster', qty: 2, on: true },
            { id: 'w_b3', defId: 'i_blaster', qty: 1, ov: { name: 'Ahto Blaster', stats: { Dmg: 5, Wt: 1 }, held: ['Dmg'], damage: '9d6', rm: 'bound', rmMsg: 'Stays mine' } },
            { id: 'w_r', defId: 'i_rune', qty: 1 }, { id: 'w_l', defId: 'i_gone', qty: 1, snap: { name: 'Old gun', stats: { Acc: 1 }, rm: 'bound' } },
            { id: 'w_c', qty: 1, def: { name: 'Mine', stats: { Acc: 3 } } }, { id: 'w_h', defId: 'i_blaster', qty: 1, hid: 1 }, { id: 'w_ch', defId: 'i_chain', qty: 1, on: true }];

        const lrOf = v => { const s = mk(v); return [s.g.listRules, s.p.listRules, 'listRules' in s.g, 'listRules' in s.p]; };
        check('F4c2 cleanListRules (Setting A): only { ownerStats: true } (false, a string, {} or nothing: absent), in both views (a player\'s sheet and their own check read it), the last key of the system; both views fixed points; a system without it and emptySystem carry none',
            j(S.cleanListRules({ ownerStats: true, junk: 1 })) === j({ ownerStats: true }) && [false, 'yes', 1, null, undefined].every(x => S.cleanListRules({ ownerStats: x }) === null) && S.cleanListRules({}) === null && S.cleanListRules('x') === null && S.cleanListRules([]) === null
            && j(lrOf({ ownerStats: true })) === j([{ ownerStats: true }, { ownerStats: true }, true, true]) && [{ ownerStats: false }, { ownerStats: 'yes' }, {}, undefined].every(v => j(lrOf(v)) === j([undefined, undefined, false, false]))
            && j(cleanSystem(sysT, { F, gmView: true })) === j(sysT) && j(cleanSystem(pvT, { F, gmView: false })) === j(pvT) && Object.keys(sysT).slice(-1)[0] === 'listRules' && !('listRules' in S.emptySystem()) && !('listRules' in sysO) && !('listRules' in pvO),
            j([lrOf({ ownerStats: true }), Object.keys(sysT)]));

        const full = S.cleanOv({ eqMsg: 'e', eq: 'bound', rmMsg: 'r' + C7 + 's', rm: 'none', cost: '5 + 1', damage: '1d6', area: { ft: 12.7, name: 'B' + C7 + 'x' }, held: ['acc', 'Zzz', 7], stats: { acc: 4, Zzz: 1, Dmg: 'x', Wt: 2e9 }, notes: ' n ', category: 'Gun', icon: 'X', name: ' Nm ', vis: 'gm', key: 'K', lvl: 3, throwSkill: 'DX', junk: 1 }, wpT);
        const ordT = clT([{ keptOn: 1, hid: 1, snap: { name: 'S' }, ov: { name: 'X' }, paid: 3, note: 'n', on: true, qty: 1, defId: 'i_gone', id: 'w_1' }])[0], custT = clT([{ id: 'w_c', qty: 1, def: { name: 'Mine' }, ov: { name: 'X' } }, { id: 'w_i', qty: 1, def: { name: 'In' }, lnk: 1, ov: { name: 'Y' } }]);
        check('F4c2 cleanOv: a copy\'s own values by shape — name, icon, category, notes, its list\'s stats (under the list\'s spelling; a stat no list has, a string or past 1e9 dropped), held (only stats it holds), a blast (whole feet, 1..3000), the damage and cost formulas, the locks (none kept) and their messages, in that order; never vis, key, level or throw skill; nothing left: null; no schema: no stats; compare only: every stat; never on a custom or inline row; a linked row carries it after paid',
            j(Object.keys(full)) === j(['name', 'icon', 'category', 'notes', 'stats', 'held', 'area', 'damage', 'cost', 'rm', 'rmMsg', 'eq', 'eqMsg']) && full.name === 'Nm' && full.notes === ' n ' && j(full.stats) === j({ Acc: 4 }) && j(full.held) === j(['Acc']) && j(full.area) === j({ ft: 12, shape: 'circle', name: 'B x' }) && full.rm === 'none' && full.rmMsg === 'r s' && full.eq === 'bound'
            && S.cleanOv({}, wpT) === null && S.cleanOv({ stats: { Zzz: 1 }, held: ['Zzz'], rm: 'evil', eq: 'constructor', vis: 'gm', notes: '   ', area: { ft: 0 } }, wpT) === null && S.cleanOv(null, wpT) === null && S.cleanOv('x', wpT) === null
            && j(S.cleanOv({ stats: { Acc: 1 }, held: ['Dmg'] }, wpT)) === j({ stats: { Acc: 1 } }) && j(S.cleanOv({ name: 'N', stats: { Acc: 1 }, held: ['Acc'] }, null)) === j({ name: 'N' }) && j(S.cleanOv({ stats: { Acc: 1, Heat: 2 } }, true)) === j({ stats: { Acc: 1, Heat: 2 } })
            && S.cleanOv({ area: { ft: 5000 } }, wpT).area.ft === 3000 && Object.getPrototypeOf(full) === Object.prototype && Object.getPrototypeOf(full.stats) === Object.prototype
            && j(Object.keys(ordT)) === j(['id', 'defId', 'qty', 'on', 'note', 'paid', 'ov', 'snap', 'hid', 'keptOn']) && custT.every(r => !('ov' in r)), j([full, ordT, custT]));

        const blT = iT('i_blaster'), blBefore = j(blT);
        const rd0 = S.rowDef(sysT, { id: 'w_1', defId: 'i_blaster', qty: 1 }), rd1 = S.rowDef(sysT, { id: 'w_1', defId: 'i_blaster', qty: 1, ov: { name: 'Ahto Blaster', stats: { Dmg: 5 }, damage: '9d6', rm: 'bound', rmMsg: 'Mine now' } });
        const mV = S.mergeOv(blT, { vis: 'gm', key: 'Z', lvl: 9, throwSkill: 'DX', name: 'N' }), rdC = S.rowDef(sysT, { id: 'w_2', defId: 'i_chain', qty: 1, ov: { rm: 'none', eq: 'none' } }), rdCm = S.rowDef(sysT, { id: 'w_2', defId: 'i_chain', qty: 1, ov: { rmMsg: 'Mine' } }), rdFm = S.rowDef(sysT, { id: 'w_3', defId: 'i_blaster', qty: 1, ov: { rmMsg: 'Orphan' } });
        const edT = JSON.parse(j(sysT)); edT.items[0].stats.Acc = 4; edT.items[0].stats.Dmg = 4; edT.items[0].name = 'Blaster Mk2';
        const rdE = S.rowDef(edT, { id: 'w_1', defId: 'i_blaster', qty: 1, ov: { stats: { Dmg: 5 } } }), rdL = S.rowDef(sysT, { id: 'w_l', defId: 'i_gone', qty: 1, snap: { name: 'Old', stats: { Acc: 1 }, rm: 'bound' }, ov: { stats: { Acc: 7 }, rm: 'none' } });
        check('F4c2 rowDef and mergeOv: without its own values a row\'s def is the library object itself (def === base); with them a new object — texts, formulas and a lock replace the base\'s, stats merge per key, the base untouched; vis, key, level and throw skill never; none lifts a bound entry (its message with it), a mode holds a free one, a message replaces the base\'s while a lock holds; a library edit reaches the fields without an override; a deleted item\'s copy merges over its snapshot',
            rd0.def === blT && rd0.base === blT && rd0.src === 'lib' && rd1.def !== blT && rd1.base === blT && rd1.def.name === 'Ahto Blaster' && j(rd1.def.stats) === j({ Acc: 2, Dmg: 5, Cost: 500, Wt: 0.0001 }) && rd1.def.damage === '9d6' && rd1.def.rm === 'bound' && rd1.def.rmMsg === 'Mine now' && j(blT) === blBefore
            && mV.name === 'N' && mV.vis === 'all' && mV.key === blT.key && mV.lvl === blT.lvl && mV.throwSkill === blT.throwSkill && S.mergeOv(blT, null) === blT && S.mergeOv(null, { name: 'x' }) === null
            && !('rm' in rdC.def) && !('rmMsg' in rdC.def) && !('eq' in rdC.def) && !('eqMsg' in rdC.def) && rdC.base.rm === 'bound' && rdCm.def.rm === 'bound' && rdCm.def.rmMsg === 'Mine' && !('rmMsg' in rdFm.def)
            && rdE.def.name === 'Blaster Mk2' && rdE.def.stats.Acc === 4 && rdE.def.stats.Dmg === 5 && rdL.src === 'lost' && rdL.def.stats.Acc === 7 && !('rm' in rdL.def) && rdL.base.rm === 'bound' && S.rowDef(sysT, { id: 'w_c', qty: 1, def: { name: 'M' } }).base.name === 'M',
            j([rd1.def, mV, rdC.def, rdL.def]));

        const chA = () => ({ id: 'c_t', name: 'T', ownerId: 'u_t', npc: false, values: { f_wp: clT(seedT()), f_sk: [{ id: 'w_s', defId: 'i_sneak', qty: 1 }], f_gr: [{ id: 'w_g', defId: 'i_knife', qty: 1 }], f_pl: [{ id: 'w_p', defId: 'i_blaster', qty: 1 }] } });
        const OP = (sys, q, o, fid, ch) => { const c = ch || chA(); const r = S.applyRowOp(sys, c, fid || 'f_wp', q, F, o || {}); if (r.ok) c.values[fid || 'f_wp'] = r.value; return [r, r.ok ? r.value : null, c]; };
        const PO = { player: true, view: pvT }, POff = { player: true, view: pvO }, ovq = (rowId, ov) => ({ op: 'ov', rowId, ov });
        const cliT = () => { const c = chA(); c.values.f_wp = S.projectRows(c.values.f_wp, pvT, libT, pT('f_wp').list); return c; };
        const [a1] = OP(sysO, ovq('w_b1', { stats: { Acc: 7 } }), POff), [a2] = OP(sysT, ovq('w_b1', { name: 'X' }), PO), [a2b] = OP(sysT, ovq('w_b1', { stats: { Acc: 7 }, name: 'X' }), PO);
        const [a3] = OP(sysT, ovq('w_r', { stats: { Acc: 7 } }), PO), [a3c] = OP(pvT, ovq('w_r', { stats: { Acc: 7 } }), PO, 'f_wp', cliT()), [a4] = OP(sysT, ovq('w_l', { stats: { Acc: 7 } }), PO), [a4c] = OP(pvT, ovq('w_l', { stats: { Acc: 7 } }), PO, 'f_wp', cliT());
        const [a5] = OP(sysT, ovq('w_c', { name: 'X' })), [a5o] = OP(sysT, ovq('w_c', { stats: { Acc: 7 } }), PO), [a6] = OP(sysT, ovq('w_b1', { stats: { Zzz: 1 } })), [a6o] = OP(sysT, ovq('w_b1', { stats: { Zzz: 1 } }), PO);
        const [a7] = OP(sysT, ovq('w_b1', { damage: '3d6+' })), [a7b] = OP(sysT, ovq('w_b1', { cost: '1d4' })), [a7c, v7c] = OP(sysT, ovq('w_b1', { cost: '5 + 1', damage: '2d6' }));
        const [a8, v8] = OP(sysT, ovq('w_b1', { stats: { Acc: 2 }, name: 'Blaster', rm: 'none', area: { ft: 10, name: 'Burst' } })), [a9, v9] = OP(sysT, ovq('w_b3', null));
        const c10 = chA(); OP(sysT, ovq('w_b3', { stats: { Acc: 7 } }), PO, 'f_wp', c10); const mid10 = j(rowOf(c10.values.f_wp, 'w_b3').ov); const [a10] = OP(sysT, ovq('w_b3', null), PO, 'f_wp', c10), after10 = rowOf(c10.values.f_wp, 'w_b3').ov;
        const [a10b, v10b] = OP(sysT, ovq('w_b3', { stats: null }), PO), [a11] = OP(sysT, ovq('w_b3', { stats: { Dmg: 9 } }), PO), [a11b] = OP(sysT, ovq('w_b3', { stats: { dmg: null } }), PO);
        const [a12] = OP(sysT, ovq('w_s', { eq: 'bound' }), {}, 'f_sk'), [a12b] = OP(sysT, ovq('w_s', { eqMsg: 'x' }), {}, 'f_sk'), [a12c, v12c] = OP(sysT, ovq('w_s', { eqMsg: null, stats: { Rel: 4 } }), {}, 'f_sk');
        const c13 = chA(), [a13] = OP(sysT, ovq('w_b1', { stats: { Acc: 4 } }), {}, 'f_wp', c13), g13 = j(rowOf(c13.values.f_wp, 'w_b1').ov); OP(sysT, ovq('w_b1', { stats: { Acc: 2 } }), {}, 'f_wp', c13); const g13b = rowOf(c13.values.f_wp, 'w_b1').ov;
        OP(sysT, ovq('w_b1', { stats: { Dmg: 8 } }), {}, 'f_wp', c13); OP(sysT, ovq('w_b1', { stats: { Dmg: null } }), {}, 'f_wp', c13); const g13c = rowOf(c13.values.f_wp, 'w_b1').ov;
        const [a14, v14] = OP(sysT, ovq('w_b1', { stats: { Acc: 7 } }), PO);
        const [a15] = OP(sysT, ovq('w_b1', { area: { ft: 2.5 } })), [a15b] = OP(sysT, ovq('w_b1', { area: { ft: 5000 } })), [a15c, v15c] = OP(sysT, ovq('w_b1', { area: { ft: 20 } }));
        const [a16] = OP(sysT, ovq('w_b1', { rm: 'evil' })), [a16b, v16b] = OP(sysT, ovq('w_ch', { rm: 'none' }));
        const [a17] = OP(sysT, ovq('w_b1', 5)), [a17b] = OP(sysT, ovq('w_b1', { vis: 'gm' })), [a17c] = OP(sysT, ovq('w_b1', { icon: 'icon:zzz-not-bundled' })), [a17d, v17d] = OP(sysT, ovq('w_b3', { name: '  ', damage: '' }));
        const [a19] = OP(sysT, ovq('w_h', { stats: { Acc: 1 } }), PO), [a19b, v19b] = OP(sysT, ovq('w_h', { stats: { Acc: 1 } })), [a20] = OP(sysT, ovq('w_p', { name: 'X' }), {}, 'f_pl');
        check('F4c2 applyRowOp ov, every cell: its owner — Setting A off: field; a key other than stats: field; an inline or deleted item\'s copy: missing (the host\'s rows and the client\'s projected copy alike); a hidden row: missing; a custom row: field; a stat the GM set: field (Q1 A); their own stat stored, not held; null or stats null takes back only their own (the GM\'s name, formula, lock and stat stay). The GM: a stat outside the list: value; a formula that does not parse, a cost with dice: value (why formula); a field equal to the base is not stored; null clears everything; a stat they set is held (equal to the base, or null: no longer); a switch lock or its message on a list with no switch: value; a blast in whole feet 1..3000; a lock a mode or none (none on a free entry: nothing); a text that cleans to nothing, a key it does not know, a patch that is not an object: value; a plain list: field; every answer names its row and quantity',
            a1.reason === 'field' && a2.reason === 'field' && a2b.reason === 'field' && a3.reason === 'missing' && a3c.reason === 'missing' && a4.reason === 'missing' && a4c.reason === 'missing' && a5.reason === 'field' && a5o.reason === 'field' && a6.reason === 'value' && a6o.reason === 'value'
            && a7.reason === 'value' && a7.why === 'formula' && a7b.reason === 'value' && a7b.why === 'formula' && a7c.ok && j(rowOf(v7c, 'w_b1').ov) === j({ cost: '5 + 1' }) && a8.ok && !('ov' in rowOf(v8, 'w_b1')) && a9.ok && !('ov' in rowOf(v9, 'w_b3'))
            && mid10 === j({ name: 'Ahto Blaster', stats: { Dmg: 5, Wt: 1, Acc: 7 }, held: ['Dmg'], damage: '9d6', rm: 'bound', rmMsg: 'Stays mine' }) && a10.ok && j(after10) === j({ name: 'Ahto Blaster', stats: { Dmg: 5 }, held: ['Dmg'], damage: '9d6', rm: 'bound', rmMsg: 'Stays mine' })
            && a10b.ok && j(rowOf(v10b, 'w_b3').ov) === j(after10) && a11.reason === 'field' && a11b.reason === 'field' && a12.reason === 'value' && a12b.reason === 'value' && a12c.ok && j(rowOf(v12c, 'w_s').ov) === j({ stats: { Rel: 4 }, held: ['Rel'] })
            && a13.ok && a13.row === 'w_b1' && a13.qty === 1 && g13 === j({ stats: { Acc: 4 }, held: ['Acc'] }) && g13b === undefined && g13c === undefined && a14.ok && j(rowOf(v14, 'w_b1').ov) === j({ stats: { Acc: 7 } })
            && a15.reason === 'value' && a15b.reason === 'value' && a15c.ok && j(rowOf(v15c, 'w_b1').ov) === j({ area: { ft: 20, shape: 'circle', name: '' } }) && a16.reason === 'value' && a16b.ok && j(rowOf(v16b, 'w_ch').ov) === j({ rm: 'none' })
            && a17.reason === 'value' && a17b.reason === 'value' && a17c.reason === 'value' && a17d.ok && !('name' in rowOf(v17d, 'w_b3').ov) && !('damage' in rowOf(v17d, 'w_b3').ov) && a19.reason === 'missing' && a19b.ok && j(rowOf(v19b, 'w_h').ov) === j({ stats: { Acc: 1 }, held: ['Acc'] }) && a20.reason === 'field',
            j([a1, a2, a3c, a4c, a5, a6, a7, a8, a10, after10, a12c, g13, a14, a15c, a16b, a17c, a17d, a19, a20]));

        const Lc = () => { const c = chA(); rowOf(c.values.f_wp, 'w_b2').ov = { eq: 'bound', eqMsg: 'Keep it on' }; rowOf(c.values.f_wp, 'w_ch').ov = { rm: 'none', eq: 'none' }; return c; };
        const [l1] = OP(sysT, { op: 'remove', rowId: 'w_b3' }, PO), [l2, lv2] = OP(sysT, { op: 'remove', rowId: 'w_ch' }, PO, 'f_wp', Lc()), [l3] = OP(sysT, { op: 'set', rowId: 'w_b2', facts: { on: false } }, PO, 'f_wp', Lc());
        const [l4] = OP(sysT, { op: 'set', rowId: 'w_ch', facts: { on: false } }, PO, 'f_wp', Lc()), [l5] = OP(sysT, { op: 'remove', rowId: 'w_ch' }, PO), [l6] = OP(sysT, ovq('w_ch', { rm: 'none' }), PO), [l7] = OP(sysT, { op: 'setQty', rowId: 'w_b2', qty: 1 }, PO, 'f_wp', Lc());
        check('F4c2 per-copy locks: a copy\'s own "bound" refuses its owner\'s removal with the copy\'s own message; none lets a bound entry go (removed, and switched off); a copy\'s switch lock holds it on (its message) and covers lowering it while on; the owner may not set a lock (field); the library\'s lock still holds a copy without one',
            l1.reason === 'stays' && l1.msg === 'Stays mine' && l2.ok && !rowOf(lv2, 'w_ch') && l3.reason === 'stays' && l3.msg === 'Keep it on' && l3.eq === true && l4.ok && l5.reason === 'stays' && l5.msg === 'It holds' && l6.reason === 'field' && l7.reason === 'stays' && l7.msg === 'Keep it on',
            j([l1, l2, l3, l4, l5, l6, l7]));

        const kc = chA(); kc.values.f_wp.push({ id: 'w_k', defId: 'i_gone2', qty: 2, on: true, paid: 3, snap: S.cleanRowDef({ name: 'Old', stats: { Acc: 1, Rel: 4 }, damage: '1d4', rm: 'bound', rmMsg: 'x' }, true), ov: { name: 'Renamed', stats: { Acc: 9 }, held: ['Acc'], damage: '4d6', rm: 'none' } });
        const [k1, kv1] = OP(sysT, { op: 'keep', rowId: 'w_k' }, {}, 'f_wp', kc);
        check('F4c2 Make custom (keep) folds the copy\'s own values into its definition (its name, stats of its list, damage; none lifts the lock) and keeps what was paid; the ov itself is not kept',
            k1.ok && j(rowOf(kv1, 'w_k')) === j({ id: 'w_k', qty: 2, on: true, paid: 3, def: { name: 'Renamed', category: '', icon: '', notes: '', vis: 'all', stats: { Acc: 9 }, damage: '4d6' } }), j(rowOf(kv1, 'w_k')));

        const ciO = (ov, absent) => { const m = { rid: 'r1', charId: 'c_t', fieldId: 'f_wp', op: 'ov', rowId: 'w_b1' }; if (!absent) m.ov = ov; return S.cleanCharItem(m); }, many17 = {}; for (let i = 0; i < 17; i++) many17['K' + i] = i;
        check('F4c2 cleanCharItem ov (critic 5): null travels (back to the library); a patch keeps the keys it knows (held and the rest dropped); ov absent, not an object, or a patch that cleans to nothing ({}, held only, unknown keys, an empty stats) refuses the message; a stat key that is not one (constructor, toString, __proto__, Constructor), 17 keys or a number past 1e9 refuses it (F5a2: a string travels — a choice\'s label, which the host judges); texts within four times their cap; a lock a mode or none; a blast { ft, name? }; an add is unchanged; a set op carries no ov',
            j(ciO(null)) === j({ rid: 'r1', charId: 'c_t', fieldId: 'f_wp', op: 'ov', rowId: 'w_b1', ov: null }) && j(ciO({ name: 'N', held: ['Acc'], junk: 1, stats: { Acc: 3, Dmg: null } }).ov) === j({ name: 'N', stats: { Acc: 3, Dmg: null } })
            && [ciO(null, true), ciO({}), ciO(5), ciO([]), ciO('x'), ciO({ held: ['Acc'] }), ciO({ foo: 1 }), ciO({ stats: {} })].every(x => x === null)
            && ['{"constructor":1}', '{"toString":1}', '{"__proto__":1}', '{"Constructor":1}', '{"Acc":2000000000}', '{"1a":1}'].every(s => ciO({ stats: JSON.parse(s) }) === null) && ciO({ stats: many17 }) === null && ciO({ stats: null }).ov.stats === null && ciO({ stats: { Acc: 'Strength' } }).ov.stats.Acc === 'Strength'
            && ciO({ name: 'x'.repeat(241) }) === null && ciO({ name: 'x'.repeat(240) }).ov.name.length === 240 && ciO({ notes: 'x'.repeat(801) }) === null && ciO({ damage: 'x'.repeat(1201) }) === null && ciO({ name: 5 }) === null
            && ciO({ rm: 'evil' }) === null && ciO({ eq: 'constructor' }) === null && ciO({ rm: 'none' }).ov.rm === 'none' && ciO({ area: { ft: '5' } }) === null && ciO({ area: { ft: 5, name: 7 } }) === null && j(ciO({ area: { ft: 5, name: 'B', x: 1 } }).ov.area) === j({ ft: 5, name: 'B' }) && ciO({ area: null }).ov.area === null
            && j(S.cleanCharItem({ rid: 'r1', charId: 'c_1', fieldId: 'f_gear', op: 'add', defId: 'i_rope', rowId: 'w_x1', qty: 2, ov: { name: 'X' }, junk: 1 })) === j({ rid: 'r1', charId: 'c_1', fieldId: 'f_gear', op: 'add', defId: 'i_rope', rowId: 'w_x1', qty: 2 })
            && !('ov' in S.cleanCharItem({ rid: 'r1', charId: 'c_1', fieldId: 'f_gear', op: 'set', rowId: 'w_x1', facts: { note: 'x' }, ov: { name: 'X' } })));

        const own5 = chA(); own5.values.f_wp = clT(seedT().concat([{ id: 'w_ri', defId: 'i_rune', qty: 1, ov: { name: 'Secret rune', stats: { Acc: 8 }, damage: '9d9', rm: 'none' } }, { id: 'w_d', defId: 'i_blaster', qty: 1, ov: { damage: '9d6', rm: 'none', eq: 'curse', eqMsg: 'm' } }])); own5.values.f_pl = [{ id: 'w_p', defId: 'i_blaster', qty: 1, ov: { name: 'Plain name', stats: { Acc: 3 } } }];
        const pv5 = S.charFor(own5, pvT, 'u_t', { items: libT }).values, pw5 = pv5.f_wp, pri = rowOf(pw5, 'w_ri');
        const quiet5 = (() => { const x = JSON.parse(j(own5)); rowOf(x.values.f_wp, 'w_b3').ov.damage = '1d100'; rowOf(x.values.f_wp, 'w_b3').ov.rm = 'curse'; rowOf(x.values.f_wp, 'w_b3').ov.cost = '7'; rowOf(x.values.f_wp, 'w_ri').ov.damage = '1d2'; return j(S.charFor(x, pvT, 'u_t', { items: libT }).values) === j(pv5); })();
        check('F4c2 the owner\'s copy: a pointer carries its own name, stats (with which the GM set: held) and blast — never the damage, cost or locks, and no ov at all when those are all it has (never an empty one); a GM-only item inline has its own values folded in, then reduced to the players\' fields (its list\'s stats only); a plain list\'s copy carries no stats; the owner\'s re-clean is a fixed point (cleanOv\'s key order); a GM change of a copy\'s formulas or locks leaves the projection unchanged',
            j(rowOf(pw5, 'w_b3')) === j({ id: 'w_b3', defId: 'i_blaster', qty: 1, ov: { name: 'Ahto Blaster', stats: { Dmg: 5, Wt: 1 }, held: ['Dmg'] } }) && j(rowOf(pw5, 'w_d')) === j({ id: 'w_d', defId: 'i_blaster', qty: 1 }) && !/9d6|9d9|bound|curse|Stays mine|"rm"|"eq"|damage|cost/.test(j(pv5))
            && pri.lnk === 1 && pri.def.name === 'Secret rune' && j(pri.def.stats) === j({ Acc: 8, Cost: 10 }) && j(pv5.f_pl) === j([{ id: 'w_p', defId: 'i_blaster', qty: 1, ov: { name: 'Plain name' } }])
        ...
        check('F4c2 ownerSeesSame (the real one, sliced; critic 1): a GM change of a copy\'s own stats, name, notes, blast or held is sent; one of its damage, cost, lock or lock message is not — on a pointer and on a GM-only item\'s inline copy',
            ...
            && same(undefined, { damage: '9d6' }) === true && same({ cost: '1' }, { cost: '2' }) === true && same(undefined, { rm: 'bound', rmMsg: 'x' }) === true && same({ eq: 'none' }, { eq: 'curse', eqMsg: 'y' }) === true
            && same({ stats: { Acc: 3 } }, { stats: { Acc: 4 } }, ovIn) === false && same({ name: 'A' }, { name: 'B' }, ovIn) === false && same(undefined, { damage: '9d6', rm: 'none' }, ovIn) === true);

        // critic 15: every owner cell of the ov matrix, run on the host (the full system, judged on the view) and on the client (the view, over its projected copy)
        const agree5 = (sys, q) => {
            const pv = cleanSystem(sys, { F, gmView: false }), lib = {}; sys.items.forEach(i => { lib[i.id] = i; }); const pf = pv.fields.find(f => f.id === 'f_wp');
            const hc = { id: 'c_h', values: { f_wp: clT(seedT()) } }, cc = { id: 'c_h', values: { f_wp: S.projectRows(hc.values.f_wp, pv, lib, pf.list || null) } };
            const h = S.applyRowOp(sys, hc, 'f_wp', q, F, { player: true, view: pv }), c = S.applyRowOp(pv, cc, 'f_wp', q, F, { player: true, view: pv });
            return h.ok === c.ok && h.reason === c.reason && (!h.ok || j(S.projectRows(h.value, pv, lib, pf.list || null)) === j(c.value)) ? (h.ok ? 'ok' : h.reason) : 'DIFFER ' + j([h, c]);
        };
        const ag5 = [[sysO, ovq('w_b1', { stats: { Acc: 7 } })], [sysT, ovq('w_b1', { name: 'X' })], [sysT, ovq('w_b1', { stats: { Acc: 7 }, damage: '1d6' })], [sysT, ovq('w_r', { stats: { Acc: 7 } })], [sysT, ovq('w_l', { stats: { Acc: 7 } })], [sysT, ovq('w_c', { stats: { Acc: 7 } })], [sysT, ovq('w_h', { stats: { Acc: 7 } })],
            [sysT, ovq('w_b1', { stats: { Zzz: 7 } })], [sysT, ovq('w_b3', { stats: { Dmg: 7 } })], [sysT, ovq('w_b3', { stats: { Acc: 7 } })], [sysT, ovq('w_b1', { stats: { Acc: 2 } })], [sysT, ovq('w_b3', null)], [sysT, ovq('w_b3', { stats: null })], [sysT, ovq('w_b1', { stats: { Acc: 2e9 } })], [sysT, ovq('w_b1', { stats: { diff: 2, Wt: 0.5 } })], [sysT, ovq('w_nope', { stats: { Acc: 1 } })]].map(c => agree5(c[0], c[1]));
        check('F4c2 (critic 15): the host and the player\'s client agree on every owner cell of the ov matrix (Setting A off, another key, an inline, deleted-copy, custom or hidden row, a stat outside the list, a held stat, their own, one equal to the base, null, stats null, a number past 1e9, a named stat, an unknown row); the host\'s answer, projected, is the client\'s own',
            j(ag5) === j(['field', 'field', 'field', 'missing', 'missing', 'field', 'missing', 'value', 'field', 'ok', 'ok', 'ok', 'ok', 'value', 'ok', 'missing']), j(ag5));

        const prevR = JSON.parse(j(sysT)), nextR = JSON.parse(j(sysT)), blN = nextR.items.find(i => i.id === 'i_blaster'); blN.stats.Dmg = 4; blN.stats.Acc = 4;
        const charsR = { c_a: { id: 'c_a', values: { f_wp: [{ id: 'w_1', defId: 'i_blaster', qty: 1 }, { id: 'w_4', defId: 'i_blaster', qty: 1 }] } }, c_b: { id: 'c_b', values: { f_wp: [{ id: 'w_2', defId: 'i_blaster', qty: 1, ov: { stats: { Dmg: 5 } } }] } }, c_c: { id: 'c_c', values: { f_wp: [{ id: 'w_3', defId: 'i_blaster', qty: 1, hid: 1 }] } },
            c_d: { id: 'c_d', values: { f_wp: [{ id: 'w_5', defId: 'i_gone', qty: 1, snap: { name: 'x' } }, { id: 'w_6', defId: 'i_blaster', qty: 1, snap: { name: 'Blaster' } }] } }, c_e: { id: 'c_e', values: { f_wp: [{ id: 'w_7', defId: 'i_knife', qty: 1 }] } } };
        const r1 = S.itemReach(prevR, nextR, charsR), next2 = JSON.parse(j(nextR)); next2.items.find(i => i.id === 'i_knife').stats.Cost = 25; next2.items.find(i => i.id === 'i_chain').name = 'Chains';
        const r2 = S.itemReach(prevR, next2, charsR), next3 = JSON.parse(j(prevR)), bl3 = next3.items.find(i => i.id === 'i_blaster'); bl3.name = 'Blaster II'; bl3.stats.Dmg = 9; bl3.damage = '3d6';
        const chars3 = { c_x: { id: 'c_x', values: { f_wp: [{ id: 'w_1', defId: 'i_blaster', qty: 1, ov: { stats: { Dmg: 5 } } }] } }, c_y: { id: 'c_y', values: { f_wp: [{ id: 'w_2', defId: 'i_blaster', qty: 1, ov: { name: 'Mine', stats: { Dmg: 6 }, damage: '1d4' } }] } } }, r3 = S.itemReach(prevR, next3, chars3);
        check('F4c2 itemReach: an edited entry\'s line counts characters, not rows (two rows of it: once), a kept curse counts, a deleted one\'s copy (a snapshot) does not, and says how many have their own value of a changed field (a stat by its label); an entry nobody carries says nothing; several say how far together; no earlier system (a first save, critic 9c), nothing changed or no characters: nothing',
            j(r1) === j({ lines: ['Blaster: affects 3 characters; 1 has its own Dmg.'], text: 'Blaster: affects 3 characters; 1 has its own Dmg.' })
            && j(r2.lines) === j(['Blaster: affects 3 characters; 1 has its own Dmg.', 'Knife: affects 1 character.']) && r2.text === '2 changed items reach 4 characters (each is in the session log’s Items).'
            && j(r3.lines) === j(['Blaster II: affects 2 characters; 2 have their own Dmg, name, damage.'])
            && j(S.itemReach(null, nextR, charsR)) === j({ lines: [], text: '' }) && j(S.itemReach(undefined, nextR, charsR)) === j({ lines: [], text: '' }) && S.itemReach(prevR, prevR, charsR).lines.length === 0 && S.itemReach(prevR, nextR, null).lines.length === 0, j([r1, r2, r3]));

        // the sheet (sliced from sheets.js, run for real on the fake DOM): ✎, the dots, the form
        const fe5 = (tag, cls, text) => ({ tag, className: cls || '', textContent: text === undefined || text === null ? '' : String(text), children: [], title: '', type: '', value: '', checked: false, disabled: false, selected: false, dataset: {}, style: {}, classList: { add() {} }, on: {}, get childNodes() { return this.children; }, appendChild(x) { this.children.push(x); return x; }, insertBefore(x) { this.children.unshift(x); return x; }, addEventListener(k, f) { this.on[k] = f; } });
        const kids5 = n => (n.children || []), walk5 = (n, p, acc) => { if (p(n)) acc.push(n); kids5(n).forEach(k => walk5(k, p, acc)); return acc; }, find5 = (n, c) => walk5(n, x => (' ' + x.className + ' ').indexOf(' ' + c + ' ') >= 0, []), cl5 = n => kids5(n).map(k => String(k.className).split(' ').pop());
        const opt5 = (v, t, s) => { const o = fe5('option', null, t); o.value = v; if (s) o.selected = true; return o; }, doc5 = { createTextNode: t => ({ tag: '#text', className: '', textContent: t, children: [] }) };
        const itSrc5 = cutT('// ---- item-list widgets', '// 5h: an effect\'s changes as short text'), commits5 = [], redraws5 = [];
        const deps5 = ['el', 'iconNode', 'rowDef', 'rowIdOf', 'rowLvl', 'rowOn', 'commitItem', 'opt', 'fmtNum', 'iconText', 'LIMITS', '_fxLive', 'document', 'renderViews', 'closeHud', 'closeSheet', 'window', 'rowStat', 'rowPaid', '_fxView'];
        const mkW5 = (live, view) => new Function(...deps5, itSrc5 + '\nreturn { itemListInto: itemListInto, itemTableInto: itemTableInto, gmItemBits: gmItemBits, setForm: function(v) { _rowForm = v; }, getForm: function() { return _rowForm; } };')(fe5, (v, cls) => fe5('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, (c, f, q) => commits5.push([f.id, JSON.parse(j(q))]), opt5, S.fmtNum, v => v || '', S.LIMITS, live, doc5, id => redraws5.push(id), () => {}, () => {}, {}, S.rowStat, S.rowPaid, view);
        const W5 = mkW5(true, 'sheet'), W5off = mkW5(false, 'sheet'), W5hud = mkW5(true, 'hud'), chW5 = { id: 'c_t', name: 'T', ownerId: 'u_t', values: {} }, rowsG = clT(seedT());
        const rowsP = S.projectRows(rowsG, pvT, libT, pT('f_wp').list), rowsPo = S.projectRows(rowsG, pvO, libT, pvO.fields.find(f => f.id === 'f_wp').list);
        const draw5 = (W, sys, fid, rows, gm, editable, table) => { const w = fe5('div'), f0 = sys.fields.find(f => f.id === fid), f = table ? Object.assign({}, f0, { table: { footer: true } }) : f0; W[table ? 'itemTableInto' : 'itemListInto'](w, f, chW5, rows, sys, false, editable !== false, gm); return w; };
        const partIs = p => x => x.dataset && x.dataset.part === p, edOf = (w, rid) => walk5(w, partIs('ed-' + rid), []).filter(x => x.className.indexOf('sheet-item-edit') >= 0);
        const lineOf = (w, rid) => kids5(w).find(l => /^sheet-item( |$)/.test(l.className) && edOf(l, rid).length + walk5(l, partIs('on-' + rid), []).length + walk5(l, partIs('paid-' + rid), []).length > 0);
        const gW5 = draw5(W5, sysT, 'f_wp', rowsG, true), oW5 = draw5(W5, pvT, 'f_wp', rowsP, false), offOwner = draw5(W5, pvO, 'f_wp', rowsPo, false), grW5 = draw5(W5, pvT, 'f_gr', [{ id: 'w_g', defId: 'i_knife', qty: 1 }], false);
        const plW5 = draw5(W5, sysT, 'f_pl', [{ id: 'w_p', defId: 'i_blaster', qty: 2 }], true), roW5 = draw5(W5, sysT, 'f_wp', rowsG, true, false);
        W5off.setForm({ charId: 'c_t', fieldId: 'f_wp', rowId: 'w_b1', view: 'sheet', typed: {} }); const prevW5 = draw5(W5off, sysT, 'f_wp', rowsG, true);
        check('F4c2 the sheet: ✎ for the GM on every linked copy of a shaped list (the library\'s, a deleted item\'s, a hidden one), never a custom row, and a plain list draws as before (name, qty, ×); for the owner only with Setting A, on a list with stats, on a library copy (never an inline or custom one); none where nothing can be changed — read-only, the Layout preview or a pop-out, where an open form is not drawn either (critic 4)',
            ['w_b1', 'w_b2', 'w_b3', 'w_r', 'w_l', 'w_h', 'w_ch'].every(r => edOf(gW5, r).length === 1) && edOf(gW5, 'w_c').length === 1 && edOf(gW5, 'w_c')[0].title === 'Change it' && edOf(gW5, 'w_b1')[0].title === 'Change this copy' && edOf(gW5, 'w_b1')[0].textContent === '✎'
            && j(cl5(plW5.children[0])) === j(['sheet-item-name', 'sheet-item-qty', 'sheet-item-rm'])
            && ['w_b1', 'w_b2', 'w_b3', 'w_ch'].every(r => edOf(oW5, r).length === 1) && ['w_r', 'w_l', 'w_c'].every(r => edOf(oW5, r).length === 0) && edOf(oW5, 'w_b1')[0].title === 'Change this copy’s stats'
            && find5(offOwner, 'sheet-item-edit').length === 0 && find5(grW5, 'sheet-item-edit').length === 0 && find5(roW5, 'sheet-item-edit').length === 0 && find5(prevW5, 'sheet-item-edit').length === 0 && find5(prevW5, 'sheet-row-form').length === 0,
            j([find5(gW5, 'sheet-item-edit').map(x => x.dataset.part), find5(oW5, 'sheet-item-edit').map(x => x.dataset.part)]));

        const b3G = lineOf(gW5, 'w_b3'), b3P = lineOf(oW5, 'w_b3'), dotsOf = n => find5(n, 'sheet-ov').map(d => d.title), stlText = n => { const s = find5(n, 'sheet-item-statline')[0]; return s ? (kids5(s).length ? kids5(s).map(x => x.tag === '#text' ? x.textContent : '[' + x.title + ']').join('') : s.textContent) : ''; };
        const lostOv = draw5(W5, sysT, 'f_wp', clT([{ id: 'w_l', defId: 'i_gone', qty: 1, snap: { name: 'Old gun', stats: { Acc: 1 } }, ov: { stats: { Acc: 7 } } }]), true), tW5 = draw5(W5, sysT, 'f_wp', rowsG, true, true, true);
        const tRow = kids5(kids5(tW5.children[0])[1]).find(tr => edOf(tr, 'w_b3').length === 1), tHead = kids5(kids5(kids5(tW5.children[0])[0])[0]).map(x => x.textContent);
        check('F4c2 the dots: the name\'s dot names each field of the copy\'s own with the library\'s value; a stat\'s dot (chip, 📝 line, table cell) the library\'s value, "by the GM" on its owner\'s sheet for a stat the GM set, "its copy" on a deleted item\'s copy; a row with no values of its own has none',
            j(dotsOf(find5(b3G, 'sheet-item-name')[0])) === j(['Changed on this copy · name, library: Blaster · damage, library: 2d6 · removal, library: free · removal message, library: (none)']) && j(dotsOf(find5(b3G, 'sheet-item-stat')[1])) === j(['Changed on this copy · library: 3']) && dotsOf(find5(b3G, 'sheet-item-stat')[0]).length === 0
            && j(dotsOf(find5(b3P, 'sheet-item-name')[0])) === j(['Changed on this copy · name, library: Blaster']) && j(dotsOf(find5(b3P, 'sheet-item-stat')[1])) === j(['Changed on this copy by the GM · library: 3'])
            && stlText(kids5(gW5)[kids5(gW5).indexOf(b3G) + 1]) === 'Acc 2 · Dmg 5[Changed on this copy · library: 3] · Cost 500 · Wt 1[Changed on this copy · library: 0.0001] · diff E · Paid 500'
            && j(dotsOf(find5(lostOv, 'sheet-item-stat')[0])) === j(['Changed on this copy · its copy: 1']) && dotsOf(lineOf(gW5, 'w_b1')).length === 0 && stlText(kids5(gW5)[kids5(gW5).indexOf(lineOf(gW5, 'w_b1')) + 1]) === 'Acc 2 · Dmg 3 · Cost 500 · Wt 0.0001 · diff E · Paid 500'
            && !!tRow && j(dotsOf(find5(tRow, 'sheet-itcol-stat')[1])) === j(['Changed on this copy · library: 3']) && find5(tRow, 'sheet-itcol-stat')[1].textContent === '5' && edOf(find5(tRow, 'sheet-itcol-act')[0], 'w_b3').length === 1 && j(tHead) === j(['Item', 'Acc', 'Dmg', 'Readied', 'Qty', 'Paid', '']),
            j([dotsOf(b3G), dotsOf(b3P), stlText(kids5(gW5)[kids5(gW5).indexOf(b3G) + 1])]));

        const formAt = (W, sys, rows, gm, rid, view, table) => { W.setForm({ charId: 'c_t', fieldId: 'f_wp', rowId: rid, view: view || 'sheet', typed: {} }); return draw5(W, sys, 'f_wp', rows, gm, true, table); };
        const gF = formAt(W5, sysT, rowsG, true, 'w_b3'), gForm = find5(gF, 'sheet-row-form'), gCaps = find5(gF, 'sheet-rf-cap').map(x => x.textContent), boxP = (w, name) => walk5(w, x => x.dataset && x.dataset.part === 'rf-w_b3-' + name && x.className.indexOf('sheet-rf-back') < 0, [])[0];
        const pF = formAt(W5, pvT, rowsP, false, 'w_b3'), pCaps = find5(pF, 'sheet-rf-cap').map(x => x.textContent), pDmg = boxP(pF, 's-Dmg'), pWt = boxP(pF, 's-Wt');
        check('F4c2 the form: one, under its row (after the 📝 line), in the view it was opened in; the GM\'s boxes — name, icon, category, notes, each stat, blast, damage, cost formula, the two locks and their messages — each holding the copy\'s own value (empty: the library\'s, shown faintly), a ↺ on each of its own, Follow the library and Done; the owner\'s: the stats alone, one the GM set read-only ("Set by the GM", no ↺); every control names its field and a part from the row id and the stat key',
            gForm.length === 1 && kids5(gF).indexOf(gForm[0]) === kids5(gF).indexOf(lineOf(gF, 'w_b3')) + 2
            && j(gCaps) === j(['Name', 'Icon', 'Category', 'Notes', 'Acc', 'Dmg', 'Cost', 'Wt', 'diff', 'Blast ft', 'Damage', 'Cost formula', 'When a player removes it', 'Message on removal', 'When a player switches it off', 'Message on switching off'])
            && boxP(gF, 'name').value === 'Ahto Blaster' && boxP(gF, 'name').placeholder === 'Blaster' && boxP(gF, 's-Acc').value === '' && boxP(gF, 's-Acc').placeholder === '2' && boxP(gF, 's-Acc').step === 'any' && boxP(gF, 's-Dmg').value === '5' && boxP(gF, 's-Dmg').title === 'Library: 3' && boxP(gF, 's-Dmg').dataset.fid === 'f_wp'
            && kids5(boxP(gF, 's-diff'))[0].textContent === 'Library: E' && boxP(gF, 'ft').placeholder === '10' && boxP(gF, 'damage').value === '9d6' && boxP(gF, 'damage').placeholder === '2d6' && kids5(boxP(gF, 'rm')).find(o => o.selected).value === 'bound' && kids5(boxP(gF, 'rm'))[0].textContent === 'Library: A player may remove it'
            && j(find5(gF, 'sheet-rf-back').map(x => x.dataset.part)) === j(['rf-w_b3-name', 'rf-w_b3-s-Dmg', 'rf-w_b3-s-Wt', 'rf-w_b3-damage', 'rf-w_b3-rm', 'rf-w_b3-rmMsg']) && find5(gF, 'sheet-rf-follow')[0].textContent === 'Follow the library' && find5(gF, 'sheet-rf-done').length === 1
            && j(pCaps) === j(['Acc', 'Dmg', 'Cost', 'Wt', 'diff']) && pDmg.disabled === true && pDmg.title === 'Set by the GM' && pWt.disabled === false && j(find5(pF, 'sheet-rf-back').map(x => x.dataset.part)) === j(['rf-w_b3-s-Wt']) && find5(pF, 'sheet-rf-follow').length === 1
            && find5(formAt(W5, sysT, rowsG, true, 'w_c'), 'sheet-row-custom').length === 1 && find5(formAt(W5, pvO, rowsPo, false, 'w_b3'), 'sheet-row-form').length === 0 && find5(formAt(W5hud, sysT, rowsG, true, 'w_b3', 'sheet'), 'sheet-row-form').length === 0 && find5(formAt(W5hud, sysT, rowsG, true, 'w_b3', 'hud'), 'sheet-row-form').length === 1 && find5(formAt(W5, sysT, rowsG, true, 'w_b3', 'hud'), 'sheet-row-form').length === 0,
            j([gCaps, pCaps, find5(gF, 'sheet-rf-back').map(x => x.dataset.part)]));

        commits5.length = 0; const g2 = formAt(W5, sysT, rowsG, true, 'w_b3'), cb = (name, v, kind) => { const b = boxP(g2, name); b.value = v; b.on[kind || 'change'](); return b; };
        const acc = cb('s-Acc', '4'); cb('s-Acc', ''); const bad1 = cb('s-Acc', 'abc').value, bad2 = cb('s-Acc', '2e9').value; cb('name', '  '); cb('name', 'X'); cb('ft', '20'); cb('ft', '0'); cb('rm', 'none'); cb('rm', ''); cb('s-diff', '2'); cb('damage', '4d6');
        find5(g2, 'sheet-rf-back').find(x => x.dataset.part === 'rf-w_b3-s-Dmg').on.click(); find5(g2, 'sheet-rf-follow')[0].on.click();
        const gCommits = commits5.slice(); commits5.length = 0; const p2 = formAt(W5, pvT, rowsP, false, 'w_b3'); const pAcc = boxP(p2, 's-Acc'); pAcc.value = '7'; pAcc.on.change(); find5(p2, 'sheet-rf-follow')[0].on.click(); const pCommits = commits5.slice();
        const O = ov => ['f_wp', { op: 'ov', rowId: 'w_b3', ov }];
        check('F4c2 the form\'s commits: each box on change — a stat as a number under its key (blank: null, the library\'s; not a number or past 1e9: the box put back, nothing sent), a text as typed (blank: null), a blast in whole feet with the library\'s name (0: null), a lock (blank: null), a named stat by its position; ↺ sends null for its field; Follow the library sends null (the GM) or stats null (the owner)',
            j(gCommits) === j([O({ stats: { Acc: 4 } }), O({ stats: { Acc: null } }), O({ name: null }), O({ name: 'X' }), O({ area: { ft: 20, name: 'Burst' } }), O({ area: null }), O({ rm: 'none' }), O({ rm: null }), O({ stats: { diff: 2 } }), O({ damage: '4d6' }), O({ stats: { Dmg: null } }), O(null)]) && bad1 === '' && bad2 === '' && acc.dataset.part === 'rf-w_b3-s-Acc'
            && j(pCommits) === j([O({ stats: { Acc: 7 } }), O({ stats: null })]), j([gCommits, pCommits]));

        W5.setForm(null); redraws5.length = 0; const e1 = draw5(W5, sysT, 'f_wp', rowsG, true); edOf(e1, 'w_b1')[0].on.click(); const st1 = W5.getForm(), rd1c = redraws5.slice();
        const e2 = draw5(W5, sysT, 'f_wp', rowsG, true), nb = boxP(e2, 'name') || walk5(e2, partIs('rf-w_b1-name'), [])[0]; nb.value = 'Typed'; nb.on.input(); const e3 = draw5(W5, sysT, 'f_wp', rowsG, true), nb3 = walk5(e3, partIs('rf-w_b1-name'), [])[0];
        const typedKept = nb3.value === 'Typed'; nb3.value = 'Typed'; commits5.length = 0; nb3.on.change(); const typedGone = !('rf-w_b1-name' in W5.getForm().typed);
        edOf(e3, 'w_b1')[0].on.click(); const closed = W5.getForm() === null; W5.setForm({ charId: 'c_t', fieldId: 'f_wp', rowId: 'w_b1', view: 'sheet', typed: {} }); const e4 = draw5(W5, sysT, 'f_wp', rowsG, true); find5(e4, 'sheet-rf-done')[0].on.click(); const doneNull = W5.getForm() === null;
        const Wv = new Function(...deps5.filter(d => d !== '_fxView'), 'var _fxView = "sheet";\n' + itSrc5 + '\nreturn { itemListInto: itemListInto, setView: function(v) { _fxView = v; }, getForm: function() { return _rowForm; } };')(fe5, (v, cls) => fe5('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, () => {}, opt5, S.fmtNum, v => v || '', S.LIMITS, true, doc5, () => {}, () => {}, () => {}, {}, S.rowStat, S.rowPaid);
        const eV = draw5(Wv, sysT, 'f_wp', rowsG, true); Wv.setView('hud'); edOf(eV, 'w_b1')[0].on.click(); const viewKept = Wv.getForm() && Wv.getForm().view === 'sheet';   // the HUD drawn after the sheet: ✎ on the sheet still opens the sheet's form
        const tF5 = formAt(W5, sysT, rowsG, true, 'w_b3', 'sheet', true), fRow = kids5(kids5(tF5.children[0])[1]).find(tr => tr.className === 'sheet-itemtable-form'), nHead = kids5(kids5(kids5(tF5.children[0])[0])[0]).length;
        check('F4c2 ✎ and the form\'s state: ✎ opens its row\'s form in this view and redraws the character (again: closes it); what is typed survives a redraw and goes once committed; Done closes it; in a table the form is a row spanning every column; no innerHTML',
            j(st1) === j({ charId: 'c_t', fieldId: 'f_wp', rowId: 'w_b1', view: 'sheet', typed: {} }) && j(rd1c) === j(['c_t']) && typedKept && typedGone && j(commits5) === j([['f_wp', { op: 'ov', rowId: 'w_b1', ov: { name: 'Typed' } }]]) && closed && doneNull && viewKept
            && !!fRow && kids5(fRow)[0].colSpan === nHead && nHead === 7 && find5(fRow, 'sheet-row-form').length === 1 && !/innerHTML/.test(itSrc5), j([st1, rd1c, typedKept, typedGone, commits5, nHead]));
        W5.setForm(null);
        // F4c2 review: a redraw that clears the sheet blurs the focused box — its change keeps the text typed and commits nothing; buildSections sets the flag around the clear and its own view after it
        const Wc = new Function(...deps5, itSrc5 + '\nreturn { itemListInto: itemListInto, setForm: function(v) { _rowForm = v; }, getForm: function() { return _rowForm; }, setClearing: function(v) { _rfClearing = v; } };')(fe5, (v, cls) => fe5('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, (c, f, q) => commits5.push([f.id, JSON.parse(j(q))]), opt5, S.fmtNum, v => v || '', S.LIMITS, true, doc5, id => redraws5.push(id), () => {}, () => {}, {}, S.rowStat, S.rowPaid, 'sheet');
        Wc.setForm({ charId: 'c_t', fieldId: 'f_wp', rowId: 'w_b1', view: 'sheet', typed: {} }); const eC = draw5(Wc, sysT, 'f_wp', rowsG, true), nbC = walk5(eC, partIs('rf-w_b1-name'), [])[0], sbC = walk5(eC, x => x.dataset && /^rf-w_b1-s-/.test(x.dataset.part || '') && x.tag === 'input', [])[0];
        commits5.length = 0; Wc.setClearing(true); nbC.value = 'Half'; nbC.on.change(); if (sbC) { sbC.value = '7'; sbC.on.change(); } const heldTyped = j(Wc.getForm().typed), nHeld = commits5.length; Wc.setClearing(false); nbC.value = 'Whole'; nbC.on.change();
        check('F4c2 review: a redraw that clears the sheet (a blur on the focused box) commits nothing half-typed — the name and a stat stay typed in the form; once cleared a change commits as before; buildSections sets the flag around its clear and its own view after it (a redraw the blur started leaves this view as it was)',
            !!nbC && !!sbC && nHeld === 0 && JSON.parse(heldTyped)['rf-w_b1-name'] === 'Half' && Object.keys(JSON.parse(heldTyped)).some(k => /^rf-w_b1-s-/.test(k)) && j(commits5) === j([['f_wp', { op: 'ov', rowId: 'w_b1', ov: { name: 'Whole' } }]])
            && /    _rfClearing = true; try \{ body\.textContent = ''; \} finally \{ _rfClearing = false; \}[^\n]*\n    _fxView = hudV \? 'hud' : 'sheet';/.test(shT), j([heldTyped, nHeld, commits5]));
        Wc.setForm(null); commits5.length = 0;

        const bitsOf = (ov, id) => { const h = fe5('span'), e = { id: 'w_x', defId: id, qty: 1, ov }; W5.gmItemBits(h, S.rowDef(sysT, e).def, e, true, true); return kids5(h).map(k => k.textContent); };
        check('F4c2 the GM\'s chips read the copy\'s own locks: its bound on a free entry, none on a bound one (nothing), its switch lock',
            j(bitsOf({ rm: 'bound' }, 'i_blaster')) === j(['bound']) && j(bitsOf({ rm: 'none', eq: 'none' }, 'i_chain')) === j([]) && j(bitsOf(undefined, 'i_chain')) === j(['bound', 'stays on']) && j(bitsOf({ eq: 'curse' }, 'i_blaster')) === j(['curse (on)']));

        // the System editor: the Rules box (renderLists, real), the handlers and the save (pinned)
        const nodes = { sysListRows: fe5('div'), sysListRules: fe5('div') };
        const RL = new Function('ui', 'draft', 'el', 'listCard', 'errorCell', 'labeledSelect', cutT('function renderLists(', 'function listCard(') + '\nreturn renderLists;');
        const lSel = (cls, cap, options, value, title) => { const l = fe5('label', 'sys-combat-item'), s = fe5('select', cls); l.appendChild(fe5('span', 'sys-num-cap', cap)); options.forEach(o => s.appendChild(opt5(o[0], o[1], o[0] === value))); s.value = value; s.title = title; l.appendChild(s); return l; };
        const runRL = dr => { nodes.sysListRows = fe5('div'); nodes.sysListRules = fe5('div'); RL(id => nodes[id], dr, fe5, () => fe5('div', 'sys-list-card'), () => fe5('div'), lSel)(); return nodes.sysListRules; };
        const rOn = runRL({ fields: [{ id: 'f_wp', kind: 'item-list' }], listRules: { ownerStats: true } }), rOff = runRL({ fields: [{ id: 'f_wp', kind: 'item-list' }] }), rNone = runRL({ fields: [{ id: 'f_n', kind: 'number' }], listRules: { ownerStats: true } });
        const rSel = find5(rOn, 'sys-listrules-stats')[0];
        check('F4c2 the Lists tab\'s Rules box (renderLists, run for real): Item stats on players\' sheets — GM only, or Players may change them (the draft\'s setting chosen); nothing without an item list; the handler writes draft.listRules or deletes it; a save logs and tells how far an item edit reaches (itemReach after the re-check, before the save); a refused ov says why; the pins of the save and of Revert hold',
            !!rSel && j(kids5(rSel).map(o => [o.value, o.textContent, !!o.selected])) === j([['', 'GM only', false], ['owner', 'Players may change them', true]]) && find5(rOn, 'sys-num-cap')[0].textContent === 'Item stats on players’ sheets' && find5(rOff, 'sys-listrules-stats')[0].value === '' && kids5(rNone).length === 0
            && /if \(c\.indexOf\('sys-listrules-stats'\) >= 0\) \{ if \(t\.value === 'owner'\) draft\.listRules = \{ ownerStats: true \}; else delete draft\.listRules; markDirty\(\); patchErrors\(\); return; \}/.test(shT)
            && /reCleanChars\(camp, clean\);[^\n]*\n[^\n]*stampRows\(clean, camp\.chars \|\| \{\}\);[^\n]*\n\s*var reach = itemReach\(prevSys, clean, camp\.chars \|\| \{\}\), nR = net\(\); if \(nR && nR\.logEvent\) reach\.lines\.forEach\(function\(l\) \{ nR\.logEvent\('items', l\); \}\);[\s\S]{0,400}?\n\s*save\(true\);/.test(shT)
        ...
        {
            const sysM = cleanSystem({ v: 1, name: 'M', rolls: [], fields: [{ id: 'f_g', key: 'Gear', label: 'Gear', kind: 'item-list', edit: 'owner', vis: 'all', list: { stats: [{ key: 'Cost' }], price: 'Cost' } }], items: [{ id: 'i_ring', name: 'Ring', rm: 'bound', stats: { Cost: 500 } }, { id: 'i_rope', name: 'Rope', stats: { Cost: 2 } }], listRules: { ownerStats: true } }, { F, gmView: true });
            const pvM = cleanSystem(sysM, { F, gmView: false }), chM = rows => ({ id: 'c_m', ownerId: 'u_m', values: { f_g: rows } });
            const a1 = S.applyRowOp(sysM, chM([{ id: 'w_r', defId: 'i_ring', qty: 1, ov: { rm: 'none' } }]), 'f_g', { op: 'add', defId: 'i_ring' }, F, { player: true, view: pvM });
            const a2 = S.applyRowOp(sysM, chM([{ id: 'w_p', defId: 'i_rope', qty: 1, ov: { name: 'Snare', rm: 'bound' } }]), 'f_g', { op: 'add', defId: 'i_rope' }, F, { player: true, view: pvM });
            const a3 = S.applyRowOp(sysM, chM([]), 'f_g', { op: 'add', defId: 'i_ring' }, F, { player: true, view: pvM });
            const o1 = S.applyRowOp(sysM, chM([{ id: 'w_r', defId: 'i_ring', qty: 1 }]), 'f_g', { op: 'ov', rowId: 'w_r', ov: { stats: { cost: 1 } } }, F, { player: true, view: pvM });
            const o2 = S.applyRowOp(sysM, chM([{ id: 'w_r', defId: 'i_ring', qty: 1, paid: 450 }]), 'f_g', { op: 'ov', rowId: 'w_r', ov: { stats: { Cost: 1 } } }, F, { player: true, view: pvM });
            const o3 = S.applyRowOp(sysM, chM([{ id: 'w_r', defId: 'i_ring', qty: 1 }]), 'f_g', { op: 'ov', rowId: 'w_r', ov: { stats: { Cost: 7 } } }, F, {});
            const hostRows = [{ id: 'w_r', defId: 'i_ring', qty: 1 }], cl = S.applyRowOp(pvM, chM(S.projectRows(hostRows, pvM, null, pvM.fields[0].list)), 'f_g', { op: 'ov', rowId: 'w_r', ov: { stats: { Cost: 1 } } }, F, { player: true, view: pvM });
            const rowOf = r => (r.value || []).find(x => x.id === 'w_r');
            check('F4c2 review: a pickup merged into a copy tells the GM that copy\'s lock — a copy the GM freed (none) gives no bound notice, a copy the GM bound gives one under its own name; a new row still reads the entry; a player\'s change of the price stat first records Paid as the row read it (500), a recorded Paid never moves, the GM\'s own change records none; the player\'s client records the same',
                a1.ok && a1.note === undefined && a2.ok && a2.note === 'bound' && a2.name === 'Snare' && a3.ok && a3.note === 'bound' && a3.name === 'Ring'
                && o1.ok && rowOf(o1).paid === 500 && j(rowOf(o1).ov) === j({ stats: { Cost: 1 } }) && o2.ok && rowOf(o2).paid === 450 && o3.ok && rowOf(o3).paid === undefined && cl.ok && rowOf(cl).paid === 500,
                j([a1.note, a2.note, a2.name, rowOf(o1), rowOf(o2), rowOf(o3), cl.ok && rowOf(cl)]));
        }
        // F4c2 review: the rename/remove warning counts a copy's own values; a list row's dot sits before its name; a copy's own category spelling is its own option
        {
            const drV = { v: 1, name: 'V', rolls: [], fields: [{ id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', vis: 'all', list: { stats: [{ key: 'Dmg' }] } }], items: [{ id: 'i_b', name: 'B' }] };
            const savedV = cleanSystem({ v: 1, name: 'V', rolls: [], items: [{ id: 'i_b', name: 'B' }], fields: [{ id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', vis: 'all', list: { stats: [{ key: 'Dmg' }, { key: 'Acc' }] } }] }, { F, gmView: true });
            const reSrcV = cutT('function refreshErrors(', 'function errorCell('), reV = new Function('draft', 'F', 'validKey', 'KINDS', 'cleanItemKey', 'cleanSystem', 'validateSystem', 'statKey', 'LIMITS', 'getActiveCampaign', 'systemOf', 'var errorsById = {}, warningsById = {};\n' + reSrcV + '\nrefreshErrors(); return errorsById;')(drV, () => F, S.validKey, S.KINDS, S.cleanItemKey, cleanSystem, validateSystem, S.statKey, S.LIMITS, () => ({ system: savedV, chars: { c_1: { values: { f_wp: [{ id: 'w_b', defId: 'i_b', qty: 1, ov: { stats: { Acc: 5 }, held: ['Acc'] } }] } } } }), c => c.system);
            const msgV = (reV['list:f_wp'] || []).map(e => e.message);
            const gDot = draw5(W5, sysT, 'f_wp', rowsG, true), nmDot = walk5(gDot, x => /(^| )sheet-item-name( |$)/.test(x.className || '') && kids5(x).some(k => (k.className || '') === 'sheet-ov'), [])[0];
            check('F4c2 review: removing a stat a copy holds a value of its own for (Acc on one copy, no item holding it) warns before Save drops it; a list row\'s dot is the name\'s first child (a long name is cut at its end, never the dot); the form offers a copy\'s own category spelling as its own choice',
                j(msgV) === j(['Save drops the “Acc” values of 1 character’s copy (a stat’s key is its identity).']) && !!nmDot && (kids5(nmDot)[0].className || '') === 'sheet-ov'
                && /if \(typeof ov\.category === 'string' && ov\.category && cats\.indexOf\(ov\.category\) < 0\) cats\.push\(ov\.category\);/.test(shT), j([msgV, nmDot && kids5(nmDot).map(k => k.className)]));
        }
        // ---- Stage 6 F4c3: custom rows — the custom op, own, D2 keys (judged on what the actor sees), + Custom…, the custom form, the GM's shared-key mark ----
        {
            const rawK = { v: 1, name: 'K', rolls: [], fields: [
                { id: 'f_sk', key: 'Skills', label: 'Skills', kind: 'item-list', edit: 'owner', vis: 'all', list: { custom: true, cats: ['Skill'], lvl: { label: 'Level', min: 0, max: 20, def: 1 }, stats: [{ key: 'Rel' }] } },
                { id: 'f_pl', key: 'Plain', label: 'Plain', kind: 'item-list', edit: 'owner', vis: 'all' }],
                items: [{ id: 'i_karate', name: 'Karate', category: 'Skill', key: 'Karate' }, { id: 'i_hidden', name: 'Hidden', category: 'Skill', key: 'Hidden', vis: 'gm' }, { id: 'i_axe', name: 'Axe', category: 'Weapon', key: 'Axe' }] };
            const sysK = cleanSystem(rawK, { F, gmView: true }), pvK = cleanSystem(sysK, { F, gmView: false }), libK = {}; sysK.items.forEach(i => { libK[i.id] = i; });
            const rawN = JSON.parse(j(rawK)); delete rawN.fields[0].list.custom; const sysN = cleanSystem(rawN, { F, gmView: true }), pvN = cleanSystem(sysN, { F, gmView: false });
            const chK = rows => ({ id: 'c_k', ownerId: 'u_k', values: { f_sk: rows } });
            const RO = (rows, q, pl, sys, view) => S.applyRowOp(sys || sysK, chK(JSON.parse(j(rows))), 'f_sk', Object.assign({ op: 'custom' }, q), F, pl ? { player: true, view: view || pvK } : {});
            const rowIn = (r, id) => (r.value || []).find(x => x.id === id), mineRow = { id: 'w_o', qty: 1, own: 1, def: { name: 'Mine', damage: '1d6', vis: 'all' } };
            const c1 = RO([], { rowId: 'w_new1', def: {} }, false), c2 = RO([], { rowId: 'w_new2', def: { name: 'Pazaak', key: 'Pazaak', stats: { rel: 2 } } }, true);
            const c3 = RO([], { rowId: 'w_new3', def: {} }, true, sysN, pvN), c4 = RO([], { rowId: 'w_karate', def: {} }, false), full = Array.from({ length: S.LIMITS.carried }, (_, i) => ({ id: 'w_f' + i, qty: 1, def: { name: 'F' + i } })), c5 = RO(full, { rowId: 'w_new5', def: {} }, false);
            const c6 = RO([], { rowId: 'w_new6', def: { damage: '2d6' } }, true), c7 = RO([], { rowId: 'w_new7', def: { vis: 'gm' } }, true), c8 = RO([mineRow], { rowId: 'w_o', def: { name: 'Mine2' } }, true);
            const c9 = RO([{ id: 'w_g', qty: 1, def: { name: 'GMs' } }], { rowId: 'w_g', def: { name: 'X' } }, true), c10 = RO([Object.assign({}, mineRow, { hid: 1 })], { rowId: 'w_o', def: { name: 'X' } }, true);
            const c11 = RO([mineRow], { rowId: 'w_o', def: { name: 'X' } }, true, sysN, pvN), c12 = RO([mineRow], { rowId: 'w_o', def: {} }, true), c13 = RO([Object.assign({}, mineRow, { def: { name: 'Mine', vis: 'gm' } })], { rowId: 'w_o', def: { name: 'X' } }, true);
            const c14 = RO([{ id: 'w_k1', defId: 'i_karate', qty: 1 }], { rowId: 'w_k1', def: { name: 'X' } }, false), c15 = RO([], { rowId: 'w_new15', def: { name: 'Hex', rm: 'curse', damage: '1d4 + 1', area: { ft: 5 } } }, false);
            check('F4c3 the custom op: the GM makes a blank row (New item, the list\'s level default, added 1 and its qty: the pickup\'s Undo) on any shaped list, a player one of their own (own, visible) only where Custom rows is ticked; a derived id or a full list is refused; a player sends only name, icon, category, notes, key and stats, changes only a row they made (never one the GM made or made GM only; a row kept from them is gone), and their change keeps the GM\'s fields on it; a create replayed is a patch of that row; a library copy changes with ov; the GM sets a blast, formulas and a lock',
                c1.ok && c1.added === 1 && c1.qty === 1 && rowIn(c1, 'w_new1').def.name === 'New item' && rowIn(c1, 'w_new1').lvl === 1 && rowIn(c1, 'w_new1').own === undefined
                && c2.ok && rowIn(c2, 'w_new2').own === 1 && rowIn(c2, 'w_new2').def.name === 'Pazaak' && rowIn(c2, 'w_new2').def.key === 'Pazaak' && j(rowIn(c2, 'w_new2').def.stats) === j({ Rel: 2 })
                && !c3.ok && c3.reason === 'field' && !c4.ok && c4.reason === 'value' && !c5.ok && c5.reason === 'field' && !c6.ok && c6.reason === 'field' && !c7.ok && c7.reason === 'field'
                && c8.ok && c8.qty === 1 && rowIn(c8, 'w_o').def.name === 'Mine2' && rowIn(c8, 'w_o').def.damage === '1d6' && rowIn(c8, 'w_o').own === 1
                && !c9.ok && c9.reason === 'field' && !c10.ok && c10.reason === 'missing' && !c11.ok && c11.reason === 'field' && c12.ok && c12.value.filter(r => r.id === 'w_o').length === 1 && rowIn(c12, 'w_o').def.name === 'Mine' && c12.added === undefined
                && !c13.ok && c13.reason === 'field' && !c14.ok && c14.reason === 'field' && c15.ok && rowIn(c15, 'w_new15').def.rm === 'curse' && rowIn(c15, 'w_new15').def.damage === '1d4 + 1' && rowIn(c15, 'w_new15').def.area.ft === 5,
                j([c1, c2.ok && rowIn(c2, 'w_new2'), c3, c4, c5.reason, c6, c7, c8.ok && rowIn(c8, 'w_o'), c9, c10, c11, c12.ok && c12.value, c13, c14, c15.ok && rowIn(c15, 'w_new15')]));
            const keyed = [{ id: 'w_z', qty: 1, own: 1, def: { name: 'Z', key: 'Zap' } }, { id: 'w_gs', qty: 1, def: { name: 'S', key: 'Sneaky', vis: 'gm' } }];
            const k1 = RO([], { rowId: 'w_n1', def: { key: 'Karate' } }, true), k2 = RO([], { rowId: 'w_n2', def: { key: 'karate' } }, true), k3 = RO(keyed, { rowId: 'w_n3', def: { key: 'zap' } }, true), k4 = RO([], { rowId: 'w_n4', def: { key: 'Hidden' } }, true);
            const k5 = RO([], { rowId: 'w_n5', def: { key: '1x' } }, true), k6 = RO([], { rowId: 'w_n6', def: { key: 'max' } }, true), k7 = RO([], { rowId: 'w_n7', def: { key: 'Axe' } }, true), k8 = RO([], { rowId: 'w_n8', def: { key: 'Hidden' } }, false), k9 = RO(keyed, { rowId: 'w_n9', def: { key: 'Sneaky' } }, true), k10 = RO(keyed, { rowId: 'w_z', def: { key: 'Zap' } }, true);
            check('F4c3 D2 keys: a custom row\'s key is a usable item key no row or item of the list the actor can see has — a visible entry\'s (any case) or a visible row\'s is taken (why key), 1x and max are not usable (why badkey); for a player a GM-only entry\'s or a GM-only row\'s key is free (never confirmed), the GM\'s is taken; an item outside the list\'s categories does not count; the row\'s own key is not a clash with itself; the refusal says only value (why never travels)',
                !k1.ok && k1.reason === 'value' && k1.why === 'key' && !k2.ok && k2.why === 'key' && !k3.ok && k3.why === 'key' && k4.ok && rowIn(k4, 'w_n4').def.key === 'Hidden' && !k5.ok && k5.why === 'badkey' && !k6.ok && k6.why === 'badkey'
                && k7.ok && !k8.ok && k8.why === 'key' && k9.ok && k10.ok, j([k1, k2, k3, k4.ok, k5, k6, k7.ok, k8, k9.ok, k10.ok]));
            const spK = pvK.fields.find(f => f.id === 'f_sk').list, agree = (hostRows, q) => { const h = RO(hostRows, q, true), c = S.applyRowOp(pvK, chK(S.projectRows(JSON.parse(j(hostRows)), pvK, libK, spK)), 'f_sk', Object.assign({ op: 'custom' }, q), F, { player: true, view: pvK }); return h.ok === c.ok && (!h.ok ? h.reason === c.reason : j(S.projectRows(h.value, pvK, libK, spK)) === j(c.value)); };
            ...
            const ciK = S.cleanCharItem({ type: 'char-item', rid: 'r1', charId: 'c_k', fieldId: 'f_sk', op: 'custom', rowId: 'w_x', def: { name: 'A', damage: '1d6', vis: 'gm', foo: 1 } }), ciK2 = S.cleanCharItem({ type: 'char-item', rid: 'r2', charId: 'c_k', fieldId: 'f_sk', op: 'custom', rowId: 'w_x', def: {} });
            const ciK3 = S.cleanCharItem({ type: 'char-item', rid: 'r3', charId: 'c_k', fieldId: 'f_sk', op: 'custom', rowId: 'w_x', def: { notes: 'n'.repeat(801) } }), ciK4 = S.cleanCharItem({ type: 'char-item', rid: 'r4', charId: 'c_k', fieldId: 'f_sk', op: 'custom', rowId: 'w_x', def: JSON.parse('{"stats":{"constructor":1}}') }), ciK5 = S.cleanCharItem({ type: 'char-item', rid: 'r5', charId: 'c_k', fieldId: 'f_sk', op: 'custom', rowId: 'w_x' });
            check('F4c3 cleaners: Custom rows only as true; own kept on a custom row, never on an inline copy; the owner\'s copy carries own and never a custom row\'s damage, lock or (for a GM-made row) anything of the GM\'s; a custom message keeps the known keys (the GM\'s too, so a player sending one hears field) and drops unknown ones, allows {} (a new row), refuses an over-long text, a prototype stat key and a missing def',
                lsK.custom === true && lsK2 && lsK2.custom === undefined && cvK.find(r => r.id === 'w_o').own === 1 && cvK.find(r => r.id === 'w_i').own === undefined
                && pjK[0].own === 1 && !/1d6|bound/.test(j(pjK)) && pjK[1].own === undefined && !!ciK && j(ciK.def) === j({ name: 'A', vis: 'gm', damage: '1d6' }) && !!ciK2 && j(ciK2.def) === j({}) && ciK3 === null && ciK4 === null && ciK5 === null,
                j([lsK, lsK2, cvK, pjK, ciK && ciK.def, ciK2, ciK3, ciK4, ciK5]));
            // the sheet (the real widgets on the fake DOM): ✎ and the custom form per who; + Custom… and the tick (pinned); the GM's shared-key mark
            const rawTC = JSON.parse(j(rawT)); rawTC.fields[0].list.custom = true; const sysTC = cleanSystem(rawTC, { F, gmView: true }), pvTC = cleanSystem(sysTC, { F, gmView: false });
            const rowsC = clT(seedT()).concat([{ id: 'w_own', qty: 1, own: 1, def: { name: 'Mine', key: 'Pazaak', stats: { Acc: 1 } } }, { id: 'w_gk', qty: 1, def: { name: 'GM key', key: 'Pazaak', vis: 'gm' } }]);
            const rowsCP = S.projectRows(JSON.parse(j(rowsC)), pvTC, libT, pvTC.fields.find(f => f.id === 'f_wp').list);
            const gC = draw5(W5, sysTC, 'f_wp', rowsC, true), oC = draw5(W5, pvTC, 'f_wp', rowsCP, false), oNo = draw5(W5, pvT, 'f_wp', S.projectRows(JSON.parse(j(rowsC)), pvT, libT, pT('f_wp').list), false);
            const shareG = find5(lineOf(gC, 'w_own') || fe5('x'), 'sheet-item-keyshare'), shareO = find5(oC, 'sheet-item-keyshare');
            const fOwn = formAt(W5, pvTC, rowsCP, false, 'w_own'), fOwnCaps = find5(fOwn, 'sheet-rf-cap').map(x => x.textContent), fGm = formAt(W5, sysTC, rowsC, true, 'w_c'), fGmCaps = find5(fGm, 'sheet-rf-cap').map(x => x.textContent);
            commits5.length = 0; const nbO = walk5(fOwn, partIs('rf-w_own-name'), [])[0]; nbO.value = 'Pazaak deck'; nbO.on.change(); const kbO = walk5(fOwn, partIs('rf-w_own-key'), [])[0]; kbO.value = ' Deck '; kbO.on.change(); const commitsO = commits5.slice(); commits5.length = 0; W5.setForm(null);
            check('F4c3 the sheet: the GM\'s ✎ on every custom row ("Change it"), its owner\'s on a row they made while the list takes custom rows (never a row the GM made, never with Custom rows off); the owner\'s form holds name, icon, category, notes, key and the stats (no GM field), the GM\'s adds the blast, the formulas, the two locks and GM only; each box sends one custom op of that field (a key trimmed); a custom row\'s key a row its owner cannot see also has is marked on the GM\'s sheet only',
                edOf(gC, 'w_c').length === 1 && edOf(gC, 'w_own').length === 1 && edOf(oC, 'w_own').length === 1 && edOf(oC, 'w_own')[0].title === 'Change it' && edOf(oC, 'w_c').length === 0 && edOf(oNo, 'w_own').length === 0
                && j(fOwnCaps) === j(['Name', 'Icon', 'Category', 'Notes', 'Key', 'Acc', 'Dmg', 'Cost', 'Wt', 'diff']) && find5(fOwn, 'sheet-row-custom').length === 1
                && ['Blast ft', 'Damage', 'Cost formula', 'When a player removes it', 'GM only'].every(c => fGmCaps.indexOf(c) >= 0) && fOwnCaps.indexOf('Damage') < 0
        ...
        {
            const rawA = { v: 1, name: 'A', rolls: [], fields: [
                { id: 'f_sk', key: 'Skills', label: 'Skills', kind: 'item-list', edit: 'owner', vis: 'all', list: { noQty: true, multi: true, custom: true, cats: ['Skill'], on: { label: 'Known' }, lvl: { label: 'Level', min: 0, max: 20, def: 1 }, stats: [{ key: 'Rel', show: true }, { key: 'Diff', labels: ['E', 'A', 'H'] }],
                    cols: [{ key: 'Cost', label: 'Cost', formula: 'Row.lvl * 2 + Row.Rel', unit: 'pts', foot: true }, { key: 'Twice', label: 'Twice', formula: 'Row.Cost * 2', hide: true, foot: true }, { key: 'Band', label: 'Band', formula: 'Row.lvl > 3', labels: ['low', 'high'] }] } },
                { id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', edit: 'owner', vis: 'all', list: { on: { label: 'Readied' }, price: 'Cost', stats: [{ key: 'Wt', show: true }, { key: 'Cost' }], cols: [{ key: 'Hit', label: 'Hit', formula: 'Skills.Blasters.lvl + GMFig' }] } },
                { id: 'f_se', key: 'Secret', label: 'Secret', kind: 'item-list', edit: 'owner', vis: 'gm', list: { stats: [{ key: 'Heat' }] } },
                { id: 'f_gm', key: 'GMFig', label: 'GM figure', kind: 'number', vis: 'gm', def: 5 },
                { id: 'f_sec', key: 'Sec', label: 'Sec', kind: 'number', vis: 'gm', def: 1 }, { id: 'f_sk2', key: 'Sec.Known', label: 'SK', kind: 'number', vis: 'all', def: 2 }, { id: 'f_ud', key: 'UsesDot', label: 'U', kind: 'formula', vis: 'all', formula: 'Sec.Known + 1' },
                { id: 'f_pts', key: 'Pts', label: 'Pts', kind: 'formula', vis: 'all', formula: 'Skills.Cost + Skills.count' },
                { id: 'f_kl', key: 'KarL', label: 'KarL', kind: 'formula', vis: 'all', formula: 'Skills.Karate.lvl * 10 + Skills.Karate.has' },
                { id: 'f_hl', key: 'HushL', label: 'HushL', kind: 'formula', vis: 'all', formula: 'Skills.Hush.lvl' },
                { id: 'f_sn', key: 'SecN', label: 'SecN', kind: 'formula', vis: 'all', formula: 'Secret.count' },
                { id: 'f_ld', key: 'Load', label: 'Load', kind: 'formula', vis: 'all', formula: 'Weapons.Wt + Weapons.on.qty * 100 + Weapons.paid / 1000' },
                { id: 'f_mx', key: 'Bad', label: 'Bad', kind: 'formula', vis: 'all', formula: 'Skills.max' }],
                items: [{ id: 'i_kar', name: 'Karate', category: 'Skill', key: 'Karate', lvl: 6, stats: { Rel: 3 } }, { id: 'i_bl', name: 'Blasters', category: 'Skill', key: 'Blasters', stats: { Rel: 1 } }, { id: 'i_hush', name: 'Hush', category: 'Skill', key: 'Hush', vis: 'gm', stats: { Rel: 9 } },
                    { id: 'i_axe', name: 'Axe', category: 'Weapon', key: 'Axe', stats: { Wt: 4, Cost: 50 } }, { id: 'i_bld', name: 'Blade', category: 'Weapon', key: 'Blade', lvl: 9 }] };
            const sysA = cleanSystem(rawA, { F, gmView: true }), pvA = cleanSystem(sysA, { F, gmView: false }), libA = {}; sysA.items.forEach(i => { libA[i.id] = i; });
            const chA = { id: 'c_a', ownerId: 'u_a', values: { f_sk: [{ id: 'w_k', defId: 'i_kar', qty: 1, lvl: 4, on: true }, { id: 'w_c', qty: 1, own: 1, def: { name: 'Pazaak', key: 'Paz', stats: { Rel: -1 } } }, { id: 'w_hid', qty: 1, hid: 1, def: { name: 'Kept', key: 'Kept', stats: { Rel: 50 } } }, { id: 'w_h', defId: 'i_hush', qty: 1, lvl: 2 }],
                f_wp: [{ id: 'w_a', defId: 'i_axe', qty: 3, on: true, paid: 40 }, { id: 'w_a2', defId: 'i_axe', qty: 1 }] } };
            const lsA = S.cleanListSpec({ stats: [{ key: 'Rel' }], cols: [{ key: 'rel', formula: '1' }, { key: 'Cost' }, { key: 'X', formula: null }, { key: 'max', formula: '1' }].concat(Array.from({ length: 7 }, (_, i) => ({ key: 'C' + i, formula: '1' }))) }, true, F);
            check('F5a1 list columns (cleanListSpec): at most six, a key by the stat rules sharing one namespace with the list\'s stats (rel repeats Rel: dropped; max refused), a missing formula kept blank, a blanked one (null) kept null (the players\' view is a fixed point), unit, value names, Hidden and Total only as set; a list without columns has no cols',
                lsA.cols.length === 6 && j(lsA.cols.slice(0, 2)) === j([{ key: 'Cost', label: 'Cost', formula: '' }, { key: 'X', label: 'X', formula: null }]) && !lsA.cols.some(c => /^(rel|max)$/i.test(c.key)) && j(sysA.fields[0].list.cols[0]) === j({ key: 'Cost', label: 'Cost', formula: 'Row.lvl * 2 + Row.Rel', unit: 'pts', foot: true })
                && sysA.fields[0].list.cols[1].hide === true && j(sysA.fields[0].list.cols[2].labels) === j(['low', 'high']) && sysA.fields[2].list.cols === undefined && j(S.cleanListSpec({ cats: ['Skill'] }, true, F)) === j({ cats: ['Skill'] }), j([lsA, sysA.fields[0].list.cols]));
            const allG = S.resolveAll(sysA, chA, F), valG = id => allG[id].error ? 'ERR ' + allG[id].error : allG[id].value;
            const chO = S.charFor(chA, pvA, 'u_a', { lib: {}, items: libA }), allO = S.resolveAll(pvA, chO, F), valO = id => allO[id] ? (allO[id].error ? 'ERR ' + allO[id].error : allO[id].value) : 'absent';
            check('F5a1 the resolver: Skills.Cost sums a column over the rows carried (a kept curse counts nowhere), Skills.count counts them; an addressed row reads its level and has (a GM-only entry\'s row: the GM\'s sheet only); Weapons.Wt is stat × qty, Weapons.on.qty the rows switched on, Weapons.paid what was paid × qty; a word no list offers (Skills.max) is unknown; a list column reading a GM-only field works on the GM\'s sheet',
                valG('f_pts') === (4 * 2 + 3) + (1 * 2 - 1) + (2 * 2 + 9) + 3 && valG('f_kl') === 41 && valG('f_hl') === 2 && valG('f_sn') === 0 && valG('f_ld') === (4 * 3 + 4 * 1) + 300 + (40 * 3 + 50 * 1) / 1000 && /Unknown name/.test(valG('f_mx'))
                && allG.f_wp.cells.w_a[0].value === 1 + 5, j([valG('f_pts'), valG('f_kl'), valG('f_hl'), valG('f_ld'), valG('f_mx'), allG.f_wp.cells]));
            check('F5a1 resolveAll: a list with columns gets each row\'s cells (value, text, a value name, an error as a dash) and its totals (a column with Total, a stat shown on the row); a list without columns gets neither; the hover card works no rows out',
                j(allG.f_sk.cells.w_k.map(c => c.text)) === j(['11', '22', 'high']) && j(allG.f_sk.cells.w_c.map(c => c.text)) === j(['1', '2', 'low']) && !!allG.f_sk.cells.w_hid && allG.f_sk.foot.Cost.value === 11 + 1 + 13 && allG.f_sk.foot.Twice.value === 50 && allG.f_sk.foot.Rel.value === 3 - 1 + 9 && allG.f_sk.foot.Band === undefined
                && !('cells' in allG.f_se) && Array.isArray(S.hoverLines(sysA, chA, F)), j([allG.f_sk.cells, allG.f_sk.foot]));
            const pvCols = pvA.fields.find(x => x.id === 'f_wp').list.cols, pvF = id => pvA.fields.find(x => x.id === id);
            check('F5a1 the players\' view: a visible formula naming a GM-only list by its first segment (Secret.count) or a GM-only entry\'s key in a list (Skills.Hush.lvl) is blanked, and so is a list column reading a GM-only field (it reads "GM only"); a visible formula reading a visible dotted key next to a GM-only one (Sec.Known) is kept (critic 5), and so is one naming a visible entry (Skills.Karate.lvl); the view is a fixed point and names nothing GM-only',
                pvF('f_sn').formula === null && pvF('f_hl').formula === null && pvCols[0].formula === null && pvF('f_ud').formula === 'Sec.Known + 1' && pvF('f_kl').formula === 'Skills.Karate.lvl * 10 + Skills.Karate.has' && pvF('f_pts').formula === 'Skills.Cost + Skills.count'
                && j(cleanSystem(pvA, { F, gmView: false })) === j(pvA) && !/Skills\.Hush|GMFig|Secret\.count|i_hush/.test(j(pvA)) && /GM only/.test(valO('f_sn')) && allO.f_wp.cells.w_a[0].error === 'GM only', j([pvF('f_sn'), pvF('f_hl'), pvCols, pvF('f_ud')]));
            check('F5a1 the owner\'s sheet agrees with the GM\'s on every visible name: the same totals (a GM-only entry\'s row counts on both — inline for the owner — and a kept curse on neither), the same addressed levels, the same cells',
                valO('f_pts') === valG('f_pts') && valO('f_kl') === valG('f_kl') && valO('f_ld') === valG('f_ld') && j(allO.f_sk.cells.w_k) === j(allG.f_sk.cells.w_k) && allO.f_sk.foot.Cost.value === allG.f_sk.foot.Cost.value && allO.f_sk.cells.w_hid === undefined,
                j([valO('f_pts'), valG('f_pts'), valO('f_kl'), valO('f_ld'), allO.f_sk.foot]));
            const gmN = S.gmOnlyNames(sysA, [{ name: 'Secret.count' }, { name: 'Skills.Hush.lvl' }, { name: 'Skills.Karate.lvl' }, { name: 'Skills.Cost' }]), gmD = S.gmDerivedNames(sysA, F, [{ name: 'Weapons.Hit' }, { name: 'Skills.Cost' }]);
            check('F5a1 the GM\'s public roll: a GM-only list and a GM-only entry\'s key are GM-only names (critic 4); a list whose column reads a GM-only field is derived from one (its totals too), a list whose columns read none is not',
                j(gmN) === j(['Secret.count', 'Skills.Hush.lvl']) && j(gmD) === j(['Weapons.Hit']), j([gmN, gmD]));
            const chR = JSON.parse(j(chA)); chR.values.f_sk.unshift({ id: 'w_gh', qty: 1, lvl: 2, def: { name: 'Secret hush', key: 'Hush', vis: 'gm' } }); chR.values.f_sk.push({ id: 'w_vh', qty: 1, own: 1, lvl: 7, def: { name: 'My hush', key: 'Hush' } });
            const rG = S.makeResolver(sysA, chR, F), rA = S.makeResolver(sysA, chA, F);
            check('F5a1 addressed rows: a row its owner can see comes before a GM-only row with the same key, on every machine (critic 7: the GM\'s sheet reads 7, as its owner\'s does); an item in the list\'s scope that is not carried reads has false, qty 0 and its level (the list default); a key nothing has reads as not carried',
                rG('Skills.Hush.lvl') === 7 && rA('Skills.Blasters.has') === false && rA('Skills.Blasters.qty') === 0 && rA('Skills.Blasters.lvl') === 1 && rA('Skills.Blasters.Rel') === 1 && rA('Skills.Nobody.has') === false && rA('Skills.Nobody.lvl') === 1 && rA('Skills.Karate.on') === true && rA('Skills.Blade.lvl') === 1 && rA('Weapons.Blade.lvl') === 0,
                j([rG('Skills.Hush.lvl'), rA('Skills.Blasters.has'), rA('Skills.Blasters.qty'), rA('Skills.Blasters.lvl'), rA('Skills.Nobody.lvl'), rA('Skills.Karate.on')]));
            const sysW = cleanSystem({ v: 1, name: 'W', rolls: [], fields: [{ id: 'f_g', key: 'Gear', label: 'Gear', kind: 'item-list', vis: 'all', edit: 'owner', list: { stats: [{ key: 'Wt', show: true }], cols: [{ key: 'Half', label: 'Half', formula: 'Row.Wt / 2', foot: true }] } }], items: [{ id: 'i_d', name: 'Dust', stats: { Wt: 0.0001 } }] }, { F, gmView: true });
            const allW = S.resolveAll(sysW, { id: 'c_w', values: { f_g: [{ id: 'w_d', defId: 'i_d', qty: 3 }] } }, F);
            check('F5a1 list values read as the sheet writes a stat: a total weight of 3 × 0.0001 is 0.0003 (never 0), a column\'s half of it 0.00005',
                allW.f_g.foot.Wt.text === '0.0003' && allW.f_g.cells.w_d[0].text === '0.00005' && allW.f_g.foot.Half.text === '0.00005', j([allW.f_g.foot, allW.f_g.cells]));
            const loopRaw = JSON.parse(j(rawA)); loopRaw.fields[0].list.cols = [{ key: 'A', label: 'A', formula: 'Row.B + 1' }, { key: 'B', label: 'B', formula: 'Row.A' }, { key: 'S', label: 'S', formula: 'Skills.S' }];
            const sysL = cleanSystem(loopRaw, { F, gmView: true }), allL = S.resolveAll(sysL, chA, F), vL = S.validateSystem(sysL, F), loopMsgs = vL.errors.filter(e => /loop/.test(e.message)).map(e => e.id + '|' + e.prop + '|' + e.message);
            check('F5a1 loops: two columns reading each other, and a column reading its own list\'s total, stop quickly with a readable message (the list, the item\'s name, the column — never a row id) on the sheet and before Save, under the list\'s card',
                /Skills\[Karate\]\.A/.test(allL.f_sk.cells.w_k[0].error) && !/#|w_k/.test(allL.f_sk.cells.w_k[0].error) && /loop/.test(allL.f_sk.cells.w_k[2].error) && loopMsgs.length >= 2 && loopMsgs.every(m => /^f_sk\|list\|/.test(m)) && loopMsgs.some(m => /Skills\.S \(column\)/.test(m)),
                j([allL.f_sk.cells.w_k, loopMsgs]));
            const vA = S.validateSystem(sysA, F), badRaw = JSON.parse(j(rawA)); badRaw.fields.push({ id: 'f_row', key: 'Row', kind: 'number', vis: 'all' }, { id: 'f_pre', key: 'Weapons.Extra', kind: 'number', vis: 'all' }, { id: 'f_ro', key: 'RowOut', kind: 'formula', vis: 'all', formula: 'Row.lvl' }, { id: 'f_ot', key: 'OnT', kind: 'formula', vis: 'all', formula: 'Skills.on.count + Secret.on.count' }, { id: 'f_nk', key: 'NoKey', kind: 'formula', vis: 'all', formula: 'Skills.Nobody.lvl' });
            badRaw.fields[0].list.cols.push({ key: 'Dice', label: 'D', formula: '1d6' }); badRaw.rolls.push({ id: 'r_long', label: 'R', formula: 'd20 + Skills.' + 'K'.repeat(60) + '.lvl', vis: 'all' });
            const vB = S.validateSystem(cleanSystem(badRaw, { F, gmView: true }), F), eB = vB.errors.map(e => e.id + '|' + e.prop + '|' + e.message), wB = vB.warnings.map(e => e.id + '|' + e.message);
            check('F5a1 the validator: the list names are known (a valid system\'s only error is the unknown Skills.max; its GM-only names warn); errors — Row outside a list\'s columns, dice in a column (under the list\'s card, "Column …"), a field keyed Row or starting with a list\'s key and a dot, a roll name past 64 characters; warnings — a GM-only list or a GM-only entry\'s key in a visible formula, a switched-on total on a list with no switch, a key no item in the list has',
                vA.errors.length === 1 && /Unknown name "Skills\.max"/.test(vA.errors[0].message) && vA.warnings.some(w => /Secret\.count/.test(w.message)) && vA.warnings.some(w => /Skills\.Hush\.lvl" names a GM-only item/.test(w.message))
                && eB.some(e => /^f_sk\|list\|Column “Dice”: Dice are not allowed/.test(e)) && eB.some(e => /^f_row\|key\|Row is the name/.test(e)) && eB.some(e => /^f_pre\|key\|A key starting "Weapons\."/.test(e)) && eB.some(e => /^r_long\|rollFormula\|A roll carries names of 64 characters at most/.test(e))
                && wB.some(w => /^f_nk\|No item in Skills has the key "Nobody"/.test(w)),
                j([vA.errors, eB, wB]));
            const rowOutRaw = JSON.parse(j(rawA)); rowOutRaw.fields.push({ id: 'f_ro', key: 'RowOut', kind: 'formula', vis: 'all', formula: 'Row.lvl + Weapons.on.count' }); const rowOutRaw2 = JSON.parse(j(rowOutRaw)); rowOutRaw2.fields[1].list = { stats: [{ key: 'Wt' }] };
            const vR = S.validateSystem(cleanSystem(rowOutRaw, { F, gmView: true }), F), vR2 = S.validateSystem(cleanSystem(rowOutRaw2, { F, gmView: true }), F);
            check('F5a1 the validator: Row.* outside a list\'s own columns is an error that says so; Weapons.on.count on a list with no switch warns',
                vR.errors.some(e => e.id === 'f_ro' && /names a row, so it is known only in a list/.test(e.message)) && vR2.warnings.some(w => w.id === 'f_ro' && /counts rows switched on, but Weapons has no switch/.test(w.message)), j([vR.errors, vR2.warnings]));
            // the sheet: a row's column chips and the totals line (the real widgets), the table's columns and footer sums; the Lists card's Columns (real)
            const Wc5 = mkW5(true, 'sheet'), fSk = sysA.fields[0], gSk = fe5('div'); Wc5.itemListInto(gSk, fSk, chA, chA.values.f_sk, sysA, false, true, true, '', allG.f_sk);
            const lineK = kids5(gSk).find(l => walk5(l, x => x.dataset && x.dataset.part === 'ed-w_k', []).length > 0), colCh = find5(lineK, 'sheet-item-col'), totL = find5(gSk, 'sheet-item-totals')[0];
            const tSk = fe5('div'); Wc5.itemTableInto(tSk, Object.assign({}, fSk, { table: { footer: true } }), chA, chA.values.f_sk, sysA, false, true, true, '', allG.f_sk);
            const thd = kids5(kids5(kids5(tSk.children[0])[0])[0]).map(x => x.textContent), tft = kids5(kids5(kids5(tSk.children[0]).find(x => x.tag === 'tfoot'))[0]).map(x => x.textContent);
            const plainSk = fe5('div'); Wc5.itemListInto(plainSk, fSk, chA, chA.values.f_sk, sysA, false, true, true, '');
            check('F5a1 the sheet: a row shows each column not Hidden as a chip (its label small, its unit, a value name), the list its totals under the rows (the stat shown on the row, the columns with Total — a Hidden one too); a table adds a column each (headed with the unit) and sums them in the footer; drawn without the resolved entry, nothing of it shows',
                j(colCh.map(c => kids5(c).map(k => k.textContent).join(''))) === j(['Cost 11 pts', 'Band high']) && !!totL && j(find5(totL, 'sheet-item-total').map(c => kids5(c).map(k => k.textContent).join(''))) === j(['Rel 11', 'Cost 25 pts', 'Twice 50'])
                && thd.indexOf('Cost (pts)') > 0 && thd.indexOf('Band') > 0 && thd.indexOf('Twice') < 0 && tft.indexOf('25') > 0 && tft.indexOf('11') > 0 && find5(plainSk, 'sheet-item-col').length === 0 && find5(plainSk, 'sheet-item-totals').length === 0,
                j([colCh.map(c => kids5(c).map(k => k.textContent)), totL && kids5(totL).map(k => k.textContent), thd, tft]));
            const inp5 = (cls, v, t, ph) => { const i = fe5('input', cls); i.type = 'text'; i.value = v == null ? '' : String(v); i.title = t || ''; i.placeholder = ph || ''; return i; };
            const rN5 = new Function('el', cutT('function numInput(', 'function select(') + '\nreturn { numInput: numInput, numField: numField };')(fe5);
            const LC5 = new Function('el', 'input', 'numField', 'document', 'LIMITS', cutT('function listCard(', 'function renderCombat(') + '\nreturn listCard;')(fe5, inp5, rN5.numField, doc5, S.LIMITS);
            const cardA = LC5(JSON.parse(j(rawA.fields[0])), ['Skill']), colRows = find5(cardA, 'sys-list-col'), six = LC5({ id: 'f_s', key: 'S', label: 'S', kind: 'item-list', list: { cols: Array.from({ length: 6 }, (_, i) => ({ key: 'C' + i, formula: '1' })) } }, []);
            check('F5a1 the Lists card (run for real): a row per column (key up to 24, label, formula up to 300, unit, value names, Hidden, Total, ▲ ▼ ×), + Column (greyed at six), and a line naming what formulas can read of the list; the handlers write the draft (pinned)',
                colRows.length === 3 && j(find5(cardA, 'sys-list-colkey').map(x => x.value)) === j(['Cost', 'Twice', 'Band']) && find5(cardA, 'sys-list-colformula')[0].maxLength === 300 && find5(cardA, 'sys-list-colkey')[0].maxLength === 24
                && j(find5(cardA, 'sys-list-colhide').map(x => x.checked)) === j([false, true, false]) && j(find5(cardA, 'sys-list-colfoot').map(x => x.checked)) === j([true, true, false]) && find5(cardA, 'sys-list-coladd')[0].disabled === false && find5(six, 'sys-list-coladd')[0].disabled === true
                && /^Formulas read Skills\.count, Skills\.qty, Skills\.Rel, Skills\.Diff, Skills\.Cost, Skills\.Twice, Skills\.Band \(the list’s totals\); Skills\.on\.… the same over the rows switched on; Skills\.<key>\.lvl/.test(find5(cardA, 'sys-list-names')[0].textContent)
                && /else if \(lcc\.indexOf\('sys-list-colformula'\) >= 0\) cD\.formula = t\.value\.slice\(0, LIMITS\.formula\);/.test(shT) && /if \(cact === 'coladd'\) \{ if \(cArr\.length >= LIMITS\.listCols\)/.test(shT) && /var chK = c\.indexOf\('sys-list-colhide'\) >= 0 \? 'hide' : 'foot';/.test(shT)
        ...
        // ---- Stage 6 F5a2: choice stats (a pick of named options; stored as the label, never the name; a GM-only option never reaches players) ----
        {
            ...
            const inpP = (cls, v, t, ph) => { const i = fe5('input', cls); i.type = 'text'; i.value = v == null ? '' : String(v); i.title = t || ''; i.placeholder = ph || ''; return i; };
            const rNP = new Function('el', cutT('function numInput(', 'function select(') + '\nreturn { numInput: numInput, numField: numField };')(fe5);
            const LC5 = new Function('el', 'input', 'numField', 'document', 'LIMITS', cutT('function listCard(', 'function renderCombat(') + '\nreturn listCard;')(fe5, inpP, rNP.numField, doc5, S.LIMITS);
            const rawP = { v: 1, name: 'P', rolls: [], listRules: { ownerStats: true }, fields: [
                { id: 'f_st', key: 'ST', label: 'ST', kind: 'number', vis: 'all', def: 12 }, { id: 'f_dx', key: 'DX', label: 'DX', kind: 'number', vis: 'all', def: 10 }, { id: 'f_gm', key: 'GMFig', label: 'G', kind: 'number', vis: 'gm', def: 5 },
                { id: 'f_sk', key: 'Skills', label: 'Skills', kind: 'item-list', vis: 'all', edit: 'owner', list: { lvl: { label: 'Level', min: 0, max: 20, def: 1 }, custom: true, price: 'Attr', stats: [{ key: 'Attr', kind: 'pick', show: true, def: 'strength', opts: [{ label: 'Strength', name: 'ST' }, { label: 'Dexterity', name: 'DX' }, { label: 'Secret', name: 'GMFig' }, { label: 'strength', name: 'DX' }, { label: 'Bad', name: '1+1' }, { label: 'Empty', name: '' }] }], cols: [{ key: 'Eff', label: 'Eff', formula: 'Row.Attr + Row.lvl' }] } },
                { id: 'f_ka', key: 'KarAttr', label: 'K', kind: 'formula', vis: 'all', formula: 'Skills.Karate.Attr' }],
                items: [{ id: 'i_kar', name: 'Karate', key: 'Karate', stats: { Attr: 'dexterity' } }, { id: 'i_sn', name: 'Sneak', key: 'Sneak', stats: { Attr: 'Secret' } }, { id: 'i_x', name: 'X', stats: { Attr: 'Nope' } }, { id: 'i_n', name: 'N', stats: { Attr: 3 } }] };
            const sysP = cleanSystem(rawP, { F, gmView: true }), pvP = cleanSystem(sysP, { F, gmView: false }), spP = sysP.fields.find(x => x.id === 'f_sk').list, pspP = pvP.fields.find(x => x.id === 'f_sk').list, libP = {}; sysP.items.forEach(i => { libP[i.id] = i; });
            const manyO = S.cleanListSpec({ stats: [{ key: 'A', kind: 'pick', opts: Array.from({ length: 60 }, (_, i) => ({ label: 'L' + i, name: 'N' + i })) }] }, true, F);
            check('F5a2 choice stats (cleanListSpec): kind pick keeps its options { label, name } — a label once ignoring case, a name one formula name (1+1 and an empty one dropped), 48 at most — and its default as an option\'s label as the list spells it; a choice is never the price; a number stat is unchanged',
                j(spP.stats[0]) === j({ key: 'Attr', label: 'Attr', kind: 'pick', opts: [{ label: 'Strength', name: 'ST' }, { label: 'Dexterity', name: 'DX' }, { label: 'Secret', name: 'GMFig' }], def: 'Strength', show: true }) && spP.price === undefined && manyO.stats[0].opts.length === 48, j([spP, manyO.stats[0].opts.length]));
            check('F5a2 an item stores a choice as its option\'s label as the list spells it (dexterity reads Dexterity), never a number or a label no option has; the players\' view drops an option reading a GM-only value and every item\'s value naming it (the label never travels), and is a fixed point',
                j(sysP.items.map(i => i.stats || null)) === j([{ Attr: 'Dexterity' }, { Attr: 'Secret' }, null, null]) && j(pvP.items.map(i => i.stats || null)) === j([{ Attr: 'Dexterity' }, null, null, null]) && j(pspP.stats[0].opts.map(o => o.label)) === j(['Strength', 'Dexterity'])
                && !/Secret|GMFig/.test(j(pvP)) && j(cleanSystem(pvP, { F, gmView: false })) === j(pvP), j([sysP.items.map(i => i.stats), pvP.items.map(i => i.stats), pspP]));
            const chP = { id: 'c_p', ownerId: 'u_p', values: { f_sk: [{ id: 'w_k', defId: 'i_kar', qty: 1, lvl: 3 }, { id: 'w_s', defId: 'i_sn', qty: 1, lvl: 2 }, { id: 'w_x', defId: 'i_x', qty: 1, lvl: 1 }] } };
            const aG = S.resolveAll(sysP, chP, F), chPO = S.charFor(chP, pvP, 'u_p', { lib: {}, items: libP }), aO = S.resolveAll(pvP, chPO, F), rP = S.makeResolver(sysP, chP, F);
            check('F5a2 formulas read a choice as its option\'s value (Row.Attr in a column, Skills.Karate.Attr): Dexterity reads DX, no choice reads the default (Strength: ST), the GM\'s secret option reads its GM-only value on the GM\'s sheet while its owner reads the default (never an error that tells there is a secret); a choice has no total; the sheet reads the label',
                j(aG.f_sk.cells.w_k.map(c => c.value)) === j([13]) && aG.f_sk.cells.w_s[0].value === 7 && aG.f_sk.cells.w_x[0].value === 13 && aG.f_ka.value === 10 && aO.f_sk.cells.w_s[0].value === 14 && aO.f_ka.value === 10 && rP('Skills.Attr') === undefined
                && S.rowStat(spP, libP.i_kar, 'Attr') === 'Dexterity' && S.rowStat(spP, {}, 'attr') === 'Strength', j([aG.f_sk.cells, aO.f_sk.cells, aG.f_ka, aO.f_ka]));
            const OVP = (q, pl) => S.applyRowOp(sysP, JSON.parse(j(chP)), 'f_sk', Object.assign({ op: 'ov', rowId: 'w_k' }, q), F, pl ? { player: true, view: pvP } : {}), rowK = r => (r.value || []).find(x => x.id === 'w_k');
            const o1 = OVP({ ov: { stats: { Attr: 'secret' } } }, true), o2 = OVP({ ov: { stats: { Attr: 'STRENGTH' } } }, true), o3 = OVP({ ov: { stats: { Attr: 'Secret' } } }, false), o4 = OVP({ ov: { stats: { Attr: 7 } } }, false), o5 = OVP({ ov: { stats: { Attr: 'dexterity' } } }, false);
            const cu1 = S.applyRowOp(sysP, JSON.parse(j(chP)), 'f_sk', { op: 'custom', rowId: 'w_n1', def: { name: 'Pazaak', stats: { Attr: 'secret' } } }, F, { player: true, view: pvP }), cu2 = S.applyRowOp(sysP, JSON.parse(j(chP)), 'f_sk', { op: 'custom', rowId: 'w_n2', def: { name: 'Pazaak', stats: { Attr: 'dexterity' } } }, F, { player: true, view: pvP });
            const agreeP = (() => { const q = { op: 'ov', rowId: 'w_k', ov: { stats: { Attr: 'strength' } } }, h = S.applyRowOp(sysP, JSON.parse(j(chP)), 'f_sk', q, F, { player: true, view: pvP }), cl = S.applyRowOp(pvP, { id: 'c_p', values: { f_sk: S.projectRows(JSON.parse(j(chP.values.f_sk)), pvP, libP, pspP) } }, 'f_sk', q, F, { player: true, view: pvP }); return h.ok && cl.ok && j(S.projectRows(h.value, pvP, libP, pspP)) === j(cl.value); })();
            check('F5a2 the host judges a choice against the actor\'s own options: a player\'s secret option is refused (value, never confirmed) and their label stored as the list spells it; the GM may choose the secret one; a number for a choice is refused; the library\'s own choice stores nothing; a custom row the same; the player\'s client and the host agree; the wire carries a label',
                !o1.ok && o1.reason === 'value' && o2.ok && j(rowK(o2).ov) === j({ stats: { Attr: 'Strength' } }) && o3.ok && j(rowK(o3).ov) === j({ stats: { Attr: 'Secret' }, held: ['Attr'] }) && !o4.ok && o4.reason === 'value' && o5.ok && rowK(o5).ov === undefined
                && !cu1.ok && cu1.reason === 'value' && cu2.ok && cu2.value.find(r => r.id === 'w_n2').def.stats.Attr === 'Dexterity' && agreeP
                && j(S.cleanCharItem({ type: 'char-item', rid: 'r', charId: 'c_p', fieldId: 'f_sk', op: 'ov', rowId: 'w_k', ov: { stats: { Attr: 'Strength' } } }).ov) === j({ stats: { Attr: 'Strength' } }), j([o1, o2.ok && rowK(o2), o3.ok && rowK(o3), o4, cu1, agreeP]));
            const vP = S.validateSystem(sysP, F), badP = JSON.parse(j(rawP)); badP.fields[3].list.stats.push({ key: 'Grip', kind: 'pick', opts: [] }, { key: 'Wild', kind: 'pick', opts: [{ label: 'Nothing', name: 'Nope' }] }); badP.fields.push({ id: 'f_g2', key: 'Gear', label: 'Gear', kind: 'item-list', vis: 'all', list: { stats: [{ key: 'Attr' }] } }, { id: 'f_tot', key: 'Tot', label: 'T', kind: 'formula', vis: 'all', formula: 'Skills.Attr' });
            const vPB = S.validateSystem(cleanSystem(badP, { F, gmView: true }), F), ePB = vPB.errors.map(e => e.message);
            check('F5a2 the validator: a choice with no options, an option reading no number formulas know, one key a choice here and a number there, and a total over a choice are errors; an option reading a GM-only value on a visible list warns (players do not get it)',
                vP.errors.length === 0 && vP.warnings.some(w => /option “Secret” reads a GM-only value/.test(w.message)) && ePB.some(m => /“Grip” is a choice with no options yet/.test(m)) && ePB.some(m => /option “Nothing” reads “Nope”, which is not a number/.test(m))
                && ePB.some(m => /“Attr” is a number here but a choice on Skills/.test(m)) && ePB.some(m => /"Skills\.Attr" is a choice, so it has no total/.test(m)), j([vP.errors, ePB]));
            // the sheet: the label on a chip; the Items tab, the copy form and the custom form offer the labels; the Lists card's Kind and Options
            const rawTP = JSON.parse(j(rawT)); rawTP.fields[0].list.custom = true; rawTP.fields[0].list.stats.push({ key: 'Grip', kind: 'pick', show: true, def: 'One hand', opts: [{ label: 'One hand', name: 'Acc' }, { label: 'Two hands', name: 'Dmg' }] }); rawTP.items[0].stats.Grip = 'two hands';
            const sysTP = cleanSystem(rawTP, { F, gmView: true }), rowsTP = clT(seedT()).concat([{ id: 'w_cg', qty: 1, def: { name: 'Club', stats: { Grip: 'One hand' } } }]);
            const gTP = draw5(W5, sysTP, 'f_wp', rowsTP, true), chipG = find5(lineOf(gTP, 'w_b1'), 'sheet-item-stat').map(c => kids5(c).map(k => k.textContent).join(''));
            const fTP = formAt(W5, sysTP, rowsTP, true, 'w_b1'), gripSel = walk5(fTP, partIs('rf-w_b1-s-Grip'), []).find(x => x.tag === 'select'), fCP = formAt(W5, sysTP, rowsTP, true, 'w_cg'), gripC = walk5(fCP, partIs('rf-w_cg-s-Grip'), []).find(x => x.tag === 'select');
            commits5.length = 0; if (gripSel) { gripSel.value = 'One hand'; gripSel.on.change(); } const gripCommit = commits5.slice(); commits5.length = 0; W5.setForm(null);
            const cardP = LC5(JSON.parse(j(rawTP.fields[0])), ['Gun']), kindS = find5(cardP, 'sys-list-statkind'), optsI = find5(cardP, 'sys-list-statopts')[0], pdefS = find5(cardP, 'sys-list-statpdef')[0];
            check('F5a2 the sheet: a choice shows its label on the row (Two hands); the copy form and the custom form offer its labels (the library\'s as the blank choice) and send the label; the Lists card has Kind (Number, Choice), and for a choice its Options as "label = name" and a default of its labels in place of a number\'s default and value names; the handlers write the draft (pinned); the price never offers a choice',
                chipG.indexOf('Grip Two hands') >= 0 && !!gripSel && j(kids5(gripSel).map(o => o.value)) === j(['', 'One hand', 'Two hands']) && kids5(gripSel)[0].textContent === 'Library: Two hands' && j(gripCommit) === j([['f_wp', { op: 'ov', rowId: 'w_b1', ov: { stats: { Grip: 'One hand' } } }]])
                && !!gripC && kids5(gripC).find(o => o.selected).value === 'One hand' && kindS.length === 6 && kids5(kindS[5]).find(o => o.selected).value === 'pick' && kids5(kindS[0]).find(o => o.selected).value === '' && !!optsI && optsI.value === 'One hand = Acc, Two hands = Dmg'
        ...
        {
            const html5c = fs.readFileSync(path.join(app, 'index.html'), 'utf8'), tut5c = fs.readFileSync(path.join(app, 'scripts', 'tutorial.js'), 'utf8'), netSrcR = fs.readFileSync(path.join(app, 'scripts', 'net.js'), 'utf8').replace(/\r\n/g, '\n'), diceSrcR = fs.readFileSync(path.join(app, 'scripts', 'dice.js'), 'utf8').replace(/\r\n/g, '\n');
            const rawB = { v: 1, name: 'B', rolls: [], fields: [
                { id: 'f_st', key: 'ST', label: 'ST', kind: 'number', vis: 'all', def: 12 }, { id: 'f_gm', key: 'GMFig', label: 'G', kind: 'number', vis: 'gm', def: 5 },
                { id: 'f_wp', key: 'Weapons', label: 'Weapons', kind: 'item-list', vis: 'all', edit: 'owner', list: { lvl: { label: 'Level', min: 0, max: 9, def: 1 }, stats: [{ key: 'Acc' }, { key: 'Grip', kind: 'pick', opts: [{ label: 'Strong', name: 'ST' }, { label: 'Secret', name: 'GMFig' }] }], cols: [{ key: 'Hit', label: 'Hit', formula: 'Row.Acc + Row.lvl' }, { key: 'Sly', label: 'Sly', formula: 'Row.lvl + GMFig' }],
                    rolls: [{ label: 'Attack', formula: 'd20 + Row.Hit' }, { label: 'Sneak', formula: 'd20 + Row.Sly' }, { label: '', formula: 'd6 + Row.Grip' }, { label: 'Four', formula: 'd4 + GMFig' }, { label: 'Five', formula: 'd4' }] } },
                { id: 'f_se', key: 'Secret', label: 'Secret', kind: 'item-list', vis: 'gm', edit: 'owner', list: { rolls: [{ label: 'X', formula: 'd6' }] } }],
                items: [{ id: 'i_bl', name: 'Blaster', stats: { Acc: 3, Grip: 'Strong' } }, { id: 'i_ru', name: 'Rune', vis: 'gm', stats: { Acc: 1 } }] };
            const sysB = cleanSystem(rawB, { F, gmView: true }), pvB = cleanSystem(sysB, { F, gmView: false }), spB = sysB.fields.find(x => x.id === 'f_wp').list, pspB = pvB.fields.find(x => x.id === 'f_wp').list;
            check('F5b a list\'s rolls (cleanListSpec): four at most, a label (blank: Roll) and a formula (dice allowed); the players\' view keeps a roll whose own formula names nothing GM-only (Sneak names Row.Sly; the column reads "GM only" there), drops one naming a GM-only value (Four: d4 + GMFig), and is a fixed point',
                spB.rolls.length === 4 && j(spB.rolls.map(r => r.label)) === j(['Attack', 'Sneak', 'Roll', 'Four']) && spB.rolls[0].formula === 'd20 + Row.Hit' && j(pspB.rolls.map(r => r.label)) === j(['Attack', 'Sneak', 'Roll']) && pspB.cols[1].formula === null && j(cleanSystem(pvB, { F, gmView: false })) === j(pvB), j([spB.rolls, pspB.rolls]));
            const chB = { id: 'c_b', ownerId: 'u_b', values: { f_wp: [{ id: 'w_b', defId: 'i_bl', qty: 1, lvl: 4 }, { id: 'w_r', defId: 'i_ru', qty: 1, lvl: 2 }, { id: 'w_h', qty: 1, hid: 1, def: { name: 'Kept' } }, { id: 'w_c', qty: 1, def: { name: 'Relic', vis: 'gm', stats: { Acc: 7 } } }] } };
            const rB = S.makeResolver(sysB, chB, F), rowB = rB.row('f_wp', 'w_b'), hitRes = F.evaluate('Row.Hit * 100 + Row.lvl + Row.Grip', { vars: rowB });
            check('F5b one row\'s names for a roll (fn.row): Row.* read that row (a column, its level, a choice\'s option value) and any other name the character; an unknown row, a kept curse or another field reads none',
                hitRes.ok && hitRes.value === 7 * 100 + 4 + 12 && rB.row('f_wp', 'w_nope') === null && rB.row('f_wp', 'w_h') === null && rB.row('f_st', 'w_b') === null, j([hitRes.value, rB.row('f_wp', 'w_h')]));
            const nm1 = S.rowRollNames(sysB, chB, 'f_wp', 'w_b', [{ name: 'Row.Sly' }, { name: 'Row.Grip' }], F), nm2 = S.rowRollNames(sysB, chB, 'f_wp', 'w_r', [], F), nm3 = S.rowRollNames(sysB, chB, 'f_wp', 'w_c', [], F), nm4 = S.rowRollNames(sysB, chB, 'f_wp', 'w_h', [], F), nm5 = S.rowRollNames(sysB, chB, 'f_se', 'w_b', [], F);
            const gmDer = S.gmOnlyNames(sysB, nm1.names).concat(S.gmDerivedNames(sysB, F, nm1.names));
            check('F5b the GM\'s privacy for a row roll (rowRollNames): a column Row.* reads as the list\'s name for it and the names its formula reads (Sly: Weapons.Sly, Row.lvl, GMFig), a choice as its option\'s name (Strong: ST); a GM-only item\'s row, a GM only custom row, a kept curse, a GM-only list or an unknown row keep the roll the GM\'s; the checks then find the GM-only value',
                !nm1.gm && j(nm1.names.map(n => n.name)) === j(['Weapons.Sly', 'GMFig', 'ST']) && nm2.gm && nm3.gm && nm4.gm && nm5.gm && gmDer.indexOf('GMFig') >= 0 && gmDer.indexOf('Weapons.Sly') >= 0, j([nm1, nm2.gm, nm3.gm, nm4.gm, nm5.gm, gmDer]));
            const badB = JSON.parse(j(rawB)); badB.fields[2].list.rolls = [{ label: 'Oops', formula: 'd20 + Nope' }, { label: 'Long', formula: 'd20 + Weapons.' + 'K'.repeat(60) + '.lvl' }, { label: 'Ok', formula: '3d6 <= Row.Hit + Row.lvl' }];
            const vBB = S.validateSystem(cleanSystem(badB, { F, gmView: true }), F), eBB = vBB.errors.map(e => e.id + '|' + e.prop + '|' + e.message);
            check('F5b the validator: a list roll allows dice and knows Row.* and the row\'s columns; its errors go under the list\'s card as "Roll “label”: …" (an unknown name, a name past 64 characters); a roll adds nothing to the loop graph',
                eBB.some(e => /^f_wp\|list\|Roll “Oops”: Unknown name "Nope"/.test(e)) && eBB.some(e => /^f_wp\|list\|Roll “Long”: A roll carries names of 64 characters at most/.test(e)) && !eBB.some(e => /Roll “Ok”|loop/.test(e)), j(eBB));
            // the sheet: the roll buttons (real), inert where nothing can be rolled; the Lists card's Rolls (real); the net and dice paths (pinned)
            const rolled = [], mkR = (live, can) => new Function(...deps5, 'canRoll', 'sheetRoll', itSrc5 + '\nreturn { itemListInto: itemListInto, itemTableInto: itemTableInto };')(fe5, (v, cls) => fe5('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, () => {}, opt5, S.fmtNum, v => v || '', S.LIMITS, live, doc5, () => {}, () => {}, () => {}, {}, S.rowStat, S.rowPaid, 'sheet', () => can, (e, cid, expr, label, o) => rolled.push([cid, expr, label, o]));
            const WR = mkR(true, true), gR = fe5('div'); WR.itemListInto(gR, sysB.fields.find(x => x.id === 'f_wp'), chB, chB.values.f_wp, sysB, false, true, true, '');
            const btnsB = find5(kids5(gR).find(l => find5(l, 'sheet-item-roll').length && /Blaster/.test(j(walk5(l, x => x.className === 'sheet-item-name', []).map(x => x.textContent)))) || fe5('x'), 'sheet-item-roll');
            if (btnsB[0]) btnsB[0].on.click({ shiftKey: false });
            const offR = fe5('div'); mkR(false, true).itemListInto(offR, sysB.fields.find(x => x.id === 'f_wp'), chB, chB.values.f_wp, sysB, false, true, true, ''); const noR = fe5('div'); mkR(true, false).itemListInto(noR, sysB.fields.find(x => x.id === 'f_wp'), chB, chB.values.f_wp, sysB, false, true, true, '');
            const tR = fe5('div'); WR.itemTableInto(tR, Object.assign({}, sysB.fields.find(x => x.id === 'f_wp'), { table: {} }), chB, chB.values.f_wp, sysB, false, true, true, '');
            ...
            const cardR = LCR(JSON.parse(j(sysB.fields.find(x => x.id === 'f_wp'))), []), four = LCR({ id: 'f_x', key: 'X', label: 'X', kind: 'item-list', list: { rolls: [1, 2, 3, 4].map(i => ({ label: 'R' + i, formula: 'd' + i })) } }, []);
        ...
        check('F4c2 tour, Help, the markup and the fixture: the Lists step (the Rules box) and the sheet step (✎, the dot, ↺, Follow the library, the locks, the reach); Help\'s line (only on lists shaped in the Lists tab, critic 14; the formulas and locks stay yours; Set by the GM); the Rules box\'s place; the dot\'s style; the d20 rows fixture lets players change their own stats, in both views',
            /<b>Item stats on players&rsquo; sheets<\/b>, at the top: GM only, or players may change their own copies&rsquo; stats\./.test(tut5) && /On a list shaped in Lists, <b>&#9998;<\/b> opens a copy&rsquo;s own values:[^']*<b>&#8634;<\/b> takes one back and <b>Follow the library<\/b> all of them; a copy can have its own removal and switch-off lock\. Saving an item a character carries says how far it reaches\./.test(tut5)
            && /<li><b>A copy&rsquo;s own values<\/b>: on a list shaped in the <b>Lists<\/b> tab[^<]*each row of a library item \(or of a deleted item&rsquo;s copy\) has a <b>&#9998;<\/b>; a plain list has none\./.test(html5) && /a gold <b>&bull;<\/b> marks it \(before the name, on a stat&rsquo;s chip or column, or in the row&rsquo;s &#128221; line for a stat not shown on the row;/.test(html5) && /The formulas and the locks stay on your machine\./.test(html5) && /their box reads <b>Set by the GM<\/b>/.test(html5) && /<b>Follow the library<\/b>/.test(html5)
            && html5.indexOf('<div id="sysListRules"></div>') > 0 && html5.indexOf('<div id="sysListRules"></div>') < html5.indexOf('<div id="sysListRows"') && html5.indexOf('<div id="sysListRules"></div>') > html5.indexOf('<div id="sysLists"') && /At the top, <b>Item stats on players&rsquo; sheets<\/b>/.test(html5)
            && /\.sheet-ov \{ color: var\(--gold\);/.test(css5) && /#sysCombatBox, #sysListRules \{ display: flex;/.test(css5) && j(d20.listRules) === j({ ownerStats: true }) && j(cleanSystem(d20, { F, gmView: false }).listRules) === j({ ownerStats: true }));
    }
    ...
    /* ---- Stage 6 HUD frame (HF2a): the HUD window — one per character, drawn from sys.sheet.hud by the sheet's own renderer ---- */
    {
        ...
        check('HUD frame HF2a: the window\'s CSS lives in the HUD block (after the look fold\'s): the panel, the grip strip, the twins of the sheet panel\'s field rules, the palette on its head, GM-only fields hidden from players as on the sheet; the glyph files exist',
            ...
            && ['wave-square'].every(g => fs.existsSync(path.join(app, 'assets', 'icons', 'fa', 'solid', g + '.svg')) && blk.indexOf('assets/icons/fa/solid/' + g + '.svg') > 0) && !/Stage 6 look fold \(L/.test(blk)
