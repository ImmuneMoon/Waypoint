...
(async () => {
    ...
    /* ---- Stage 6 F4b: row facts (level, switch, note), list options, the equip lock, the Lists tab ---- */
    {
        ...
        const deps = ['el', 'iconNode', 'rowDef', 'rowIdOf', 'rowLvl', 'rowOn', 'commitItem', 'opt', 'fmtNum', 'iconText', 'LIMITS', '_fxLive', 'document', 'renderViews', 'closeHud', 'closeSheet', 'window', 'systemOf', 'getActiveCampaign', 'facingTarget', 'uid', 'F', 'fxText', 'canRoll', 'sheetRoll', 'commit', 'valueTone', 'TONE_CLASS', 'rowStat', 'rowPaid'];
        const depV = [fe, (v, cls) => fe('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, (c, f, q) => commits.push([f.id, q]), optF, S.fmtNum, v => v || '', S.LIMITS, true, docF, () => {}, () => {}, () => {}, {}, () => sysB, () => null, () => null, p => p + 'new', () => F, S.fxText, () => true, () => {}, () => {}, S.valueTone, {}, S.rowStat, S.rowPaid];
    ...
    /* ---- Stage 6 F4b review: the lock covers dropping while it is on; a stored "on"; the level clamp's fixed point; the UI findings ---- */
    {
        ...
        const WR = new Function('el', 'iconNode', 'rowDef', 'rowIdOf', 'rowLvl', 'rowOn', 'commitItem', 'opt', 'fmtNum', 'iconText', 'LIMITS', '_fxLive', 'document', 'renderViews', 'closeHud', 'closeSheet', 'window', 'rowStat', 'rowPaid', cutR('// ---- item-list widgets', '// 5h: an effect\'s changes as short text') + '\nreturn { itemListInto: itemListInto };')(
            feR, (v, cls) => feR('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, () => {}, optR, S.fmtNum, v => v || '', S.LIMITS, true, docR, () => {}, () => {}, () => {}, {}, S.rowStat, S.rowPaid);
    ...
    /* ---- Stage 6 F4c1: item stats, the price and what was paid ---- */
    {
        ...
        const itSrc4 = cutS('// ---- item-list widgets', '// 5h: an effect\'s changes as short text'), deps4 = ['el', 'iconNode', 'rowDef', 'rowIdOf', 'rowLvl', 'rowOn', 'commitItem', 'opt', 'fmtNum', 'iconText', 'LIMITS', '_fxLive', 'document', 'renderViews', 'closeHud', 'closeSheet', 'window', 'rowStat', 'rowPaid'];
        const mkW = live => new Function(...deps4, itSrc4 + '\nreturn { itemListInto: itemListInto, itemTableInto: itemTableInto, statFmt: statFmt, statText: statText };')(fe4, (v, cls) => fe4('span', cls, v), S.rowDef, S.rowIdOf, S.rowLvl, S.rowOn, (c, f, q) => commits4.push([f.id, q]), opt4, S.fmtNum, v => v || '', S.LIMITS, live, doc4, () => {}, () => {}, () => {}, {}, S.rowStat, S.rowPaid);
        ...
        check('F4c1 the sheet (the GM, a list with stats): the row reads name, a chip per stat shown On the row (a small label, then its value), the switch, the Paid box, 📝, the quantity and ×; a category chip too while its rows span two categories; the 📝 line reads every stat (a value name for a named one) and what one cost (the list price where none was recorded); a plain list and an F4b list draw as before',
            j(cl4(r0)) === j(['sheet-item-name', 'sheet-item-stat', 'sheet-item-stat', 'sheet-item-onl', 'sheet-item-paidl', 'sheet-item-notes-t', 'sheet-item-qty', 'sheet-item-rm']) && j(cl4(gWK.children[0])) === j(['sheet-item-name', 'sheet-chip', 'sheet-item-stat', 'sheet-item-stat', 'sheet-item-onl', 'sheet-item-paidl', 'sheet-item-notes-t', 'sheet-item-qty', 'sheet-item-rm'])
        ...
        const apiLine = (fs.readFileSync(path.join(app, 'scripts', 'systemcore.js'), 'utf8').match(/^var API = \{[^\n]*\};/m) || [''])[0], notExp = scImp.filter(n => !(n in S)), notApi = ['STAT_KEY', 'statKey', 'cleanEntryStats', 'rowStats', 'rowStat', 'rowPaid', 'cleanListSpec'].filter(n => !(n in S) || !new RegExp('[{,] ' + n + ': ' + n + '[,}\\s]').test(apiLine));
        check('F4c1 (critic 3): every name a sheets.js slice runs with is declared in sheets.js or imported by it, every name it imports from systemcore is exported there, and the new reads are in both the export list and the window API',
            missing4.length === 0 && scImp.length > 60 && notExp.length === 0 && notApi.length === 0 && ['cleanListSpec', 'STAT_KEY', 'statKey', 'rowStat', 'rowPaid'].every(n => scImp.indexOf(n) >= 0), j({ missing4, notExp, notApi }));
    }
