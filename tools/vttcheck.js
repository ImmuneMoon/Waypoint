...
(async () => {
    ...
    /* ---- the roster ---- */
    check('roster is elevation, posture, minimap, sound, dice, sheets, fx, fog, turning, music, turns with labels', V.FEATURES.map(f => f.id).join(',') === 'elevation,posture,minimap,sound,dice,sheets,fx,fog,turning,music,turns' && V.FEATURES.every(f => typeof f.label === 'string' && f.label));
    ...
    check('turns (turn-based combat) is the last roster feature and the first opt-in one: GM-controlled (noLocal), OFF by default (def false), no legacy key', V.FEATURES[10].id === 'turns' && V.FEATURES[10].noLocal === true && V.FEATURES[10].def === false && V.FEATURES[10].legacyKey === null && V.FEATURES.filter(f => f.def === false).length === 1);
    ...
    check('solo: C over the on-screen campaign', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog1turning1music1turns0');
    ...
    check('solo: hostFlags = C of the active campaign', j(V.hostFlags()) === j({ elevation: false, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true, turns: false }));
    ...
    check('master off: every feature off, hostFlags all false (no master on the wire)', eff() === 'elevation0posture0minimap0sound0dice0sheets0fx0fog0turning0music0turns0' && Object.values(V.hostFlags()).every(x => x === false));
    ...
    store.clear();
    world([camp('B')], 'B');
    check('no camp.vtt, no keys: on', eff() === 'elevation1posture1minimap1sound1dice1sheets1fx1fog1turning1music1turns0');
    localStorage.setItem('wp_elevation', 'off');
    check('no camp.vtt: the legacy key is the seed', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog1turning1music1turns0');
    world([camp('B', { v: 1, master: true, features: { elevation: 'yes', posture: null } })], 'B');
    check('non-boolean feature values fall back per feature', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog1turning1music1turns0');
    ...
    localStorage.setItem('wp_elevation', 'off');
    let c = camp('C');
    check('fill adds vtt from the default (legacy elevation off) and reports a change', V.fill(c) === true && j(c.vtt) === j({ v: 1, master: true, features: { elevation: false, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true, turns: false } }));
    ...
    check('allOn is every roster feature on (fog included)', j(V.allOn()) === j({ v: 1, master: true, features: { elevation: true, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true, turns: true } }));
    ...
    localStorage.setItem('wp_vtt_global', j({ v: 1, master: true, features: { elevation: false, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true } }));
    c = camp('G', { v: 1, master: true, features: { elevation: true, posture: false, minimap: false, sound: true, dice: true, sheets: true, fx: true, fog: false, turning: true, music: true, turns: false, extra: 1 } });
    ...
    check('host: mode, not locked, C, hostFlags = what players receive', V.mode() === 'host' && !V.locked() && eff() === 'elevation0posture1minimap0sound1dice1sheets1fx1fog1turning1music1turns0' && j(V.hostFlags()) === j({ elevation: false, posture: true, minimap: false, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true, turns: false }));
    ...
    check('awaiting: mode, locked, C over the OWN campaign (a stale stance is ignored)', V.mode() === 'awaiting' && V.locked() && eff() === 'elevation1posture1minimap1sound1dice1sheets1fx1fog1turning1music1turns0' && V.ceiling() === null);
    ...
    check('joined: mode client, locked, G not C', V.mode() === 'client' && V.locked() && eff() === 'elevation1posture0minimap1sound1dice1sheets1fx1fog0turning0music0turns0');
    ...
    check('joined: stanceCamps[onScreen] beats the legacy stance', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog0turning0music0turns0');
    state.appState.activeCampaignId = 'Y'; state.appState.campaigns.Y = camp('Y');
    check('joined: a stage onto Y reads Y\'s flags and Y\'s table key', eff() === 'elevation1posture1minimap0sound1dice1sheets1fx1fog0turning0music0turns0' && V.tableKey() === 't:Y__u_gm1');
    state.appState.activeCampaignId = 'X';
    check('cleanStanceCamps: roster ids only, hostile ids dropped, absent map null', (() => { const m = V.cleanStanceCamps({ X: { elevation: 0, posture: 1, junk: true }, 'bad id!': {}, '': {} }); return m && j(m.X) === j({ elevation: false, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true, turns: false }) && !('bad id!' in m) && Object.keys(m).length === 1 && V.cleanStanceCamps(null) === null; })());
    ...
    check('cleanStanceCamps: an own __proto__ campaign id lands as a plain flags entry on the null-prototype map, Object.prototype untouched', (() => {
        ...
        return m && Object.keys(m).length === 2 && Object.prototype.hasOwnProperty.call(m, '__proto__') && j(m['__proto__']) === j({ elevation: true, posture: false, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true, turns: false })
    ...
    gmA.active = false;
    check('reconnect gap: ceiling retained', V.mode() === 'client' && eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog0turning0music0turns0');
    ...
    check('after load(): solo, C over the own campaign, no ceiling, no table key', V.mode() === 'solo' && V.ceiling() === null && eff() === 'elevation0posture1minimap0sound1dice1sheets1fx1fog1turning1music1turns0' && V.tableKey() === '');
    ...
    check('stream: C over the disk campaign, the stance and local-off never applied, group hidden (mode stream)', V.mode() === 'stream' && V.ceiling() === null && eff() === 'elevation0posture1minimap0sound1dice1sheets1fx1fog1turning1music1turns0' && V.locked());

    /* ---- an old host: two-key stance payload ---- */
    check('cleanFlags: an absent key takes its default (minimap ON, fog ON); present keys are booleans', j(V.cleanFlags({ elevation: true, posture: 0 })) === j({ elevation: true, posture: false, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true, turns: false }));
    check('cleanFlags: no payload is every feature on', j(V.cleanFlags(undefined)) === j({ elevation: true, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true, turns: false }));
    net({ active: true, role: 'client', foreign: true, gmId: '', notepad: { gmId: 'u_old' }, stance: V.cleanFlags({ elevation: false, posture: true }) });
    world([camp('X')], 'X');
    check('old host: minimap on, elevation off; table key falls back to notepad.gmId', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog1turning1music1turns0' && V.tableKey() === 't:X__u_old');
    ...
    localStorage.setItem('wp_vtt_global', j({ v: 1, master: true, features: { elevation: false, posture: false, minimap: true, sound: true, dice: true, sheets: true, fx: true } }));
    const gmE = net({ active: true, role: 'client', foreign: true, gmId: 'u_gm5', stance: { elevation: false, posture: false, minimap: true, sound: true, dice: true, sheets: true, fx: true, music: true } });
    world([camp('T')], 'T');
    V.joined();
    ...
    prev = gmE.stance; gmE.stance = { elevation: true, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, music: true };
    V.ceilingChanged(prev, 'T');
    t = JSON.parse(localStorage.getItem('wp_vtt_local')).tables['t:T__u_gm5'];
    ...
    const gmC = net({ active: true, role: 'client', foreign: true, gmId: 'u_gm3', stance: { elevation: true, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, music: true } });
    V.joined();
    ...
    const gmF = net({ active: true, role: 'client', foreign: true, gmId: 'u_fog', stance: { elevation: true, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true, turns: false } });
    world([camp('FT')], 'FT');
    ...
    const nFog = toasts.length; let pFog = gmF.stance; gmF.stance = { elevation: true, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: false, music: true };
