...
(async () => {
    ...
    /* ---- the roster ---- */
    check('roster is elevation, posture, minimap, sound, dice, sheets, fx, fog, turning, music with labels', V.FEATURES.map(f => f.id).join(',') === 'elevation,posture,minimap,sound,dice,sheets,fx,fog,turning,music' && V.FEATURES.every(f => typeof f.label === 'string' && f.label));
    ...
    check('music is the last roster feature: a self-toggle (not noLocal), on by default, no legacy key or def flag', V.FEATURES[9].id === 'music' && V.FEATURES[9].legacyKey === null && V.FEATURES[9].noLocal === undefined && V.FEATURES[9].def === undefined);
    ...
    check('solo: C over the on-screen campaign', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog1turning1music1');
    ...
    check('solo: hostFlags = C of the active campaign', j(V.hostFlags()) === j({ elevation: false, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true }));
    ...
    check('master off: every feature off, hostFlags all false (no master on the wire)', eff() === 'elevation0posture0minimap0sound0dice0sheets0fx0fog0turning0music0' && Object.values(V.hostFlags()).every(x => x === false));
    ...
    store.clear();
    world([camp('B')], 'B');
    check('no camp.vtt, no keys: on', eff() === 'elevation1posture1minimap1sound1dice1sheets1fx1fog1turning1music1');
    localStorage.setItem('wp_elevation', 'off');
    check('no camp.vtt: the legacy key is the seed', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog1turning1music1');
    world([camp('B', { v: 1, master: true, features: { elevation: 'yes', posture: null } })], 'B');
    check('non-boolean feature values fall back per feature', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog1turning1music1');
    ...
    localStorage.setItem('wp_elevation', 'off');
    let c = camp('C');
    check('fill adds vtt from the default (legacy elevation off) and reports a change', V.fill(c) === true && j(c.vtt) === j({ v: 1, master: true, features: { elevation: false, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true } }));
    ...
    check('allOn is every roster feature on (fog included)', j(V.allOn()) === j({ v: 1, master: true, features: { elevation: true, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true } }));
    ...
    localStorage.setItem('wp_vtt_global', j({ v: 1, master: true, features: { elevation: false, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true } }));
    c = camp('G', { v: 1, master: true, features: { elevation: true, posture: false, minimap: false, sound: true, dice: true, sheets: true, fx: true, fog: false, turning: true, music: true, extra: 1 } });
    ...
    check('host: mode, not locked, C, hostFlags = what players receive', V.mode() === 'host' && !V.locked() && eff() === 'elevation0posture1minimap0sound1dice1sheets1fx1fog1turning1music1' && j(V.hostFlags()) === j({ elevation: false, posture: true, minimap: false, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true }));
    ...
    check('awaiting: mode, locked, C over the OWN campaign (a stale stance is ignored)', V.mode() === 'awaiting' && V.locked() && eff() === 'elevation1posture1minimap1sound1dice1sheets1fx1fog1turning1music1' && V.ceiling() === null);
    ...
    check('joined: mode client, locked, G not C', V.mode() === 'client' && V.locked() && eff() === 'elevation1posture0minimap1sound1dice1sheets1fx1fog0turning0music0');
    ...
    check('joined: stanceCamps[onScreen] beats the legacy stance', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog0turning0music0');
    state.appState.activeCampaignId = 'Y'; state.appState.campaigns.Y = camp('Y');
    check('joined: a stage onto Y reads Y\'s flags and Y\'s table key', eff() === 'elevation1posture1minimap0sound1dice1sheets1fx1fog0turning0music0' && V.tableKey() === 't:Y__u_gm1');
    state.appState.activeCampaignId = 'X';
    check('cleanStanceCamps: roster ids only, hostile ids dropped, absent map null', (() => { const m = V.cleanStanceCamps({ X: { elevation: 0, posture: 1, junk: true }, 'bad id!': {}, '': {} }); return m && j(m.X) === j({ elevation: false, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true }) && !('bad id!' in m) && Object.keys(m).length === 1 && V.cleanStanceCamps(null) === null; })());
    ...
    check('cleanStanceCamps: an own __proto__ campaign id lands as a plain flags entry on the null-prototype map, Object.prototype untouched', (() => {
        ...
        return m && Object.keys(m).length === 2 && Object.prototype.hasOwnProperty.call(m, '__proto__') && j(m['__proto__']) === j({ elevation: true, posture: false, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true })
    ...
    gmA.active = false;
    check('reconnect gap: ceiling retained', V.mode() === 'client' && eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog0turning0music0');
    ...
    check('after load(): solo, C over the own campaign, no ceiling, no table key', V.mode() === 'solo' && V.ceiling() === null && eff() === 'elevation0posture1minimap0sound1dice1sheets1fx1fog1turning1music1' && V.tableKey() === '');
    ...
    check('stream: C over the disk campaign, the stance and local-off never applied, group hidden (mode stream)', V.mode() === 'stream' && V.ceiling() === null && eff() === 'elevation0posture1minimap0sound1dice1sheets1fx1fog1turning1music1' && V.locked());

    /* ---- an old host: two-key stance payload ---- */
    check('cleanFlags: an absent key takes its default (minimap ON, fog ON); present keys are booleans', j(V.cleanFlags({ elevation: true, posture: 0 })) === j({ elevation: true, posture: false, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true }));
    check('cleanFlags: no payload is every feature on', j(V.cleanFlags(undefined)) === j({ elevation: true, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true }));
    net({ active: true, role: 'client', foreign: true, gmId: '', notepad: { gmId: 'u_old' }, stance: V.cleanFlags({ elevation: false, posture: true }) });
    world([camp('X')], 'X');
    check('old host: minimap on, elevation off; table key falls back to notepad.gmId', eff() === 'elevation0posture1minimap1sound1dice1sheets1fx1fog1turning1music1' && V.tableKey() === 't:X__u_old');
    ...
    const gmF = net({ active: true, role: 'client', foreign: true, gmId: 'u_fog', stance: { elevation: true, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true } });
    world([camp('FT')], 'FT');
    ...
    pFog = gmF.stance; gmF.stance = { elevation: false, posture: true, minimap: true, sound: true, dice: true, sheets: true, fx: true, fog: true, turning: true, music: true };   // a self-toggle feature AND fog both move
    V.ceilingChanged(pFog, 'FT');
